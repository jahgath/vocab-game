export const GROUP_1 = [
  {
    word: "abound",
    meaning: "be present in large quantities",
    partOfSpeech: "Verb",
    examples: [
      "Wildlife abounds in this untouched forest.",
      "Opportunities abound for skilled workers in this industry.",
    ],
    antonyms: ["lack", "want"],
    synonyms: ["teem", "overflow"],
  },
  {
    word: "amorphous",
    meaning: "lacking structure; unclear",
    partOfSpeech: "Adjective",
    examples: [
      "The amorphous cloud of gas slowly took shape.",
      "Her amorphous career path made it hard to define her expertise.",
    ],
    antonyms: ["structured", "defined"],
    synonyms: ["shapeless", "formless"],
  },
  {
    word: "austere",
    meaning: "(of a person) strict and stern in character",
    partOfSpeech: "Adjective",
    examples: [
      "The austere teacher tolerated no nonsense in her classroom.",
      "His austere demeanor made him seem unapproachable.",
    ],
    antonyms: ["indulgent", "lenient"],
    synonyms: ["severe", "stern"],
  },
  {
    word: "austere",
    meaning: "(of living conditions) lacking luxury; extremely harsh",
    partOfSpeech: "Adjective",
    examples: [
      "The monks lived in austere conditions with minimal possessions.",
      "The prison cell was austere, containing only a cot and toilet.",
    ],
    antonyms: ["luxurious", "opulent"],
    synonyms: ["spartan", "ascetic"],
  },
  {
    word: "belie",
    meaning: "give a false impression of; mask",
    partOfSpeech: "Verb",
    examples: [
      "His calm voice belied the anger he felt inside.",
      "The cheerful decor belied the building's troubled history.",
    ],
    antonyms: ["reveal", "expose"],
    synonyms: ["misrepresent", "disguise"],
  },
  {
    word: "capricious",
    meaning: "having varying moods; fickle",
    partOfSpeech: "Adjective",
    examples: [
      "The capricious weather changed from sunny to stormy in minutes.",
      "Dealing with a capricious boss required constant adaptation.",
    ],
    antonyms: ["consistent", "steady"],
    synonyms: ["mercurial", "temperamental"],
  },
  {
    word: "cerebral",
    meaning: "related to the brain",
    partOfSpeech: "Adjective",
    examples: [
      "The cerebral hemorrhage required immediate medical attention.",
      "Doctors studied the cerebral activity during the experiment.",
    ],
    antonyms: ["physical", "bodily"],
    synonyms: ["neural", "intellectual"],
  },
  {
    word: "cerebral",
    meaning: "intellectual; focused on learning rather than emotion",
    partOfSpeech: "Adjective",
    examples: [
      "She preferred cerebral puzzles over physical challenges.",
      "The film was too cerebral for mainstream audiences.",
    ],
    antonyms: ["emotional", "instinctive"],
    synonyms: ["thoughtful", "highbrow"],
  },
  {
    word: "congenial",
    meaning:
      "(of a person) likable and friendly, often due to shared interests",
    partOfSpeech: "Adjective",
    examples: [
      "We found our new neighbors to be quite congenial.",
      "The congenial professor made learning enjoyable.",
    ],
    antonyms: ["unfriendly", "disagreeable"],
    synonyms: ["amicable", "affable"],
  },
  {
    word: "congenial",
    meaning: "(of a thing) enjoyable because it aligns with one's tendencies",
    partOfSpeech: "Adjective",
    examples: [
      "The quiet café provided a congenial atmosphere for writing.",
      "He found the company's values congenial to his own.",
    ],
    antonyms: ["inhospitable", "unpleasant"],
    synonyms: ["compatible", "agreeable"],
  },
  {
    word: "conspicuous",
    meaning: "easily noticed; obvious",
    partOfSpeech: "Adjective",
    examples: [
      "Her bright red coat made her conspicuous in the crowd.",
      "There was a conspicuous absence of managers at the meeting.",
    ],
    antonyms: ["inconspicuous", "unnoticeable"],
    synonyms: ["prominent", "obvious"],
  },
  {
    word: "cursory",
    meaning: "lacking effort; without care",
    partOfSpeech: "Adjective",
    examples: [
      "He gave the report only a cursory glance before signing.",
      "The cursory inspection missed several safety violations.",
    ],
    antonyms: ["thorough", "detailed"],
    synonyms: ["perfunctory", "superficial"],
  },
  {
    word: "daunting",
    meaning: "scary or frightening; (of a task) seeming impossible",
    partOfSpeech: "Adjective",
    examples: [
      "The sheer size of the project was daunting.",
      "Facing the champion boxer seemed a daunting prospect.",
    ],
    antonyms: ["encouraging", "reassuring"],
    synonyms: ["intimidating", "formidable"],
  },
  {
    word: "deify",
    meaning: "look up to someone or something as a god",
    partOfSpeech: "Verb",
    examples: [
      "Some fans deify their favorite celebrities.",
      "Ancient cultures would often deify natural phenomena.",
    ],
    antonyms: ["vilify", "demean"],
    synonyms: ["idolize", "worship"],
  },
  {
    word: "didactic",
    meaning:
      "meant for teaching; educational, sometimes in a condescending manner",
    partOfSpeech: "Adjective",
    examples: [
      "The fable had a clear didactic purpose about honesty.",
      "Her didactic tone made the students feel patronized.",
    ],
    antonyms: ["uninstructive", "nondidactic"],
    synonyms: ["pedagogical", "preachy"],
  },
  {
    word: "disseminate",
    meaning: "distribute or circulate broadly",
    partOfSpeech: "Verb",
    examples: [
      "The organization works to disseminate health information.",
      "Social media helps disseminate news quickly.",
    ],
    antonyms: ["withhold", "suppress"],
    synonyms: ["spread", "circulate"],
  },
  {
    word: "feasible",
    meaning: "likely to be achieved; practical",
    partOfSpeech: "Adjective",
    examples: [
      "The construction plan seems feasible within our budget.",
      "Is it feasible to complete this project by Friday?",
    ],
    antonyms: ["impossible", "impractical"],
    synonyms: ["viable", "workable"],
  },
  {
    word: "flout",
    meaning: "break a rule without guilt",
    partOfSpeech: "Verb",
    examples: [
      "The company continued to flout environmental regulations.",
      "He openly flouted the dress code by wearing jeans.",
    ],
    antonyms: ["obey", "comply"],
    synonyms: ["defy", "disregard"],
  },
  {
    word: "homogeneous",
    meaning: "composed of similar parts; alike",
    partOfSpeech: "Adjective",
    examples: [
      "The neighborhood was homogeneous, with nearly identical houses.",
      "The team lacked diversity, being homogeneous in background.",
    ],
    antonyms: ["diverse", "heterogeneous"],
    synonyms: ["uniform", "similar"],
  },
  {
    word: "humdrum",
    meaning: "boring; unlikely to excite",
    partOfSpeech: "Adjective",
    examples: [
      "She longed to escape her humdrum routine.",
      "The meeting was a humdrum affair with little discussion.",
    ],
    antonyms: ["exciting", "stimulating"],
    synonyms: ["mundane", "tedious"],
  },
  {
    word: "insipid",
    meaning: "without flavor; lacking energy",
    partOfSpeech: "Adjective",
    examples: [
      "The soup was insipid and needed more seasoning.",
      "His insipid presentation failed to engage the audience.",
    ],
    antonyms: ["flavorful", "exciting"],
    synonyms: ["bland", "vapid"],
  },
  {
    word: "loquacious",
    meaning: "likely to talk a lot; wordy",
    partOfSpeech: "Adjective",
    examples: [
      "The loquacious tour guide shared endless stories.",
      "Her loquacious nature made her perfect for radio.",
    ],
    antonyms: ["taciturn", "reticent"],
    synonyms: ["talkative", "verbose"],
  },
  {
    word: "misanthropic",
    meaning: "having a distaste for humans and society",
    partOfSpeech: "Adjective",
    examples: [
      "The misanthropic old man refused all visitors.",
      "His misanthropic views made him avoid crowded places.",
    ],
    antonyms: ["philanthropic", "sociable"],
    synonyms: ["antisocial", "cynical"],
  },
  {
    word: "misnomer",
    meaning: "a misleading or inaccurate name",
    partOfSpeech: "Noun",
    examples: [
      "'Fun Size' candy bars are a misnomer for such small portions.",
      "The 'common cold' is a misnomer since there are many viruses.",
    ],
    antonyms: ["apt name", "proper term"],
    synonyms: ["wrong name", "incorrect label"],
  },
  {
    word: "negligent",
    meaning: "lacking care or attention",
    partOfSpeech: "Adjective",
    examples: [
      "The negligent driver caused the accident.",
      "The landlord was negligent in maintaining the property.",
    ],
    antonyms: ["careful", "attentive"],
    synonyms: ["careless", "remiss"],
  },
  {
    word: "obsequious",
    meaning: "excessively submissive to authority",
    partOfSpeech: "Adjective",
    examples: [
      "The obsequious assistant agreed with everything the boss said.",
      "His obsequious behavior made his colleagues uncomfortable.",
    ],
    antonyms: ["assertive", "independent"],
    synonyms: ["servile", "sycophantic"],
  },
  {
    word: "placate",
    meaning: "decrease someone's anger or other negative emotion",
    partOfSpeech: "Verb",
    examples: [
      "The manager tried to placate the angry customer.",
      "They offered concessions to placate the protesters.",
    ],
    antonyms: ["aggravate", "provoke"],
    synonyms: ["appease", "mollify"],
  },
  {
    word: "proclivity",
    meaning: "a fondness or liking for something",
    partOfSpeech: "Noun",
    examples: [
      "She had a proclivity for spicy foods.",
      "His early proclivity for music led to a career in composition.",
    ],
    antonyms: ["aversion", "disinclination"],
    synonyms: ["inclination", "penchant"],
  },
  {
    word: "puerile",
    meaning: "immature and childish",
    partOfSpeech: "Adjective",
    examples: [
      "The puerile jokes were inappropriate for the meeting.",
      "His puerile behavior embarrassed his colleagues.",
    ],
    antonyms: ["mature", "sophisticated"],
    synonyms: ["juvenile", "infantile"],
  },
  {
    word: "quixotic",
    meaning: "excessively visionary; unfeasible",
    partOfSpeech: "Adjective",
    examples: [
      "His quixotic plan to end world hunger was unrealistic.",
      "The quixotic quest for perfection wasted valuable time.",
    ],
    antonyms: ["practical", "realistic"],
    synonyms: ["idealistic", "utopian"],
  },
  {
    word: "spendthrift",
    meaning: "someone who spends money recklessly",
    partOfSpeech: "Noun",
    examples: [
      "The spendthrift heir quickly wasted his inheritance.",
      "Her spendthrift habits left her constantly in debt.",
    ],
    antonyms: ["miser", "saver"],
    synonyms: ["prodigal", "wastrel"],
  },
  {
    word: "taciturn",
    meaning: "of few words; speaking little",
    partOfSpeech: "Adjective",
    examples: [
      "The taciturn cowboy rarely joined conversations.",
      "His taciturn nature made him seem mysterious.",
    ],
    antonyms: ["talkative", "loquacious"],
    synonyms: ["reticent", "reserved"],
  },
  {
    word: "wary",
    meaning: "exercising caution; careful",
    partOfSpeech: "Adjective",
    examples: [
      "Investors were wary of putting money into the new venture.",
      "She grew wary of strangers after the incident.",
    ],
    antonyms: ["heedless", "trusting"],
    synonyms: ["cautious", "vigilant"],
  },
];

export const GROUP_2 = [
  {
    word: "adulterate",
    meaning: "damage the quality of; corrupt",
    partOfSpeech: "Verb",
    examples: [
      "The company was fined for adulterating their products with cheap fillers.",
      "Adding water to milk adulterates its nutritional value.",
    ],
    synonyms: ["contaminate", "degrade", "taint"],
    antonyms: ["purify", "refine"],
  },
  {
    word: "advocate",
    meaning: "support; be in favor of",
    partOfSpeech: "Verb",
    examples: [
      "She advocates for better healthcare policies.",
      "The professor advocates using primary sources in research.",
    ],
    synonyms: ["support", "champion", "promote"],
    antonyms: ["oppose", "criticize"],
  },
  {
    word: "advocate",
    meaning: "someone who supports a cause, organization, or person",
    partOfSpeech: "Noun",
    examples: [
      "He works as a child advocate in family court.",
      "The environmental advocate spoke passionately about conservation.",
    ],
    synonyms: ["supporter", "proponent", "defender"],
    antonyms: ["opponent", "adversary"],
  },
  {
    word: "aggrandize",
    meaning: "enhance one's power or standing",
    partOfSpeech: "Verb",
    examples: [
      "The dictator sought to aggrandize himself through propaganda.",
      "She aggrandized her reputation by taking credit for others' work.",
    ],
    synonyms: ["exaggerate", "magnify", "elevate"],
    antonyms: ["diminish", "belittle"],
  },
  {
    word: "alacrity",
    meaning: "eagerness to do something",
    partOfSpeech: "Noun",
    examples: [
      "She accepted the challenge with alacrity.",
      "The staff responded with alacrity to the emergency situation.",
    ],
    synonyms: ["eagerness", "willingness", "readiness"],
    antonyms: ["reluctance", "hesitation"],
  },
  {
    word: "ambivalent",
    meaning: "having conflicting emotions; unsure",
    partOfSpeech: "Adjective",
    examples: [
      "I feel ambivalent about the job offer - it pays well but requires relocation.",
      "His ambivalent attitude made decision-making difficult.",
    ],
    synonyms: ["uncertain", "conflicted", "torn"],
    antonyms: ["decisive", "resolute"],
  },
  {
    word: "ameliorate",
    meaning: "improve a bad feeling or bad situation",
    partOfSpeech: "Verb",
    examples: [
      "The new policy ameliorated working conditions.",
      "Medication helped ameliorate her chronic pain.",
    ],
    synonyms: ["improve", "enhance", "relieve"],
    antonyms: ["worsen", "aggravate"],
  },
  {
    word: "amenable",
    meaning: "(of a person) receptive to change; open",
    partOfSpeech: "Adjective",
    examples: [
      "The team was amenable to trying the new strategy.",
      "She's generally amenable to constructive criticism.",
    ],
    synonyms: ["receptive", "open-minded", "agreeable"],
    antonyms: ["resistant", "uncooperative"],
  },
  {
    word: "amenable",
    meaning: "(of a thing) responsive to",
    partOfSpeech: "Adjective",
    examples: [
      "This material is amenable to various shaping techniques.",
      "The disease is amenable to treatment with antibiotics.",
    ],
    synonyms: ["responsive", "adaptable", "susceptible"],
    antonyms: ["immune", "resistant"],
  },
  {
    word: "anachronistic",
    meaning: "of the wrong time or era",
    partOfSpeech: "Adjective",
    examples: [
      "The film's depiction of smartphones in the 1920s was anachronistic.",
      "His views on gender roles seem anachronistic in modern society.",
    ],
    synonyms: ["outdated", "obsolete", "anachronous"],
    antonyms: ["timely", "contemporary"],
  },
  {
    word: "audacious",
    meaning: "bold; unafraid of risks",
    partOfSpeech: "Adjective",
    examples: [
      "Her audacious plan to climb the mountain alone succeeded.",
      "The startup made an audacious bid for the contract.",
    ],
    synonyms: ["bold", "daring", "fearless"],
    antonyms: ["timid", "cautious"],
  },
  {
    word: "audacious",
    meaning: "rude; exhibiting disrespect",
    partOfSpeech: "Adjective",
    examples: [
      "His audacious remarks offended many attendees.",
      "The student's audacious behavior disrupted the class.",
    ],
    synonyms: ["impudent", "brazen", "insolent"],
    antonyms: ["respectful", "courteous"],
  },
  {
    word: "avaricious",
    meaning: "greedy; always wanting more",
    partOfSpeech: "Adjective",
    examples: [
      "The avaricious CEO prioritized profits over employee welfare.",
      "His avaricious nature made him an unpopular business partner.",
    ],
    synonyms: ["greedy", "covetous", "rapacious"],
    antonyms: ["generous", "selfless"],
  },
  {
    word: "banal",
    meaning: "unoriginal and dull; lacking creativity",
    partOfSpeech: "Adjective",
    examples: [
      "The speech was full of banal platitudes.",
      "The movie's plot was banal and predictable.",
    ],
    synonyms: ["trite", "clichéd", "hackneyed"],
    antonyms: ["original", "inspired"],
  },
  {
    word: "benign",
    meaning: "harmless; having little effect",
    partOfSpeech: "Adjective",
    examples: [
      "The tumor turned out to be benign.",
      "The new law had a benign impact on small businesses.",
    ],
    synonyms: ["harmless", "mild", "innocuous"],
    antonyms: ["harmful", "malignant"],
  },
  {
    word: "benign",
    meaning: "(of a person's character) kind hearted; nice",
    partOfSpeech: "Adjective",
    examples: [
      "His benign smile put everyone at ease.",
      "The teacher had a benign approach to discipline.",
    ],
    synonyms: ["kind", "gentle", "benevolent"],
    antonyms: ["cruel", "hostile"],
  },
  {
    word: "brazen",
    meaning: "shameless and excessively forward",
    partOfSpeech: "Adjective",
    examples: [
      "She made a brazen attempt to cut in line.",
      "The brazen theft occurred in broad daylight.",
    ],
    synonyms: ["shameless", "bold", "impudent"],
    antonyms: ["modest", "meek"],
  },
  {
    word: "calumny",
    meaning: "slander and defamation",
    partOfSpeech: "Noun",
    examples: [
      "The article was full of calumny against the politician.",
      "He sued the newspaper for spreading calumny about his business.",
    ],
    synonyms: ["slander", "libel", "defamation"],
    antonyms: ["praise", "commendation"],
  },
  {
    word: "candid",
    meaning: "honest; without deception",
    partOfSpeech: "Adjective",
    examples: [
      "I appreciate your candid feedback about my performance.",
      "The CEO gave a candid interview about the company's challenges.",
    ],
    synonyms: ["honest", "frank", "forthright"],
    antonyms: ["deceitful", "dishonest"],
  },
  {
    word: "castigate",
    meaning: "scold someone harshly",
    partOfSpeech: "Verb",
    examples: [
      "The coach castigated the team for their poor performance.",
      "The editorial castigated city officials for their inaction.",
    ],
    synonyms: ["rebuke", "reprimand", "berate"],
    antonyms: ["praise", "commend"],
  },
  {
    word: "caustic",
    meaning: "(of speech) hostile and bitter",
    partOfSpeech: "Adjective",
    examples: [
      "Her caustic remarks created tension in the meeting.",
      "The critic's caustic review devastated the young author.",
    ],
    synonyms: ["biting", "sarcastic", "acerbic"],
    antonyms: ["gentle", "kind"],
  },
  {
    word: "caustic",
    meaning: "(of a chemical) corrosive; acidic",
    partOfSpeech: "Adjective",
    examples: [
      "Wear gloves when handling caustic cleaning solutions.",
      "The spill involved a caustic substance that burned through metal.",
    ],
    synonyms: ["corrosive", "burning", "acidic"],
    antonyms: ["neutral", "mild"],
  },
  {
    word: "construe",
    meaning: "understand something subjectively",
    partOfSpeech: "Verb",
    examples: [
      "Her silence could be construed as agreement.",
      "The law is often construed differently by various judges.",
    ],
    synonyms: ["interpret", "understand", "analyze"],
    antonyms: ["misunderstand", "misinterpret"],
  },
  {
    word: "contrite",
    meaning: "feeling regretful or guilty",
    partOfSpeech: "Adjective",
    examples: [
      "He seemed genuinely contrite about his mistake.",
      "Her contrite apology helped mend their friendship.",
    ],
    synonyms: ["remorseful", "repentant", "apologetic"],
    antonyms: ["unrepentant", "shameless"],
  },
  {
    word: "convoluted",
    meaning: "difficult to understand or follow",
    partOfSpeech: "Adjective",
    examples: [
      "The instructions were so convoluted that no one could follow them.",
      "The movie had a convoluted plot with too many twists.",
    ],
    synonyms: ["complex", "complicated", "intricate"],
    antonyms: ["simple", "clear"],
  },
  {
    word: "covet",
    meaning: "strongly desire; yearn for",
    partOfSpeech: "Verb",
    examples: [
      "She coveted her neighbor's luxurious lifestyle.",
      "Many collectors covet rare first edition books.",
    ],
    synonyms: ["desire", "crave", "yearn"],
    antonyms: ["disdain", "reject"],
  },
  {
    word: "craven",
    meaning: "lacking bravery; weak-kneed",
    partOfSpeech: "Adjective",
    examples: [
      "His craven refusal to stand up for his principles disappointed many.",
      "The craven soldier deserted his post during the attack.",
    ],
    synonyms: ["cowardly", "timid", "fearful"],
    antonyms: ["brave", "courageous"],
  },
  {
    word: "decorum",
    meaning: "politeness and proper behavior",
    partOfSpeech: "Noun",
    examples: [
      "The judge demanded decorum in the courtroom.",
      "She maintained her decorum despite the rude comments.",
    ],
    synonyms: ["propriety", "etiquette", "courtesy"],
    antonyms: ["impropriety", "rudeness"],
  },
  {
    word: "deft",
    meaning: "highly proficient; skilled",
    partOfSpeech: "Adjective",
    examples: [
      "The chef's deft knife skills were impressive.",
      "Her deft handling of the crisis prevented panic.",
    ],
    synonyms: ["skillful", "adept", "nimble"],
    antonyms: ["clumsy", "inept"],
  },
  {
    word: "demur",
    meaning: "hesitate or object; express doubt",
    partOfSpeech: "Verb",
    examples: [
      "She demurred when asked to take on extra responsibilities.",
      "He demurred at the suggestion that he was the best candidate.",
    ],
    synonyms: ["object", "protest", "hesitate"],
    antonyms: ["accept", "agree"],
  },
  {
    word: "demur",
    meaning: "hesitation",
    partOfSpeech: "Noun",
    examples: [
      "After some demur, she finally accepted the award.",
      "There was no demur from the board about the proposed changes.",
    ],
    synonyms: ["objection", "reluctance", "reservation"],
    antonyms: ["approval", "consent"],
  },
  {
    word: "derivative",
    meaning: "based on someone else's work (often used in a negative sense)",
    partOfSpeech: "Adjective",
    examples: [
      "The film felt derivative of earlier, better movies.",
      "Critics dismissed her work as derivative and unoriginal.",
    ],
    synonyms: ["unoriginal", "copied", "imitative"],
    antonyms: ["innovative", "original"],
  },
  {
    word: "desiccate",
    meaning: "dry out; remove all moisture",
    partOfSpeech: "Verb",
    examples: [
      "The desert sun quickly desiccated the plants.",
      "Scientists desiccated the samples for preservation.",
    ],
    synonyms: ["dry", "dehydrate", "parch"],
    antonyms: ["moisten", "rehydrate"],
  },
  {
    word: "diatribe",
    meaning: "an angry rant; verbal abuse",
    partOfSpeech: "Noun",
    examples: [
      "He launched into a diatribe against modern art.",
      "The meeting turned into a diatribe against management.",
    ],
    synonyms: ["tirade", "rant", "harangue"],
    antonyms: ["praise", "commendation"],
  },
  {
    word: "incredulous",
    meaning: "doubtful and disbelieving",
    partOfSpeech: "Adjective",
    examples: [
      "She gave me an incredulous look when I told her the news.",
      "His claims were met with incredulous laughter.",
    ],
    synonyms: ["skeptical", "doubtful", "disbelieving"],
    antonyms: ["believing", "convinced"],
  },
  {
    word: "ingenuous",
    meaning: "naive and lacking experience",
    partOfSpeech: "Adjective",
    examples: [
      "Her ingenuous questions revealed her lack of worldly experience.",
      "The ingenuous young tourist fell for every scam.",
    ],
    synonyms: ["naive", "innocent", "unsuspecting"],
    antonyms: ["worldly", "sophisticated"],
  },
];

export const GROUP_3 = [
  {
    word: "abate",
    meaning: "decrease in intensity; lessen",
    partOfSpeech: "Verb",
    examples: [
      "The storm began to abate by evening.",
      "Medication helped abate her symptoms.",
    ],
    antonyms: ["intensify", "increase"],
    synonyms: ["subside", "diminish"],
  },
  {
    word: "abjure",
    meaning: "strongly reject (an idea); forgo",
    partOfSpeech: "Verb",
    examples: [
      "He abjured his former political beliefs.",
      "The monk abjured all worldly possessions.",
    ],
    antonyms: ["embrace", "accept"],
    synonyms: ["renounce", "repudiate"],
  },
  {
    word: "anomalous",
    meaning: "differing from what is normal; atypical",
    partOfSpeech: "Adjective",
    examples: [
      "The anomalous test results required further investigation.",
      "His cheerful mood was anomalous given the circumstances.",
    ],
    antonyms: ["normal", "typical"],
    synonyms: ["abnormal", "irregular"],
  },
  {
    word: "antipathy",
    meaning: "profound hostility or hatred",
    partOfSpeech: "Noun",
    examples: [
      "She felt deep antipathy toward her rival.",
      "There was mutual antipathy between the two departments.",
    ],
    antonyms: ["affinity", "fondness"],
    synonyms: ["animosity", "aversion"],
  },
  {
    word: "arcane",
    meaning: "comprehended by very few people; secret",
    partOfSpeech: "Adjective",
    examples: [
      "The professor specialized in arcane medieval texts.",
      "The company's decision-making process was arcane to outsiders.",
    ],
    antonyms: ["common", "mainstream"],
    synonyms: ["esoteric", "obscure"],
  },
  {
    word: "arduous",
    meaning: "very challenging; needing great effort",
    partOfSpeech: "Adjective",
    examples: [
      "The hike up the mountain was arduous.",
      "She completed the arduous training program.",
    ],
    antonyms: ["easy", "effortless"],
    synonyms: ["strenuous", "grueling"],
  },
  {
    word: "artless",
    meaning: "naive; inexperienced",
    partOfSpeech: "Adjective",
    examples: [
      "Her artless questions revealed her innocence.",
      "The artless young tourist fell for the scam.",
    ],
    antonyms: ["sophisticated", "worldly"],
    synonyms: ["guileless", "ingenuous"],
  },
  {
    word: "artless",
    meaning: "plain and simple",
    partOfSpeech: "Adjective",
    examples: [
      "The cabin had an artless charm.",
      "He preferred artless designs without ornamentation.",
    ],
    antonyms: ["ornate", "elaborate"],
    synonyms: ["unadorned", "unpretentious"],
  },
  {
    word: "artless",
    meaning: "lacking skill",
    partOfSpeech: "Adjective",
    examples: [
      "The artless painting showed little technique.",
      "His artless attempt at repair made things worse.",
    ],
    antonyms: ["skillful", "accomplished"],
    synonyms: ["amateurish", "clumsy"],
  },
  {
    word: "ascetic",
    meaning: "(of a lifestyle) very strict and lacking luxury",
    partOfSpeech: "Adjective",
    examples: [
      "The monk led an ascetic life of prayer and fasting.",
      "Her ascetic apartment contained only basic necessities.",
    ],
    antonyms: ["indulgent", "luxurious"],
    synonyms: ["austere", "spartan"],
  },
  {
    word: "ascetic",
    meaning: "someone who adopts an ascetic lifestyle",
    partOfSpeech: "Noun",
    examples: [
      "The ascetic lived in a cave for twenty years.",
      "Ancient ascetics often denied themselves physical comforts.",
    ],
    antonyms: ["hedonist", "sybarite"],
    synonyms: ["monk", "austerity"],
  },
  {
    word: "assuage",
    meaning: "decrease a negative feeling",
    partOfSpeech: "Verb",
    examples: [
      "The apology helped assuage her anger.",
      "Nothing could assuage his grief over the loss.",
    ],
    antonyms: ["aggravate", "intensify"],
    synonyms: ["alleviate", "mitigate"],
  },
  {
    word: "assuage",
    meaning: "fulfill a certain desire",
    partOfSpeech: "Verb",
    examples: [
      "The snack assuaged his hunger temporarily.",
      "She read constantly to assuage her thirst for knowledge.",
    ],
    antonyms: ["deprive", "starve"],
    synonyms: ["satisfy", "quench"],
  },
  {
    word: "betray",
    meaning: "unknowingly disclose or reveal",
    partOfSpeech: "Verb",
    examples: [
      "Her trembling hands betrayed her nervousness.",
      "The document betrayed secret government plans.",
    ],
    antonyms: ["conceal", "hide"],
    synonyms: ["reveal", "expose"],
  },
  {
    word: "bucolic",
    meaning: "associated with tranquil rural life",
    partOfSpeech: "Adjective",
    examples: [
      "The painting depicted a bucolic countryside scene.",
      "They longed for a bucolic retreat from city life.",
    ],
    antonyms: ["urban", "metropolitan"],
    synonyms: ["pastoral", "rustic"],
  },
  {
    word: "burgeon",
    meaning: "quickly increase or grow; skyrocket",
    partOfSpeech: "Verb",
    examples: [
      "The tech industry continues to burgeon.",
      "Her interest in art burgeoned after the museum visit.",
    ],
    antonyms: ["decline", "shrink"],
    synonyms: ["flourish", "prosper"],
  },
  {
    word: "cacophonous",
    meaning: "extremely noisy; grating",
    partOfSpeech: "Adjective",
    examples: [
      "The cacophonous construction work made concentration impossible.",
      "The band's early music was deliberately cacophonous.",
    ],
    antonyms: ["harmonious", "melodious"],
    synonyms: ["discordant", "raucous"],
  },
  {
    word: "canonize",
    meaning: "treat like a god; deeply venerate",
    partOfSpeech: "Verb",
    examples: [
      "The church decided to canonize the martyred priest.",
      "Fans tend to canonize their favorite musicians.",
    ],
    antonyms: ["vilify", "denigrate"],
    synonyms: ["deify", "idolize"],
  },
  {
    word: "censure",
    meaning: "strongly criticize or disapprove",
    partOfSpeech: "Verb",
    examples: [
      "The senator was censured for ethical violations.",
      "The editorial censured the mayor's handling of the crisis.",
    ],
    antonyms: ["praise", "commend"],
    synonyms: ["condemn", "reprimand"],
  },
  {
    word: "chicanery",
    meaning: "using deception to attain a certain goal",
    partOfSpeech: "Noun",
    examples: [
      "The election was marred by political chicanery.",
      "He used legal chicanery to avoid paying taxes.",
    ],
    antonyms: ["honesty", "integrity"],
    synonyms: ["trickery", "deceit"],
  },
  {
    word: "coalesce",
    meaning: "bring elements together; unite",
    partOfSpeech: "Verb",
    examples: [
      "The rival factions coalesced to oppose the new law.",
      "Her ideas coalesced into a comprehensive theory.",
    ],
    antonyms: ["disperse", "separate"],
    synonyms: ["merge", "unify"],
  },
  {
    word: "cogent",
    meaning: "persuasive and clear; compelling",
    partOfSpeech: "Adjective",
    examples: [
      "She presented a cogent argument for policy reform.",
      "The lawyer's cogent summary convinced the jury.",
    ],
    antonyms: ["weak", "unconvincing"],
    synonyms: ["logical", "forceful"],
  },
  {
    word: "compelling",
    meaning: "persuasive and clear",
    partOfSpeech: "Adjective",
    examples: [
      "The evidence made a compelling case for conviction.",
      "His compelling testimony changed many minds.",
    ],
    antonyms: ["unpersuasive", "ineffective"],
    synonyms: ["convincing", "cogent"],
  },
  {
    word: "compelling",
    meaning: "attracting interest; fascinating",
    partOfSpeech: "Adjective",
    examples: [
      "The novel had a compelling plot that kept me reading.",
      "She has a compelling personality that draws people in.",
    ],
    antonyms: ["boring", "uninteresting"],
    synonyms: ["captivating", "engrossing"],
  },
  {
    word: "contend",
    meaning: "argue a certain point of view; assert",
    partOfSpeech: "Verb",
    examples: [
      "The author contends that modern art has lost its way.",
      "Scientists contend that climate change is accelerating.",
    ],
    antonyms: ["concede", "agree"],
    synonyms: ["assert", "maintain"],
  },
  {
    word: "contend",
    meaning: "compete",
    partOfSpeech: "Verb",
    examples: [
      "Three teams will contend for the championship.",
      "She contended against more experienced athletes.",
    ],
    antonyms: ["withdraw", "surrender"],
    synonyms: ["vie", "compete"],
  },
  {
    word: "contend",
    meaning: "struggle or attempt to overcome something",
    partOfSpeech: "Verb",
    examples: [
      "The village contended with food shortages after the storm.",
      "He's contending with serious health issues.",
    ],
    antonyms: ["succumb", "yield"],
    synonyms: ["grapple", "cope"],
  },
  {
    word: "copious",
    meaning: "existing in great supply; abundant",
    partOfSpeech: "Adjective",
    examples: [
      "She took copious notes during the lecture.",
      "The region receives copious rainfall in spring.",
    ],
    antonyms: ["scarce", "meager"],
    synonyms: ["plentiful", "ample"],
  },
  {
    word: "cosmopolitan",
    meaning: "(of a city) highly diverse",
    partOfSpeech: "Adjective",
    examples: [
      "New York is a truly cosmopolitan metropolis.",
      "The university attracts a cosmopolitan student body.",
    ],
    antonyms: ["provincial", "homogeneous"],
    synonyms: ["multicultural", "diverse"],
  },
  {
    word: "cosmopolitan",
    meaning: "(of a person) sophisticated and urbane",
    partOfSpeech: "Adjective",
    examples: [
      "His cosmopolitan manners impressed the diplomats.",
      "After years of travel, she had become quite cosmopolitan.",
    ],
    antonyms: ["unsophisticated", "provincial"],
    synonyms: ["worldly", "cultured"],
  },
  {
    word: "deference",
    meaning: "high regard and respect; esteem",
    partOfSpeech: "Noun",
    examples: [
      "The young lawyer treated the judge with deference.",
      "In deference to his age, they let him speak first.",
    ],
    antonyms: ["disrespect", "defiance"],
    synonyms: ["respect", "reverence"],
  },
  {
    word: "desultory",
    meaning: "unplanned; without enthusiasm",
    partOfSpeech: "Adjective",
    examples: [
      "They made desultory attempts to clean the garage.",
      "The meeting proceeded in a desultory fashion.",
    ],
    antonyms: ["enthusiastic", "focused"],
    synonyms: ["halfhearted", "aimless"],
  },
  {
    word: "diffident",
    meaning: "reserved or shy; without confidence",
    partOfSpeech: "Adjective",
    examples: [
      "His diffident manner hid a brilliant mind.",
      "The diffident student rarely spoke in class.",
    ],
    antonyms: ["confident", "assertive"],
    synonyms: ["timid", "bashful"],
  },
  {
    word: "dilatory",
    meaning: "slow and unhurried; meant to stall",
    partOfSpeech: "Adjective",
    examples: [
      "The company used dilatory tactics to avoid payment.",
      "His dilatory response frustrated those waiting for answers.",
    ],
    antonyms: ["prompt", "expeditious"],
    synonyms: ["delaying", "procrastinating"],
  },
  {
    word: "equivocate",
    meaning: "speak evasively; avoid taking a stance",
    partOfSpeech: "Verb",
    examples: [
      "Politicians often equivocate when asked difficult questions.",
      "Don't equivocate - give me a straight answer.",
    ],
    antonyms: ["declare", "affirm"],
    synonyms: ["prevaricate", "hedge"],
  },
  {
    word: "polarize",
    meaning: "separate into two opposing groups",
    partOfSpeech: "Verb",
    examples: [
      "The controversial issue polarized the community.",
      "Social media tends to polarize political opinions.",
    ],
    antonyms: ["unite", "reconcile"],
    synonyms: ["divide", "split"],
  },
  {
    word: "prodigal",
    meaning: "carelessly wasteful; extravagant",
    partOfSpeech: "Adjective",
    examples: [
      "His prodigal spending left him in debt.",
      "The prodigal use of resources couldn't continue.",
    ],
    antonyms: ["frugal", "thrifty"],
    synonyms: ["extravagant", "profligate"],
  },
  {
    word: "prodigal",
    meaning: "a person who is excessively wasteful",
    partOfSpeech: "Noun",
    examples: [
      "The prodigal returned home after wasting his inheritance.",
      "She reformed from being a prodigal to quite frugal.",
    ],
    antonyms: ["miser", "saver"],
    synonyms: ["spendthrift", "wastrel"],
  },
  {
    word: "verbose",
    meaning: "excessively talkative; wordy",
    partOfSpeech: "Adjective",
    examples: [
      "The professor's verbose explanation confused students more.",
      "Legal documents are often unnecessarily verbose.",
    ],
    antonyms: ["concise", "succinct"],
    synonyms: ["loquacious", "prolix"],
  },
];

export const GROUP_4 = [
  {
    word: "abstain",
    meaning: "avoid taking part in something",
    partOfSpeech: "Verb",
    examples: [
      "He decided to abstain from voting in the election.",
      "Doctors advise patients to abstain from alcohol before surgery.",
    ],
    synonyms: ["refrain", "withhold", "desist"],
    antonyms: ["indulge", "partake", "engage"],
  },
  {
    word: "approbation",
    meaning: "appreciation toward someone",
    partOfSpeech: "Noun",
    examples: [
      "The film received widespread approbation from critics.",
      "She smiled with satisfaction at the approbation of her peers.",
    ],
    synonyms: ["approval", "praise", "endorsement"],
    antonyms: ["disapproval", "criticism", "condemnation"],
  },
  {
    word: "cherish",
    meaning: "greatly love or protect something",
    partOfSpeech: "Verb",
    examples: [
      "She will always cherish the memories of her childhood.",
      "They cherish their family traditions deeply.",
    ],
    synonyms: ["treasure", "value", "hold dear"],
    antonyms: ["neglect", "disregard", "despise"],
  },
  {
    word: "corroborate",
    meaning: "lend support to; bolster",
    partOfSpeech: "Verb",
    examples: [
      "The witness could not corroborate the suspect’s alibi.",
      "Studies corroborate the link between stress and heart disease.",
    ],
    synonyms: ["confirm", "verify", "support"],
    antonyms: ["deny", "refute", "contradict"],
  },
  {
    word: "disparate",
    meaning: "different or contrasting",
    partOfSpeech: "Adjective",
    examples: [
      "They came from disparate backgrounds but became close friends.",
      "The novel combines disparate elements of fantasy and science fiction.",
    ],
    synonyms: ["dissimilar", "distinct", "divergent"],
    antonyms: ["similar", "alike", "homogeneous"],
  },
  {
    word: "emulate",
    meaning: "imitate someone or something",
    partOfSpeech: "Verb",
    examples: [
      "She tried to emulate her mentor's dedication to her work.",
      "Young athletes often emulate their sports heroes.",
    ],
    synonyms: ["imitate", "mirror", "copy"],
    antonyms: ["neglect", "ignore", "disregard"],
  },
  {
    word: "enervate",
    meaning: "sap the energy out of someone; fatigue",
    partOfSpeech: "Verb",
    examples: [
      "The long meeting enervated everyone in the room.",
      "A hot climate can enervate even the most energetic people.",
    ],
    synonyms: ["weaken", "exhaust", "debilitate"],
    antonyms: ["invigorate", "energize", "strengthen"],
  },
  {
    word: "ephemeral",
    meaning: "extremely short lasting",
    partOfSpeech: "Adjective",
    examples: [
      "Fame in the digital age is often ephemeral.",
      "The beauty of the sunset was ephemeral but breathtaking.",
    ],
    synonyms: ["fleeting", "transient", "momentary"],
    antonyms: ["enduring", "lasting", "permanent"],
  },
  {
    word: "fervid",
    meaning: "excessively energetic or eager",
    partOfSpeech: "Adjective",
    examples: [
      "The speaker gave a fervid plea for environmental action.",
      "Her fervid imagination brought the story to life.",
    ],
    synonyms: ["passionate", "zealous", "enthusiastic"],
    antonyms: ["apathetic", "indifferent", "unemotional"],
  },
  {
    word: "garrulous",
    meaning: "tending to talk too much; wordy",
    partOfSpeech: "Adjective",
    examples: [
      "The garrulous host kept the conversation going all night.",
      "He grew annoyed with the garrulous old man sitting next to him.",
    ],
    synonyms: ["talkative", "loquacious", "chatty"],
    antonyms: ["taciturn", "reserved", "quiet"],
  },
  {
    word: "incendiary",
    meaning: "provocative; causing conflict",
    partOfSpeech: "Adjective",
    examples: [
      "His incendiary remarks sparked a heated debate.",
      "The article was full of incendiary language meant to provoke outrage.",
    ],
    synonyms: ["provocative", "inflammatory", "agitating"],
    antonyms: ["calming", "soothing", "conciliatory"],
  },
  {
    word: "inimical",
    meaning: "damaging; hostile in manner or tone",
    partOfSpeech: "Adjective",
    examples: [
      "Their relationship became increasingly inimical.",
      "Such policies are inimical to democratic values.",
    ],
    synonyms: ["hostile", "harmful", "adverse"],
    antonyms: ["friendly", "beneficial", "helpful"],
  },
  {
    word: "intimate",
    meaning: "subtly suggest or imply",
    partOfSpeech: "Verb",
    examples: [
      "He intimated that he might retire soon.",
      "Her tone intimated dissatisfaction with the decision.",
    ],
    synonyms: ["suggest", "imply", "hint"],
    antonyms: ["declare", "announce", "proclaim"],
  },
  {
    word: "invigorate",
    meaning: "provide energy or vitality to",
    partOfSpeech: "Verb",
    examples: [
      "A brisk walk in the morning can invigorate you for the day.",
      "The new strategy invigorated the company’s sales team.",
    ],
    synonyms: ["energize", "revitalize", "stimulate"],
    antonyms: ["weaken", "enervate", "exhaust"],
  },
  {
    word: "mitigate",
    meaning: "diminish negative feelings or situation",
    partOfSpeech: "Verb",
    examples: [
      "We need to mitigate the effects of climate change.",
      "The doctor gave her medication to mitigate the pain.",
    ],
    synonyms: ["alleviate", "ease", "lessen"],
    antonyms: ["aggravate", "intensify", "exacerbate"],
  },
  {
    word: "obsolete",
    meaning: "no longer useful; outmoded",
    partOfSpeech: "Adjective",
    examples: [
      "Typewriters have become obsolete in modern offices.",
      "The law is considered obsolete and rarely enforced.",
    ],
    synonyms: ["outdated", "archaic", "antiquated"],
    antonyms: ["modern", "current", "up-to-date"],
  },
  {
    word: "opaque",
    meaning: "unclear; hard to comprehend",
    partOfSpeech: "Adjective",
    examples: [
      "His explanation was too opaque to understand.",
      "The technical jargon made the document opaque to readers.",
    ],
    synonyms: ["obscure", "murky", "confusing"],
    antonyms: ["clear", "transparent", "lucid"],
  },
  {
    word: "opaque",
    meaning: "not able to be seen through",
    partOfSpeech: "Adjective",
    examples: [
      "The windows were opaque with steam.",
      "Frost made the glass completely opaque.",
    ],
    synonyms: ["cloudy", "nontransparent", "frosted"],
    antonyms: ["clear", "transparent", "see-through"],
  },
  {
    word: "paradigmatic",
    meaning: "representing the standard example",
    partOfSpeech: "Adjective",
    examples: [
      "Her career is a paradigmatic example of hard work leading to success.",
      "The novel became a paradigmatic text of modern literature.",
    ],
    synonyms: ["typical", "model", "standard"],
    antonyms: ["atypical", "unusual", "unrepresentative"],
  },
  {
    word: "pedantic",
    meaning: "overly concerned with (minor) details",
    partOfSpeech: "Adjective",
    examples: [
      "His pedantic comments slowed the meeting down.",
      "She has a pedantic habit of correcting everyone’s grammar.",
    ],
    synonyms: ["nitpicky", "fussy", "precise"],
    antonyms: ["careless", "imprecise", "informal"],
  },
  {
    word: "placid",
    meaning: "calm; free from activity or irritation",
    partOfSpeech: "Adjective",
    examples: [
      "The placid lake reflected the sky perfectly.",
      "He remained placid even during the crisis.",
    ],
    synonyms: ["peaceful", "serene", "tranquil"],
    antonyms: ["agitated", "disturbed", "turbulent"],
  },
  {
    word: "polemical",
    meaning: "giving a strong, controversial opinion",
    partOfSpeech: "Adjective",
    examples: [
      "The article was full of polemical criticism of the government.",
      "His polemical stance alienated some of his supporters.",
    ],
    synonyms: ["critical", "contentious", "argumentative"],
    antonyms: ["conciliatory", "diplomatic", "neutral"],
  },
  {
    word: "precipitate",
    meaning: "cause to happen",
    partOfSpeech: "Verb",
    examples: [
      "The incident precipitated a political crisis.",
      "The resignation precipitated the fall of the government.",
    ],
    synonyms: ["trigger", "cause", "spark"],
    antonyms: ["prevent", "halt", "delay"],
  },
  {
    word: "precipitate",
    meaning: "done quickly or suddenly",
    partOfSpeech: "Adjective",
    examples: [
      "His decision was precipitate and poorly thought out.",
      "They made a precipitate withdrawal from the negotiations.",
    ],
    synonyms: ["hasty", "rash", "impulsive"],
    antonyms: ["deliberate", "measured", "cautious"],
  },
  {
    word: "profundity",
    meaning: "great wisdom or intelligence",
    partOfSpeech: "Noun",
    examples: [
      "The profundity of his insights impressed the audience.",
      "Her speech was full of emotional depth and profundity.",
    ],
    synonyms: ["depth", "wisdom", "insight"],
    antonyms: ["shallowness", "superficiality", "ignorance"],
  },
  {
    word: "prophetic",
    meaning: "able to correctly predict the future",
    partOfSpeech: "Adjective",
    examples: [
      "His warnings turned out to be prophetic.",
      "The book’s predictions were eerily prophetic.",
    ],
    synonyms: ["predictive", "visionary", "foretelling"],
    antonyms: ["unpredictive", "mundane", "uninspired"],
  },
  {
    word: "prudent",
    meaning: "wise in decision-making",
    partOfSpeech: "Adjective",
    examples: [
      "It's prudent to save money for emergencies.",
      "She made a prudent investment in real estate.",
    ],
    synonyms: ["wise", "sensible", "cautious"],
    antonyms: ["reckless", "foolish", "imprudent"],
  },
  {
    word: "punctilious",
    meaning: "highly concerned with details or manners",
    partOfSpeech: "Adjective",
    examples: [
      "He was punctilious in observing the rules.",
      "Her punctilious nature made her a great proofreader.",
    ],
    synonyms: ["meticulous", "exact", "scrupulous"],
    antonyms: ["careless", "sloppy", "negligent"],
  },
  {
    word: "recondite",
    meaning: "obscure and difficult to understand",
    partOfSpeech: "Adjective",
    examples: [
      "The professor’s lecture was full of recondite concepts.",
      "His writing is often too recondite for the average reader.",
    ],
    synonyms: ["obscure", "esoteric", "abstruse"],
    antonyms: ["simple", "clear", "accessible"],
  },
  {
    word: "scrupulous",
    meaning: "careful with details",
    partOfSpeech: "Adjective",
    examples: [
      "She kept scrupulous records of every transaction.",
      "The investigation was scrupulous and thorough.",
    ],
    synonyms: ["meticulous", "precise", "thorough"],
    antonyms: ["careless", "negligent", "inaccurate"],
  },
  {
    word: "scrupulous",
    meaning: "possessing moral integrity",
    partOfSpeech: "Adjective",
    examples: [
      "He is known for being scrupulous in all his dealings.",
      "Her scrupulous honesty won everyone's respect.",
    ],
    synonyms: ["honest", "ethical", "principled"],
    antonyms: ["dishonest", "unethical", "corrupt"],
  },
  {
    word: "tranquil",
    meaning: "peaceful; free from activity",
    partOfSpeech: "Adjective",
    examples: [
      "They enjoyed a tranquil afternoon by the lake.",
      "The village is a tranquil place far from the city.",
    ],
    synonyms: ["calm", "serene", "peaceful"],
    antonyms: ["noisy", "chaotic", "agitated"],
  },
  {
    word: "vacillate",
    meaning: "go back and forth with one's view",
    partOfSpeech: "Verb",
    examples: [
      "She vacillated between studying medicine and law.",
      "The government continues to vacillate on the issue.",
    ],
    synonyms: ["waver", "hesitate", "dither"],
    antonyms: ["decide", "choose", "settle"],
  },
];

export const GROUP_5 = [
  {
    word: "aloof",
    meaning: "not warm or friendly; impersonal",
    partOfSpeech: "Adjective",
    examples: [
      "She remained aloof during the group discussion.",
      "His aloof attitude made it hard to approach him.",
    ],
    synonyms: ["distant", "detached", "reserved"],
    antonyms: ["friendly", "sociable"],
  },
  {
    word: "clangor",
    meaning: "a disturbingly loud noise",
    partOfSpeech: "Noun",
    examples: [
      "The clangor of pots and pans echoed through the kitchen.",
      "The clangor from the construction site was unbearable.",
    ],
    synonyms: ["racket", "din", "uproar"],
    antonyms: ["silence", "quiet"],
  },
  {
    word: "conventional",
    meaning: "adhering to established practices or accepted standards",
    partOfSpeech: "Adjective",
    examples: [
      "She wore a conventional dress to the formal event.",
      "He prefers conventional methods over experimental ones.",
    ],
    synonyms: ["traditional", "customary", "standard"],
    antonyms: ["unorthodox", "unconventional"],
  },
  {
    word: "debunk",
    meaning: "prove a belief or idea false",
    partOfSpeech: "Verb",
    examples: [
      "The scientist debunked the myth with evidence.",
      "The article aims to debunk common misconceptions.",
    ],
    synonyms: ["disprove", "expose", "invalidate"],
    antonyms: ["validate", "confirm"],
  },
  {
    word: "diminutive",
    meaning: "exceptionally small in size or stature",
    partOfSpeech: "Adjective",
    examples: [
      "She was a diminutive figure among the tall players.",
      "The diminutive car fit easily into the small space.",
    ],
    synonyms: ["tiny", "petite", "miniature"],
    antonyms: ["huge", "gigantic"],
  },
  {
    word: "discernible",
    meaning: "able to be recognized or understood",
    partOfSpeech: "Adjective",
    examples: [
      "There was a discernible difference in tone.",
      "His accent was barely discernible after years abroad.",
    ],
    synonyms: ["perceptible", "noticeable", "detectable"],
    antonyms: ["indistinct", "imperceptible"],
  },
  {
    word: "enigmatic",
    meaning: "hard to understand; mysterious",
    partOfSpeech: "Adjective",
    examples: [
      "She gave him an enigmatic smile.",
      "The painting has an enigmatic quality.",
    ],
    synonyms: ["mysterious", "puzzling", "cryptic"],
    antonyms: ["clear", "obvious"],
  },
  {
    word: "estranged",
    meaning: "emotionally distanced from someone",
    partOfSpeech: "Adjective",
    examples: [
      "He felt estranged from his old friends.",
      "The argument left them emotionally estranged.",
    ],
    synonyms: ["alienated", "separated", "detached"],
    antonyms: ["connected", "reconciled"],
  },
  {
    word: "extravagant",
    meaning: "excessive, especially in terms of cost",
    partOfSpeech: "Adjective",
    examples: [
      "They threw an extravagant wedding party.",
      "His taste in clothes is rather extravagant.",
    ],
    synonyms: ["lavish", "excessive", "over-the-top"],
    antonyms: ["modest", "frugal"],
  },
  {
    word: "fanciful",
    meaning: "highly imaginative; unrealistic",
    partOfSpeech: "Adjective",
    examples: [
      "The story was too fanciful to be believable.",
      "She had a fanciful idea about traveling through time.",
    ],
    synonyms: ["whimsical", "imaginative", "fantastical"],
    antonyms: ["realistic", "practical"],
  },
  {
    word: "frivolous",
    meaning: "lacking in seriousness or importance; silly",
    partOfSpeech: "Adjective",
    examples: [
      "They wasted money on frivolous purchases.",
      "His frivolous attitude annoyed the serious team.",
      "She's too frivolous to manage such responsibility.",
      "He was seen as frivolous and unreliable.",
    ],
    synonyms: ["silly", "trivial", "petty"],
    antonyms: ["serious", "important"],
  },
  {
    word: "heterogeneous",
    meaning: "composed of varying, diverse elements",
    partOfSpeech: "Adjective",
    examples: [
      "The crowd was a heterogeneous mix of people.",
      "The materials were too heterogeneous to blend well.",
    ],
    synonyms: ["diverse", "varied", "mixed"],
    antonyms: ["homogeneous", "uniform"],
  },
  {
    word: "imperious",
    meaning: "having an arrogant or domineering manner",
    partOfSpeech: "Adjective",
    examples: [
      "His imperious tone silenced the room.",
      "She gave imperious orders that no one dared question.",
    ],
    synonyms: ["overbearing", "authoritative", "commanding"],
    antonyms: ["humble", "meek"],
  },
  {
    word: "impertinent",
    meaning: "disrespectful or inappropriate",
    partOfSpeech: "Adjective",
    examples: [
      "The student was punished for his impertinent remark.",
      "She considered his question impertinent.",
      "The comment was impertinent to the discussion.",
      "He kept making impertinent observations.",
    ],
    synonyms: ["rude", "insolent", "irrelevant"],
    antonyms: ["respectful", "relevant"],
  },
  {
    word: "invasive",
    meaning:
      "infringing on others' privacy / spreading rapidly and causing harm",
    partOfSpeech: "Adjective",
    examples: [
      "She found the reporter’s questions invasive.",
      "Invasive surveillance raised privacy concerns.",
      "The invasive species disrupted the ecosystem.",
      "Invasive plants choked the native vegetation.",
    ],
    synonyms: ["intrusive", "aggressive", "encroaching"],
    antonyms: ["noninvasive", "contained"],
  },
  {
    word: "irresolute",
    meaning: "uncertain or lacking determination",
    partOfSpeech: "Adjective",
    examples: [
      "He stood irresolute at the crossroads.",
      "Her irresolute response disappointed the team.",
    ],
    synonyms: ["indecisive", "vacillating", "hesitant"],
    antonyms: ["resolute", "determined"],
  },
  {
    word: "laudable",
    meaning: "worthy of admiration or praise",
    partOfSpeech: "Adjective",
    examples: [
      "Her efforts to help the homeless are laudable.",
      "He made a laudable attempt to resolve the issue.",
    ],
    synonyms: ["praiseworthy", "commendable", "admirable"],
    antonyms: ["blameworthy", "shameful"],
  },
  {
    word: "lax",
    meaning: "not diligent or careful; casual",
    partOfSpeech: "Adjective",
    examples: [
      "The manager was lax about enforcing deadlines.",
      "Lax security led to the breach.",
    ],
    synonyms: ["careless", "slack", "negligent"],
    antonyms: ["strict", "rigorous"],
  },
  {
    word: "marginalize",
    meaning: "treat someone or something as insignificant",
    partOfSpeech: "Verb",
    examples: [
      "The policies marginalized minority groups.",
      "Artists often feel marginalized by mainstream media.",
    ],
    synonyms: ["exclude", "alienate", "oppress"],
    antonyms: ["include", "empower"],
  },
  {
    word: "panache",
    meaning: "a stylish and confident manner",
    partOfSpeech: "Noun",
    examples: [
      "She delivered the speech with great panache.",
      "He entered the room with flair and panache.",
    ],
    synonyms: ["flair", "style", "elegance"],
    antonyms: ["awkwardness", "clumsiness"],
  },
  {
    word: "plodding",
    meaning: "moving slowly or laboriously",
    partOfSpeech: "Adjective",
    examples: [
      "The movie had a plodding pace.",
      "She took a plodding approach to her tasks.",
    ],
    synonyms: ["slow", "tedious", "labored"],
    antonyms: ["brisk", "energetic"],
  },
  {
    word: "prosaic",
    meaning: "straightforward; unoriginal; unimaginative",
    partOfSpeech: "Adjective",
    examples: [
      "His writing style is prosaic and lacks flair.",
      "The design is functional but prosaic.",
    ],
    synonyms: ["mundane", "dull", "uninspired"],
    antonyms: ["imaginative", "creative"],
  },
  {
    word: "remedial",
    meaning: "serving to correct or cure something",
    partOfSpeech: "Adjective",
    examples: [
      "He was placed in a remedial math class.",
      "The school offers remedial programs for struggling students.",
    ],
    synonyms: ["corrective", "therapeutic", "curative"],
    antonyms: ["harmful", "damaging"],
  },
  {
    word: "restive",
    meaning: "demonstrating difficulty in staying still",
    partOfSpeech: "Adjective",
    examples: [
      "The children became restive during the long ceremony.",
      "The crowd grew restive as the speech dragged on.",
    ],
    synonyms: ["unruly", "fidgety", "restless"],
    antonyms: ["calm", "composed"],
  },
  {
    word: "sporadic",
    meaning: "happening occasionally and irregularly",
    partOfSpeech: "Adjective",
    examples: [
      "There were sporadic power outages throughout the week.",
      "Sporadic rainfall delayed the harvest.",
    ],
    synonyms: ["intermittent", "occasional", "scattered"],
    antonyms: ["frequent", "continuous"],
  },
  {
    word: "stigmatize",
    meaning: "label or mark as disgraceful",
    partOfSpeech: "Verb",
    examples: [
      "Society tends to stigmatize mental illness.",
      "He was unfairly stigmatized by his past mistakes.",
    ],
    synonyms: ["brand", "label", "condemn"],
    antonyms: ["glorify", "praise"],
  },
  {
    word: "undermine",
    meaning: "weaken or sabotage, often subtly",
    partOfSpeech: "Verb",
    examples: [
      "His actions undermined the team's confidence.",
      "The scandal undermined her credibility.",
    ],
    synonyms: ["weaken", "subvert", "erode"],
    antonyms: ["strengthen", "support"],
  },
  {
    word: "utterly",
    meaning: "completely and without exception",
    partOfSpeech: "Adverb",
    examples: [
      "She was utterly exhausted after the hike.",
      "The film was utterly disappointing.",
    ],
    synonyms: ["completely", "totally", "entirely"],
    antonyms: ["partially", "somewhat"],
  },
  {
    word: "weary",
    meaning: "feeling tired or exhausted",
    partOfSpeech: "Adjective",
    examples: [
      "She looked weary after the long journey.",
      "He grew weary of repeating himself.",
    ],
    synonyms: ["tired", "exhausted", "fatigued"],
    antonyms: ["energetic", "rested"],
  },
  {
    word: "zealous",
    meaning: "intensely passionate",
    partOfSpeech: "Adjective",
    examples: [
      "The zealous fans cheered loudly for their team.",
      "She was zealous in advocating for animal rights.",
    ],
    synonyms: ["enthusiastic", "fervent", "passionate"],
    antonyms: ["indifferent", "apathetic"],
  },
];

export const GROUP_6 = [
  {
    word: "admonish",
    meaning: "scold someone seriously",
    partOfSpeech: "Verb",
    synonyms: ["reprimand", "rebuke", "reprove"],
    antonyms: ["praise", "commend"],
    examples: [
      "The teacher admonished the students for cheating on the test.",
      "His mother admonished him for coming home so late.",
    ],
  },
  {
    word: "admonish",
    meaning: "give someone urgent advice",
    partOfSpeech: "Verb",
    synonyms: ["urge", "advise", "exhort"],
    antonyms: ["ignore", "neglect"],
    examples: [
      "The doctor admonished her to quit smoking immediately.",
      "I must admonish you to reconsider this risky investment.",
    ],
  },
  {
    word: "aesthetic",
    meaning: "pertaining to beauty or the appreciation thereof",
    partOfSpeech: "Adjective",
    synonyms: ["artistic", "tasteful", "elegant"],
    antonyms: ["ugly", "unaesthetic"],
    examples: [
      "The museum's aesthetic appeal attracted many visitors.",
      "She made aesthetic improvements to the garden design.",
    ],
  },
  {
    word: "affectation",
    meaning: "a pretense or show meant to impress others",
    partOfSpeech: "Noun",
    synonyms: ["pretension", "pretense", "artificiality"],
    antonyms: ["authenticity", "sincerity"],
    examples: [
      "His British accent was clearly an affectation.",
      "She saw through his affectation of sophistication.",
    ],
  },
  {
    word: "alleviate",
    meaning: "lessen the intensity or severity of something negative",
    partOfSpeech: "Verb",
    synonyms: ["ease", "relieve", "mitigate"],
    antonyms: ["exacerbate", "aggravate"],
    examples: [
      "The medication helped alleviate her chronic pain.",
      "Volunteers worked to alleviate suffering after the disaster.",
    ],
  },
  {
    word: "analogous",
    meaning: "comparable or similar to",
    partOfSpeech: "Adjective",
    synonyms: ["comparable", "similar", "parallel"],
    antonyms: ["dissimilar", "unlike"],
    examples: [
      "The structure of an atom is analogous to a solar system.",
      "Their relationship was analogous to that of siblings.",
    ],
  },
  {
    word: "bolster",
    meaning: "lend support or boost",
    partOfSpeech: "Verb",
    synonyms: ["support", "strengthen", "reinforce"],
    antonyms: ["weaken", "undermine"],
    examples: [
      "The new evidence bolstered their case in court.",
      "She drank coffee to bolster her energy before the meeting.",
    ],
  },
  {
    word: "chauvinistic",
    meaning: "excessively patriotic or biased in favor of a group or cause",
    partOfSpeech: "Adjective",
    synonyms: ["jingoistic", "patriotic", "biased"],
    antonyms: ["impartial", "open-minded"],
    examples: [
      "His chauvinistic remarks about other countries offended many.",
      "The company was criticized for its chauvinistic hiring practices.",
    ],
  },
  {
    word: "connoisseur",
    meaning: "a discerning expert in matters of taste",
    partOfSpeech: "Noun",
    synonyms: ["expert", "specialist", "aficionado"],
    antonyms: ["amateur", "novice"],
    examples: [
      "As a wine connoisseur, he could identify rare vintages.",
      "The art connoisseur recognized the forgery immediately.",
    ],
  },
  {
    word: "dissemble",
    meaning: "hide or misrepresent one's true feelings or beliefs",
    partOfSpeech: "Verb",
    synonyms: ["pretend", "feign", "conceal"],
    antonyms: ["reveal", "disclose"],
    examples: [
      "He dissembled his disappointment with a forced smile.",
      "Politicians often dissemble when answering difficult questions.",
    ],
  },
  {
    word: "dogged",
    meaning: "demonstrating tenacity and an unwillingness to give up",
    partOfSpeech: "Adjective",
    synonyms: ["tenacious", "persistent", "determined"],
    antonyms: ["yielding", "lazy"],
    examples: [
      "Her dogged determination led to the discovery.",
      "The detective pursued the case with dogged persistence.",
    ],
  },
  {
    word: "dupe",
    meaning: "trick or fool someone",
    partOfSpeech: "Verb",
    synonyms: ["deceive", "trick", "mislead"],
    antonyms: ["enlighten", "inform"],
    examples: [
      "Scammers duped elderly victims out of their savings.",
      "He felt duped after realizing the product didn't work as advertised.",
    ],
  },
  {
    word: "empirical",
    meaning: "based on observation or experience rather than conjecture",
    partOfSpeech: "Adjective",
    synonyms: ["observational", "practical", "factual"],
    antonyms: ["theoretical", "speculative"],
    examples: [
      "Scientists rely on empirical evidence to support theories.",
      "The study provided empirical data about consumer behavior.",
    ],
  },
  {
    word: "engender",
    meaning: "produce or stimulate the development of something",
    partOfSpeech: "Verb",
    synonyms: ["cause", "generate", "provoke"],
    antonyms: ["prevent", "halt"],
    examples: [
      "The controversial policy engendered much debate.",
      "His kindness engendered trust among his colleagues.",
    ],
  },
  {
    word: "entitled",
    meaning: "assuming oneself to be deserving of special rights or privileges",
    partOfSpeech: "Adjective",
    synonyms: ["privileged", "self-important", "presumptuous"],
    antonyms: ["humble", "modest"],
    examples: [
      "The entitled customer demanded special treatment.",
      "His entitled attitude made him unpopular with coworkers.",
    ],
  },
  {
    word: "pertinacious",
    meaning: "stubbornly holding onto a belief or course of action",
    partOfSpeech: "Adjective",
    synonyms: ["persistent", "obstinate", "resolute"],
    antonyms: ["compliant", "yielding"],
    examples: [
      "Her pertinacious refusal to compromise stalled negotiations.",
      "The pertinacious reporter kept investigating despite obstacles.",
    ],
  },
  {
    word: "presumptuous",
    meaning: "overstepping boundaries, often in an overconfident way",
    partOfSpeech: "Adjective",
    synonyms: ["brazen", "forward", "arrogant"],
    antonyms: ["modest", "deferential"],
    examples: [
      "It was presumptuous of him to speak for the entire group.",
      "Her presumptuous suggestion offended the senior staff.",
    ],
  },
  {
    word: "probity",
    meaning: "the characteristic of having strong ethical standards",
    partOfSpeech: "Noun",
    synonyms: ["integrity", "honesty", "uprightness"],
    antonyms: ["corruption", "dishonesty"],
    examples: [
      "The judge was respected for his unquestionable probity.",
      "Financial advisors must demonstrate absolute probity.",
    ],
  },
  {
    word: "proliferate",
    meaning: "rapidly increase or multiply in number",
    partOfSpeech: "Verb",
    synonyms: ["multiply", "spread", "expand"],
    antonyms: ["diminish", "decrease"],
    examples: [
      "Fast food chains continue to proliferate worldwide.",
      "Online scams proliferated during the pandemic.",
    ],
  },
  {
    word: "specious",
    meaning: "seemingly reasonable but actually wrong",
    partOfSpeech: "Adjective",
    synonyms: ["misleading", "fallacious", "deceptive"],
    antonyms: ["valid", "genuine"],
    examples: [
      "His specious argument fell apart under scrutiny.",
      "The advertisement made specious claims about the product.",
    ],
  },
  {
    word: "spurious",
    meaning: "not genuine; bogus",
    partOfSpeech: "Adjective",
    synonyms: ["false", "phony", "fraudulent"],
    antonyms: ["authentic", "genuine"],
    examples: [
      "The museum discovered the painting was a spurious imitation.",
      "He made spurious accusations without evidence.",
    ],
  },
  {
    word: "subjective",
    meaning: "pertaining to personal feelings rather than objective reality",
    partOfSpeech: "Adjective",
    synonyms: ["personal", "emotional", "biased"],
    antonyms: ["objective", "neutral"],
    examples: [
      "Art appreciation is largely subjective.",
      "Her review contained many subjective opinions.",
    ],
  },
  {
    word: "subvert",
    meaning: "undermine or weaken, typically an authority of some sort",
    partOfSpeech: "Verb",
    synonyms: ["undermine", "destabilize", "sabotage"],
    antonyms: ["support", "uphold"],
    examples: [
      "The rebels sought to subvert the government.",
      "The parody subtly subverted traditional gender roles.",
    ],
  },
  {
    word: "timorous",
    meaning: "displaying nervousness; lacking confidence",
    partOfSpeech: "Adjective",
    synonyms: ["shy", "timid", "hesitant"],
    antonyms: ["bold", "confident"],
    examples: [
      "The timorous intern hesitated to share her ideas.",
      "His timorous approach to negotiations weakened his position.",
    ],
  },
  {
    word: "tortuous",
    meaning: "marked by numerous twists or turns",
    partOfSpeech: "Adjective",
    synonyms: ["winding", "twisting", "meandering"],
    antonyms: ["straight", "direct"],
    examples: [
      "The tortuous mountain road made driving difficult.",
      "They followed the tortuous path through the dense forest.",
    ],
  },
  {
    word: "tortuous",
    meaning: "overly complicated and long",
    partOfSpeech: "Adjective",
    synonyms: ["convoluted", "complex", "lengthy"],
    antonyms: ["simple", "clear"],
    examples: [
      "The contract's tortuous language confused many signers.",
      "The approval process became tortuous with added regulations.",
    ],
  },
  {
    word: "tractable",
    meaning: "easily managed or controlled",
    partOfSpeech: "Adjective",
    synonyms: ["manageable", "docile", "obedient"],
    antonyms: ["unruly", "stubborn"],
    examples: [
      "The tractable horse was perfect for beginner riders.",
      "She found the students remarkably tractable after lunch.",
    ],
  },
  {
    word: "tractable",
    meaning: "(of a situation) easy to solve or handle",
    partOfSpeech: "Adjective",
    synonyms: ["solvable", "resolvable", "workable"],
    antonyms: ["complicated", "intractable"],
    examples: [
      "The math problem proved surprisingly tractable.",
      "What seemed like a crisis became tractable with proper analysis.",
    ],
  },
  {
    word: "transient",
    meaning: "not lasting, enduring, or permanent; transitory",
    partOfSpeech: "Adjective",
    synonyms: ["temporary", "fleeting", "short-lived"],
    antonyms: ["permanent", "enduring"],
    examples: [
      "The drug's effects were powerful but transient.",
      "They enjoyed the transient beauty of the cherry blossoms.",
    ],
  },
  {
    word: "ubiquitous",
    meaning: "being everywhere at once",
    partOfSpeech: "Adjective",
    synonyms: ["omnipresent", "universal", "pervasive"],
    antonyms: ["rare", "scarce"],
    examples: [
      "Smartphones have become ubiquitous in modern society.",
      "The ubiquitous advertising made the product instantly recognizable.",
    ],
  },
  {
    word: "underscore",
    meaning: "emphasize or show the importance of something",
    partOfSpeech: "Verb",
    synonyms: ["emphasize", "highlight", "stress"],
    antonyms: ["downplay", "minimize"],
    examples: [
      "The report underscores the need for immediate action.",
      "Her speech underscored the values of equality and justice.",
    ],
  },
  {
    word: "venal",
    meaning: "open to bribery or corruption",
    partOfSpeech: "Adjective",
    synonyms: ["corrupt", "dishonest", "bribable"],
    antonyms: ["honest", "incorruptible"],
    examples: [
      "The venal official accepted bribes for political favors.",
      "The investigation revealed a venal system of kickbacks.",
    ],
  },
  {
    word: "venerate",
    meaning: "regard with deep respect or reverence",
    partOfSpeech: "Verb",
    synonyms: ["revere", "respect", "honor"],
    antonyms: ["disrespect", "dishonor"],
    examples: [
      "Many cultures venerate their ancestors.",
      "The scientist was venerated for her groundbreaking discoveries.",
    ],
  },
];

export const GROUP_7 = [
  {
    word: "appease",
    meaning: "calm someone down",
    partOfSpeech: "Verb",
    examples: [
      "The manager appeased the angry customer by offering a full refund.",
      "She appeased her crying child with a favorite toy.",
    ],
    synonyms: ["pacify", "placate", "mollify"],
    antonyms: ["provoke", "aggravate", "antagonize"],
  },
  {
    word: "appease",
    meaning: "satisfy a demand or desire",
    partOfSpeech: "Verb",
    examples: [
      "The peace agreement appeased both nations' territorial claims.",
      "A small snack appeased his hunger until dinner.",
    ],
    synonyms: ["satisfy", "gratify", "fulfill"],
    antonyms: ["deny", "frustrate", "deprive"],
  },
  {
    word: "arbitrary",
    meaning: "determined by chance, not reason",
    partOfSpeech: "Adjective",
    examples: [
      "The teacher's grading system seemed completely arbitrary.",
      "The arbitrary selection process frustrated qualified candidates.",
    ],
    synonyms: ["random", "capricious", "whimsical"],
    antonyms: ["rational", "reasoned", "systematic"],
  },
  {
    word: "archaic",
    meaning: "ancient or outmoded; out of date",
    partOfSpeech: "Adjective",
    examples: [
      "'Thou' is an archaic form of 'you' rarely used today.",
      "The law contained several archaic provisions from the 1800s.",
    ],
    synonyms: ["obsolete", "antiquated", "dated"],
    antonyms: ["modern", "current", "contemporary"],
  },
  {
    word: "clamorous",
    meaning: "characterized by extremely loud noise",
    partOfSpeech: "Adjective",
    examples: [
      "The clamorous protest could be heard blocks away.",
      "A clamorous group of children ran through the playground.",
    ],
    synonyms: ["boisterous", "vociferous", "uproarious"],
    antonyms: ["quiet", "silent", "hushed"],
  },
  {
    word: "dearth",
    meaning: "lack of something in a particular area",
    partOfSpeech: "Noun",
    examples: [
      "There was a dearth of reliable information about the crisis.",
      "The drought caused a dearth of fresh produce in markets.",
    ],
    synonyms: ["scarcity", "shortage", "paucity"],
    antonyms: ["abundance", "surplus", "plethora"],
  },
  {
    word: "explicable",
    meaning: "able to be explained or understood",
    partOfSpeech: "Adjective",
    examples: [
      "The sudden market crash was not immediately explicable.",
      "Her anger was explicable given the circumstances.",
    ],
    synonyms: ["understandable", "accountable", "comprehensible"],
    antonyms: ["inexplicable", "unfathomable", "mysterious"],
  },
  {
    word: "hyperbole",
    meaning: "exaggeration to make a point or be funny",
    partOfSpeech: "Noun",
    examples: [
      "Saying 'I'm so hungry I could eat a horse' is hyperbole.",
      "The advertisement used hyperbole to emphasize the product's benefits.",
    ],
    synonyms: ["overstatement", "embellishment", "exaggeration"],
    antonyms: ["understatement", "minimization", "downplay"],
  },
  {
    word: "immutable",
    meaning: "not capable of being changed",
    partOfSpeech: "Adjective",
    examples: [
      "The laws of physics are generally considered immutable.",
      "His immutable principles guided all his decisions.",
    ],
    synonyms: ["unchangeable", "fixed", "permanent"],
    antonyms: ["changeable", "mutable", "flexible"],
  },
  {
    word: "indefatigable",
    meaning: "highly determined; resistant to tiring out",
    partOfSpeech: "Adjective",
    examples: [
      "The indefatigable researcher worked 16-hour days.",
      "Her indefatigable spirit inspired the whole team.",
    ],
    synonyms: ["tireless", "unflagging", "relentless"],
    antonyms: ["lazy", "fatigued", "weary"],
  },
  {
    word: "indolent",
    meaning: "habitually lazy; averse to activity",
    partOfSpeech: "Adjective",
    examples: [
      "The indolent student slept through most classes.",
      "An indolent lifestyle can lead to health problems.",
    ],
    synonyms: ["slothful", "idle", "languid"],
    antonyms: ["industrious", "diligent", "energetic"],
  },
  {
    word: "insular",
    meaning: "limited in outlook or experience; narrow",
    partOfSpeech: "Adjective",
    examples: [
      "His insular worldview prevented him from understanding other cultures.",
      "The small village maintained an insular way of life.",
    ],
    synonyms: ["parochial", "provincial", "narrow-minded"],
    antonyms: ["cosmopolitan", "worldly", "sophisticated"],
  },
  {
    word: "intransigent",
    meaning: "refusing to compromise; stubborn",
    partOfSpeech: "Adjective",
    examples: [
      "The intransigent negotiator wouldn't budge on any terms.",
      "Her intransigent attitude made resolution impossible.",
    ],
    synonyms: ["uncompromising", "inflexible", "unyielding"],
    antonyms: ["flexible", "accommodating", "compliant"],
  },
  {
    word: "intrepid",
    meaning: "resolutely fearless; brave",
    partOfSpeech: "Adjective",
    examples: [
      "Intrepid explorers ventured into uncharted territory.",
      "The intrepid reporter entered the war zone to get the story.",
    ],
    synonyms: ["courageous", "dauntless", "valiant"],
    antonyms: ["timid", "fearful", "cowardly"],
  },
  {
    word: "irreverent",
    meaning: "lacking proper respect or seriousness",
    partOfSpeech: "Adjective",
    examples: [
      "The comedian's irreverent humor offended some audience members.",
      "His irreverent attitude toward authority often got him in trouble.",
    ],
    synonyms: ["disrespectful", "flippant", "impudent"],
    antonyms: ["reverent", "respectful", "deferential"],
  },
  {
    word: "loathe",
    meaning: "dislike greatly, often with disgust",
    partOfSpeech: "Verb",
    examples: [
      "I absolutely loathe the taste of cilantro.",
      "She loathed having to wake up early on weekends.",
    ],
    synonyms: ["detest", "abhor", "despise"],
    antonyms: ["love", "adore", "cherish"],
  },
  {
    word: "malign",
    meaning: "evil and harmful",
    partOfSpeech: "Adjective",
    examples: [
      "The dictator's malign influence corrupted the entire government.",
      "She claimed her ex-husband had malign intentions.",
    ],
    synonyms: ["malevolent", "wicked", "vicious"],
    antonyms: ["benign", "benevolent", "kind"],
  },
  {
    word: "malign",
    meaning: "speak about someone in a bitter and critical manner",
    partOfSpeech: "Verb",
    examples: [
      "The article maligned the mayor without presenting evidence.",
      "He felt maligned by his colleague's unfair accusations.",
    ],
    synonyms: ["defame", "slander", "vilify"],
    antonyms: ["praise", "commend", "compliment"],
  },
  {
    word: "malleable",
    meaning: "changeable or flexible; capable of being influenced",
    partOfSpeech: "Adjective",
    examples: [
      "Gold is highly malleable and can be shaped easily.",
      "Young children's minds are particularly malleable.",
    ],
    synonyms: ["adaptable", "pliable", "impressionable"],
    antonyms: ["inflexible", "rigid", "unyielding"],
  },
  {
    word: "neophyte",
    meaning: "an amateur or novice in a field",
    partOfSpeech: "Noun",
    examples: [
      "As a neophyte gardener, she made many beginner mistakes.",
      "The training program was designed for neophytes in the industry.",
    ],
    synonyms: ["beginner", "novice", "apprentice"],
    antonyms: ["expert", "veteran", "professional"],
  },
  {
    word: "plastic",
    meaning: "capable of being molded; flexible",
    partOfSpeech: "Adjective",
    examples: [
      "The artist preferred clay for its plastic qualities.",
      "Children's minds are wonderfully plastic and adaptable.",
    ],
    synonyms: ["malleable", "pliable", "flexible"],
    antonyms: ["rigid", "inflexible", "unyielding"],
  },
  {
    word: "plastic",
    meaning: "artificial; unnatural",
    partOfSpeech: "Adjective",
    examples: [
      "The movie's plastic characters lacked depth or realism.",
      "She disliked the plastic atmosphere of the trendy nightclub.",
    ],
    synonyms: ["fake", "synthetic", "contrived"],
    antonyms: ["genuine", "authentic", "natural"],
  },
  {
    word: "platitude",
    meaning: "a trite or banal remark or statement",
    partOfSpeech: "Noun",
    examples: [
      "The politician's speech was full of empty platitudes.",
      "'Everything happens for a reason' is a common platitude.",
    ],
    synonyms: ["cliché", "banality", "truism"],
    antonyms: ["originality", "insight", "profundity"],
  },
  {
    word: "prescient",
    meaning: "possessing foresight of upcoming events",
    partOfSpeech: "Adjective",
    examples: [
      "Her prescient warning about the economic crisis went unheeded.",
      "The novel's prescient vision of future technology was remarkable.",
    ],
    synonyms: ["prophetic", "farsighted", "visionary"],
    antonyms: ["shortsighted", "unperceptive", "blind"],
  },
  {
    word: "pristine",
    meaning: "in its original, unspoiled state",
    partOfSpeech: "Adjective",
    examples: [
      "The archeologists found the tomb in pristine condition.",
      "The mountain lake's pristine waters reflected the sky perfectly.",
    ],
    synonyms: ["unspoiled", "immaculate", "untouched"],
    antonyms: ["spoiled", "polluted", "corrupted"],
  },
  {
    word: "reproach",
    meaning: "express criticism towards; reprimand",
    partOfSpeech: "Verb",
    examples: [
      "She reproached him for forgetting their anniversary.",
      "The coach reproached the team for their lack of effort.",
    ],
    synonyms: ["rebuke", "scold", "chide"],
    antonyms: ["praise", "commend", "compliment"],
  },
  {
    word: "robust",
    meaning: "capable of surviving despite adverse conditions",
    partOfSpeech: "Adjective",
    examples: [
      "The robust software didn't crash even under heavy use.",
      "He maintained a robust immune system through exercise.",
    ],
    synonyms: ["hardy", "resilient", "sturdy"],
    antonyms: ["fragile", "delicate", "weak"],
  },
  {
    word: "robust",
    meaning: "(of an object, person, or organization) strong and powerful",
    partOfSpeech: "Adjective",
    examples: [
      "The company reported robust earnings this quarter.",
      "At 70, he still had a robust physique from years of exercise.",
    ],
    synonyms: ["vigorous", "strong", "powerful"],
    antonyms: ["weak", "feeble", "frail"],
  },
  {
    word: "salubrious",
    meaning: "promoting health or well-being",
    partOfSpeech: "Adjective",
    examples: [
      "The mountain air was salubrious for her respiratory condition.",
      "Doctors recommended the salubrious effects of moderate exercise.",
    ],
    synonyms: ["healthful", "beneficial", "wholesome"],
    antonyms: ["unhealthy", "harmful", "deleterious"],
  },
  {
    word: "salubrious",
    meaning: "(of a place or accommodation) pleasant",
    partOfSpeech: "Adjective",
    examples: [
      "They found a salubrious neighborhood to raise their children.",
      "The hotel offered salubrious lodgings at reasonable prices.",
    ],
    synonyms: ["agreeable", "comfortable", "inviting"],
    antonyms: ["unpleasant", "disagreeable", "uninviting"],
  },
  {
    word: "sanction",
    meaning: "authorize; permit",
    partOfSpeech: "Verb",
    examples: [
      "The committee sanctioned the use of additional funds.",
      "The school sanctioned the student protest as long as it remained peaceful.",
    ],
    synonyms: ["approve", "endorse", "ratify"],
    antonyms: ["prohibit", "ban", "forbid"],
  },
  {
    word: "sanction",
    meaning: "penalize; punish",
    partOfSpeech: "Verb",
    examples: [
      "The UN sanctioned the country for human rights violations.",
      "The league sanctioned the team for breaking recruitment rules.",
    ],
    synonyms: ["penalize", "punish", "censure"],
    antonyms: ["reward", "praise", "commend"],
  },
  {
    word: "sedulous",
    meaning: "paying attention to details; diligent",
    partOfSpeech: "Adjective",
    examples: [
      "Her sedulous work habits impressed her supervisors.",
      "The restoration required sedulous attention to historical accuracy.",
    ],
    synonyms: ["assiduous", "meticulous", "painstaking"],
    antonyms: ["negligent", "careless", "slapdash"],
  },
  {
    word: "soporific",
    meaning: "causing sleep or lethargy",
    partOfSpeech: "Adjective",
    examples: [
      "The professor's soporific lecture put half the class to sleep.",
      "The warm room and soporific music made everyone drowsy.",
    ],
    synonyms: ["somnolent", "hypnotic", "sleep-inducing"],
    antonyms: ["stimulating", "invigorating", "energizing"],
  },
  {
    word: "stern",
    meaning: "hard, harsh, or severe in manner",
    partOfSpeech: "Adjective",
    examples: [
      "The stern teacher tolerated no nonsense in her classroom.",
      "His stern expression made the children immediately behave.",
    ],
    synonyms: ["strict", "austere", "unyielding"],
    antonyms: ["lenient", "gentle", "kind"],
  },
  {
    word: "tendentious",
    meaning: "biased toward a particular view, often a provocative one",
    partOfSpeech: "Adjective",
    examples: [
      "The documentary was criticized for its tendentious portrayal of events.",
      "His tendentious arguments alienated more moderate audience members.",
    ],
    synonyms: ["partisan", "prejudiced", "polemical"],
    antonyms: ["impartial", "neutral", "unbiased"],
  },
];

export const GROUP_8 = [
  {
    word: "accentuate",
    meaning: "give emphasis or draw attention to something",
    partOfSpeech: "Verb",
    examples: [
      "The lighting accentuated the dramatic angles of her face.",
      "His speech accentuated the importance of environmental conservation.",
    ],
    synonyms: ["highlight", "emphasize", "underscore"],
    antonyms: ["downplay", "minimize", "understate"],
  },
  {
    word: "conjectural",
    meaning: "pertaining to a guess or inference made with limited data",
    partOfSpeech: "Adjective",
    examples: [
      "The scientist presented conjectural hypotheses that needed further testing.",
      "Our plans remain conjectural until we receive more information.",
    ],
    synonyms: ["speculative", "hypothetical", "theoretical"],
    antonyms: ["certain", "proven", "verified"],
  },
  {
    word: "convivial",
    meaning:
      "demonstrating a jovial nature or a fondness for festivities and companionship",
    partOfSpeech: "Adjective",
    examples: [
      "The convivial atmosphere at the party made everyone feel welcome.",
      "He was known for his convivial personality and love of hosting dinners.",
    ],
    synonyms: ["sociable", "festive", "genial"],
    antonyms: ["unsociable", "withdrawn", "hostile"],
  },
  {
    word: "decadent",
    meaning:
      "displaying excessive indulgence, sometimes to a morally degenerate extent",
    partOfSpeech: "Adjective",
    examples: [
      "The millionaire led a decadent lifestyle of private jets and champagne baths.",
      "The chocolate cake was so rich it felt almost decadent to eat it.",
    ],
    synonyms: ["self-indulgent", "dissolute", "hedonistic"],
    antonyms: ["austere", "moderate", "ascetic"],
  },
  {
    word: "egregious",
    meaning: "remarkably bad or shocking, to a conspicuous degree",
    partOfSpeech: "Adjective",
    examples: [
      "The judge called the defendant's actions an egregious violation of trust.",
      "There were egregious errors in the financial report that required correction.",
    ],
    synonyms: ["flagrant", "glaring", "outrageous"],
    antonyms: ["minor", "slight", "insignificant"],
  },
  {
    word: "evanescent",
    meaning: "vanishing or fading away quickly, like mist or a phantom",
    partOfSpeech: "Adjective",
    examples: [
      "The evanescent morning fog disappeared as the sun rose higher.",
      "Their happiness proved evanescent as financial troubles soon emerged.",
    ],
    synonyms: ["fleeting", "transitory", "ephemeral"],
    antonyms: ["permanent", "enduring", "everlasting"],
  },
  {
    word: "flamboyant",
    meaning: "characterized by a showy, confident, or audacious display",
    partOfSpeech: "Adjective",
    examples: [
      "The flamboyant performer wore sequined costumes and danced extravagantly.",
      "His flamboyant writing style divided critics - some loved it, others hated it.",
    ],
    synonyms: ["ostentatious", "theatrical", "dazzling"],
    antonyms: ["modest", "restrained", "understated"],
  },
  {
    word: "forestall",
    meaning: "prevent something by taking strategic action in advance",
    partOfSpeech: "Verb",
    examples: [
      "The company raised prices to forestall potential losses from inflation.",
      "Quick medical treatment forestalled more serious complications.",
    ],
    synonyms: ["preempt", "avert", "thwart"],
    antonyms: ["allow", "permit", "facilitate"],
  },
  {
    word: "gainsay",
    meaning: "deny or refute an assertion or argument",
    partOfSpeech: "Verb",
    examples: [
      "No one could gainsay the evidence presented by the prosecution.",
      "Her accomplishments were so remarkable that none could gainsay her qualifications.",
    ],
    synonyms: ["contradict", "dispute", "challenge"],
    antonyms: ["confirm", "verify", "endorse"],
  },
  {
    word: "galvanize",
    meaning: "inspire or stir up to action or heightened activity",
    partOfSpeech: "Verb",
    examples: [
      "The tragic event galvanized the community into organizing safety reforms.",
      "The coach's fiery speech galvanized the team for the championship game.",
    ],
    synonyms: ["motivate", "stimulate", "energize"],
    antonyms: ["demotivate", "discourage", "dishearten"],
  },
  {
    word: "indiscriminate",
    meaning:
      "lacking selectivity or judgment, often with potentially harmful effects",
    partOfSpeech: "Adjective",
    examples: [
      "The indiscriminate bombing caused heavy civilian casualties.",
      "Her indiscriminate spending habits left her deep in debt.",
    ],
    synonyms: ["random", "haphazard", "unselective"],
    antonyms: ["selective", "discriminating", "judicious"],
  },
  {
    word: "innocuous",
    meaning: "harmless, having no ill effect",
    partOfSpeech: "Adjective",
    examples: [
      "The spider looked scary but was actually innocuous.",
      "What seemed like an innocuous comment actually offended many people.",
    ],
    synonyms: ["harmless", "inoffensive", "benign"],
    antonyms: ["harmful", "dangerous", "toxic"],
  },
  {
    word: "momentary",
    meaning: "lasting for a brief moment or interval",
    partOfSpeech: "Adjective",
    examples: [
      "There was a momentary pause before the audience erupted in applause.",
      "She felt a momentary pang of guilt before proceeding with her plan.",
    ],
    synonyms: ["fleeting", "transient", "brief"],
    antonyms: ["permanent", "lasting", "enduring"],
  },
  {
    word: "mundane",
    meaning: "commonplace or ordinary, lacking excitement",
    partOfSpeech: "Adjective",
    examples: [
      "He longed for adventure to escape his mundane routine.",
      "The meeting covered mundane topics like office supplies and parking spaces.",
    ],
    synonyms: ["ordinary", "routine", "banal"],
    antonyms: ["extraordinary", "remarkable", "exceptional"],
  },
  {
    word: "nettlesome",
    meaning: "causing irritation or annoyance",
    partOfSpeech: "Adjective",
    examples: [
      "The nettlesome regulations made starting a business difficult.",
      "Her nettlesome habit of interrupting people frustrated her colleagues.",
    ],
    synonyms: ["irritating", "vexing", "bothersome"],
    antonyms: ["pleasing", "delightful", "soothing"],
  },
  {
    word: "nullify",
    meaning: "to render ineffective, cancel out or invalidate",
    partOfSpeech: "Verb",
    examples: [
      "The appeals court nullified the earlier verdict.",
      "Taking this medication will nullify the effects of your allergy shots.",
    ],
    synonyms: ["annul", "void", "negate"],
    antonyms: ["validate", "confirm", "ratify"],
  },
  {
    word: "obviate",
    meaning: "sidestep or bypass a potential problem or difficulty",
    partOfSpeech: "Verb",
    examples: [
      "Advanced planning obviated the need for last-minute changes.",
      "The new technology obviates many of the old system's shortcomings.",
    ],
    synonyms: ["preclude", "prevent", "circumvent"],
    antonyms: ["necessitate", "require", "demand"],
  },
  {
    word: "omnipresent",
    meaning: "existing or seeming to exist everywhere at once",
    partOfSpeech: "Adjective",
    examples: [
      "Security cameras have become omnipresent in urban areas.",
      "In our digital age, social media seems omnipresent.",
    ],
    synonyms: ["ubiquitous", "pervasive", "universal"],
    antonyms: ["rare", "scarce", "limited"],
  },
  {
    word: "oust",
    meaning: "displace or eject from a position or place",
    partOfSpeech: "Verb",
    examples: [
      "The shareholders voted to oust the underperforming CEO.",
      "The new technology quickly ousted the older systems.",
    ],
    synonyms: ["remove", "expel", "depose"],
    antonyms: ["install", "appoint", "retain"],
  },
  {
    word: "palpable",
    meaning: "able to be touched or felt; tangible",
    partOfSpeech: "Adjective",
    examples: [
      "There was palpable tension in the room before the verdict was announced.",
      "The relief was palpable when the missing child was found safe.",
    ],
    synonyms: ["tangible", "perceptible", "noticeable"],
    antonyms: ["intangible", "imperceptible", "subtle"],
  },
  {
    word: "perfidy",
    meaning: "act of betrayal or unfaithfulness",
    partOfSpeech: "Noun",
    examples: [
      "The spy's perfidy shocked his colleagues who had trusted him completely.",
      "History remembers the king's perfidy in breaking the peace treaty.",
    ],
    synonyms: ["treachery", "deceit", "duplicity"],
    antonyms: ["loyalty", "faithfulness", "fidelity"],
  },
  {
    word: "profuse",
    meaning: "abundant, plentiful, or occurring in large amounts",
    partOfSpeech: "Adjective",
    examples: [
      "She offered profuse apologies for her mistake.",
      "The wound caused profuse bleeding that required immediate attention.",
    ],
    synonyms: ["copious", "exuberant", "lavish"],
    antonyms: ["scanty", "meager", "sparse"],
  },
  {
    word: "pugnacious",
    meaning: "quick to engage in a conflict or argument; combative",
    partOfSpeech: "Adjective",
    examples: [
      "The pugnacious boxer thrived on confrontation both in and out of the ring.",
      "His pugnacious attitude made diplomatic negotiations difficult.",
    ],
    synonyms: ["belligerent", "aggressive", "quarrelsome"],
    antonyms: ["peaceable", "amicable", "conciliatory"],
  },
  {
    word: "sagacious",
    meaning: "demonstrating wisdom or sound judgment",
    partOfSpeech: "Adjective",
    examples: [
      "The sagacious old professor offered valuable life advice to his students.",
      "Her sagacious investments paid off handsomely over time.",
    ],
    synonyms: ["wise", "shrewd", "perceptive"],
    antonyms: ["foolish", "unwise", "stupid"],
  },
  {
    word: "sanguine",
    meaning: "cheerfully optimistic, particularly in a difficult situation",
    partOfSpeech: "Adjective",
    examples: [
      "Despite the setbacks, she remained sanguine about the project's success.",
      "His sanguine outlook helped the team stay motivated during tough times.",
    ],
    synonyms: ["optimistic", "hopeful", "confident"],
    antonyms: ["pessimistic", "gloomy", "doubtful"],
  },
  {
    word: "scant",
    meaning: "hardly sufficient, meager, or minimal",
    partOfSpeech: "Adjective",
    examples: [
      "They paid scant attention to the warning signs before the crisis.",
      "The refugees survived on scant rations during their long journey.",
    ],
    synonyms: ["inadequate", "insufficient", "sparse"],
    antonyms: ["ample", "abundant", "plentiful"],
  },
  {
    word: "skullduggery",
    meaning: "sneaky or dishonest actions",
    partOfSpeech: "Noun",
    examples: [
      "The election was marred by accusations of skullduggery on both sides.",
      "Corporate skullduggery often goes unpunished due to legal loopholes.",
    ],
    synonyms: ["trickery", "chicanery", "duplicity"],
    antonyms: ["honesty", "integrity", "fairness"],
  },
  {
    word: "trivial",
    meaning: "of minor significance or value",
    partOfSpeech: "Adjective",
    examples: [
      "Don't waste time on trivial details when there are bigger issues to address.",
      "What seemed trivial at the time turned out to be crucially important.",
    ],
    synonyms: ["insignificant", "petty", "frivolous"],
    antonyms: ["important", "significant", "crucial"],
  },
  {
    word: "utilitarian",
    meaning: "emphasizing practicality and functionality over aesthetics",
    partOfSpeech: "Adjective",
    examples: [
      "The building's utilitarian design prioritized efficiency over beauty.",
      "She preferred utilitarian clothing that was comfortable and durable.",
    ],
    synonyms: ["practical", "functional", "pragmatic"],
    antonyms: ["decorative", "ornamental", "aesthetic"],
  },
  {
    word: "vapid",
    meaning: "uninteresting, lacking flavor or spark",
    partOfSpeech: "Adjective",
    examples: [
      "The conversation was so vapid that I struggled to stay awake.",
      "Critics panned the film as vapid and uninspired.",
    ],
    synonyms: ["insipid", "dull", "banal"],
    antonyms: ["stimulating", "engaging", "vibrant"],
  },
];

export const GROUP_9 = [
  {
    word: "boorish",
    meaning: "rude or unrefined in manners or behavior",
    partOfSpeech: "Adjective",
    examples: [
      "His boorish behavior at the dinner table embarrassed everyone.",
      "She found his boorish remarks completely inappropriate.",
    ],
    synonyms: ["uncouth", "crude", "insensitive"],
    antonyms: ["polite", "refined", "graceful"],
  },
  {
    word: "brook",
    meaning: "to tolerate or endure something, especially an annoyance",
    partOfSpeech: "Verb",
    examples: [
      "She would not brook any criticism of her decision.",
      "He brooked no delay in completing the project.",
    ],
    synonyms: ["tolerate", "endure", "allow"],
    antonyms: ["reject", "refuse", "prohibit"],
  },
  {
    word: "circumspect",
    meaning:
      "cautious or wary, especially when considering potential consequences",
    partOfSpeech: "Adjective",
    examples: [
      "She was circumspect in revealing too much personal information.",
      "A circumspect approach was taken in the negotiations.",
    ],
    synonyms: ["cautious", "prudent", "wary"],
    antonyms: ["reckless", "careless", "rash"],
  },
  {
    word: "comity",
    meaning: "a courteous harmony or mutual respect toward others",
    partOfSpeech: "Noun",
    examples: [
      "International comity is essential for peace.",
      "They maintained comity despite their differences.",
    ],
    synonyms: ["civility", "courtesy", "respect"],
    antonyms: ["hostility", "rudeness", "discord"],
  },
  {
    word: "commensurate",
    meaning: "comparable in size, extent, or degree; proportionate",
    partOfSpeech: "Adjective",
    examples: [
      "The salary should be commensurate with experience.",
      "Punishment must be commensurate with the crime.",
    ],
    synonyms: ["proportionate", "equal", "equivalent"],
    antonyms: ["disproportionate", "unequal", "inappropriate"],
  },
  {
    word: "cordial",
    meaning: "characterized by warmth and sincerity",
    partOfSpeech: "Adjective",
    examples: [
      "They exchanged cordial greetings.",
      "She was cordial toward her former colleagues.",
    ],
    synonyms: ["warm", "friendly", "gracious"],
    antonyms: ["hostile", "cold", "unfriendly"],
  },
  {
    word: "deleterious",
    meaning: "having a harmful effect",
    partOfSpeech: "Adjective",
    examples: [
      "The policy had a deleterious impact on the economy.",
      "Smoking has deleterious effects on health.",
    ],
    synonyms: ["harmful", "damaging", "injurious"],
    antonyms: ["beneficial", "helpful", "advantageous"],
  },
  {
    word: "dichotomy",
    meaning: "a split into two contrasting or opposite parts",
    partOfSpeech: "Noun",
    examples: [
      "There is a clear dichotomy between theory and practice.",
      "The book explores the dichotomy of good and evil.",
    ],
    synonyms: ["contrast", "division", "duality"],
    antonyms: ["unity", "agreement", "harmony"],
  },
  {
    word: "edify",
    meaning: "enlighten or uplift through knowledge; teach",
    partOfSpeech: "Verb",
    examples: [
      "The documentary aimed to edify its audience.",
      "Stories that edify as well as entertain are rare.",
    ],
    synonyms: ["educate", "enlighten", "instruct"],
    antonyms: ["mislead", "confuse", "misinform"],
  },
  {
    word: "elicit",
    meaning: "extract a reply or action; provoke",
    partOfSpeech: "Verb",
    examples: [
      "The teacher's question elicited no response.",
      "He elicited laughter with his jokes.",
    ],
    synonyms: ["evoke", "extract", "draw out"],
    antonyms: ["suppress", "hide", "repress"],
  },
  {
    word: "erudite",
    meaning: "possessing great knowledge, typically from study or research",
    partOfSpeech: "Adjective",
    examples: [
      "She gave an erudite lecture on ancient philosophy.",
      "The professor was known for his erudite writing.",
    ],
    synonyms: ["learned", "scholarly", "intellectual"],
    antonyms: ["ignorant", "uneducated", "uninformed"],
  },
  {
    word: "fecund",
    meaning: "fertile or highly productive",
    partOfSpeech: "Adjective",
    examples: [
      "The fecund soil yielded a bountiful harvest.",
      "Her fecund imagination produced numerous stories.",
    ],
    synonyms: ["fertile", "productive", "fruitful"],
    antonyms: ["barren", "unproductive", "sterile"],
  },
  {
    word: "feeble",
    meaning: "weak; deficient in strength",
    partOfSpeech: "Adjective",
    examples: [
      "He gave a feeble excuse for being late.",
      "The patient’s pulse was feeble after the surgery.",
    ],
    synonyms: ["weak", "frail", "fragile"],
    antonyms: ["strong", "robust", "powerful"],
  },

  {
    word: "felicitous",
    meaning: "well-suited or apt",
    partOfSpeech: "Adjective",
    examples: [
      "She made a felicitous remark during the meeting.",
      "His choice of words was felicitous for the occasion.",
    ],
    synonyms: ["appropriate", "apt", "suitable"],
    antonyms: ["awkward", "inappropriate", "unfitting"],
  },
  {
    word: "felicitous",
    meaning: "causing delight or satisfaction",
    partOfSpeech: "Adjective",
    examples: [
      "The song ended on a felicitous note.",
      "Their reunion was a felicitous moment for the entire family.",
    ],
    synonyms: ["pleasing", "delightful", "charming"],
    antonyms: ["unpleasant", "disagreeable", "disturbing"],
  },
  {
    word: "forbear",
    meaning: "refrain or abstain from doing something",
    partOfSpeech: "Verb",
    examples: [
      "He decided to forbear from commenting further.",
      "She could not forbear a smile.",
    ],
    synonyms: ["refrain", "withhold", "abstain"],
    antonyms: ["indulge", "submit", "yield"],
  },
  {
    word: "haphazard",
    meaning: "lacking clear planning, order, or direction",
    partOfSpeech: "Adjective",
    examples: [
      "Their approach to the task was haphazard at best.",
      "Books lay around the room in a haphazard fashion.",
    ],
    synonyms: ["random", "disorganized", "chaotic"],
    antonyms: ["methodical", "organized", "systematic"],
  },
  {
    word: "hodgepodge",
    meaning: "a mixture or assortment of different things",
    partOfSpeech: "Noun",
    examples: [
      "Her report was a hodgepodge of facts and opinions.",
      "The museum was a hodgepodge of unrelated exhibits.",
    ],
    synonyms: ["jumble", "medley", "mishmash"],
    antonyms: ["uniformity", "consistency", "order"],
  },
  {
    word: "impede",
    meaning: "obstruct or hinder progress",
    partOfSpeech: "Verb",
    examples: [
      "Poor roads impede economic development.",
      "The heavy rain impeded their progress.",
    ],
    synonyms: ["obstruct", "hinder", "block"],
    antonyms: ["assist", "advance", "facilitate"],
  },
  {
    word: "impetuous",
    meaning: "rash or hasty in action or decision",
    partOfSpeech: "Adjective",
    examples: [
      "His impetuous actions got him into trouble.",
      "She regretted her impetuous decision.",
    ],
    synonyms: ["reckless", "rash", "hasty"],
    antonyms: ["cautious", "deliberate", "thoughtful"],
  },
  {
    word: "irascible",
    meaning: "easily angered or irritated",
    partOfSpeech: "Adjective",
    examples: [
      "The irascible old man shouted at the children.",
      "Her irascible nature made her hard to work with.",
    ],
    synonyms: ["irritable", "short-tempered", "testy"],
    antonyms: ["calm", "easygoing", "good-natured"],
  },
  {
    word: "mercenary",
    meaning: "motivated primarily by a desire for monetary or material gain",
    partOfSpeech: "Adjective",
    examples: [
      "His mercenary motives were obvious.",
      "She viewed the deal as a purely mercenary endeavor.",
    ],
    synonyms: ["greedy", "money-minded", "selfish"],
    antonyms: ["selfless", "generous", "altruistic"],
  },
  {
    word: "meticulous",
    meaning: "exhibiting great care to detail",
    partOfSpeech: "Adjective",
    examples: [
      "He is meticulous about his work.",
      "The report was meticulously prepared.",
    ],
    synonyms: ["careful", "precise", "thorough"],
    antonyms: ["careless", "sloppy", "haphazard"],
  },
  {
    word: "mordant",
    meaning: "biting or sharply critical in thought or expression",
    partOfSpeech: "Adjective",
    examples: [
      "The critic's review was mordant but insightful.",
      "She had a mordant wit.",
    ],
    synonyms: ["biting", "sarcastic", "acerbic"],
    antonyms: ["gentle", "mild", "complimentary"],
  },
  {
    word: "outstrip",
    meaning: "surpass or do better than",
    partOfSpeech: "Verb",
    examples: [
      "The new product quickly outstripped its competitors.",
      "She outstripped all expectations.",
    ],
    synonyms: ["surpass", "exceed", "outperform"],
    antonyms: ["fall behind", "lag", "trail"],
  },
  {
    word: "precarious",
    meaning: "uncertain and likely to cause danger",
    partOfSpeech: "Adjective",
    examples: [
      "They were in a precarious financial position.",
      "The ladder looked precarious against the wall.",
    ],
    synonyms: ["unstable", "risky", "uncertain"],
    antonyms: ["safe", "secure", "stable"],
  },
  {
    word: "quirky",
    meaning: "unusual in an endearing or peculiar way",
    partOfSpeech: "Adjective",
    examples: [
      "She has a quirky sense of humor.",
      "The café had a quirky interior design.",
    ],
    synonyms: ["eccentric", "peculiar", "offbeat"],
    antonyms: ["conventional", "normal", "common"],
  },
  {
    word: "repudiate",
    meaning: "reject or refute",
    partOfSpeech: "Verb",
    examples: [
      "He repudiated the allegations.",
      "She publicly repudiated her former beliefs.",
    ],
    synonyms: ["reject", "deny", "disown"],
    antonyms: ["accept", "endorse", "embrace"],
  },
  {
    word: "tact",
    meaning: "skill in handling difficult situations without causing offense",
    partOfSpeech: "Noun",
    examples: [
      "She handled the criticism with tact.",
      "Tact is essential in diplomacy.",
    ],
    synonyms: ["diplomacy", "discretion", "sensitivity"],
    antonyms: ["bluntness", "insensitivity", "rudeness"],
  },
  {
    word: "trifling",
    meaning: "lacking significance or importance",
    partOfSpeech: "Adjective",
    examples: [
      "Don't waste time on trifling matters.",
      "The cost is trifling compared to the benefits.",
    ],
    synonyms: ["insignificant", "minor", "petty"],
    antonyms: ["important", "serious", "meaningful"],
  },
  {
    word: "turbulent",
    meaning: "full of conflict or chaos",
    partOfSpeech: "Adjective",
    examples: [
      "They had a turbulent relationship.",
      "The 1960s were a turbulent decade in history.",
    ],
    synonyms: ["chaotic", "stormy", "unsettled"],
    antonyms: ["peaceful", "calm", "orderly"],
  },
];

export const GROUP_10 = [
  {
    word: "acumen",
    meaning: "quickness and keenness in understanding and judgment",
    partOfSpeech: "Noun",
    examples: [
      "Her business acumen helped the company thrive in a competitive market.",
      "His political acumen made him a formidable strategist.",
    ],
    synonyms: ["shrewdness", "astuteness", "perceptiveness"],
    antonyms: ["stupidity", "ignorance", "naivety"],
  },
  {
    word: "antithesis",
    meaning: "the direct opposite or contrast",
    partOfSpeech: "Noun",
    examples: [
      "Love is the antithesis of hate.",
      "His luxurious lifestyle was the antithesis of minimalism.",
    ],
    synonyms: ["opposite", "contrary", "reverse"],
    antonyms: ["same", "similarity", "equivalent"],
  },
  {
    word: "ascribe",
    meaning: "attribute or credit something to a particular cause or source",
    partOfSpeech: "Verb",
    examples: [
      "Scholars ascribe the poem to an unknown author from the 14th century.",
      "He ascribed his success to hard work and perseverance.",
    ],
    synonyms: ["attribute", "assign", "credit"],
    antonyms: ["deny", "disavow", "reject"],
  },
  {
    word: "befuddled",
    meaning: "confused, bewildered, or perplexed",
    partOfSpeech: "Adjective",
    examples: [
      "The complex instructions left him completely befuddled.",
      "She gave him a befuddled look when he mentioned the obscure reference.",
    ],
    synonyms: ["confused", "bewildered", "perplexed"],
    antonyms: ["clear-headed", "lucid", "understanding"],
  },
  {
    word: "eschew",
    meaning: "intentionally avoid or abstain from",
    partOfSpeech: "Verb",
    examples: [
      "He eschews social media, preferring face-to-face interactions.",
      "The doctor advised her to eschew fatty foods for better health.",
    ],
    synonyms: ["avoid", "shun", "abstain from"],
    antonyms: ["embrace", "indulge in", "seek"],
  },
  {
    word: "esoteric",
    meaning: "understood by few or a small group of people",
    partOfSpeech: "Adjective",
    examples: [
      "The professor's lecture was full of esoteric references only specialists would understand.",
      "The ancient text contained esoteric knowledge passed down through generations.",
    ],
    synonyms: ["obscure", "arcane", "abstruse"],
    antonyms: ["common", "mainstream", "accessible"],
  },
  {
    word: "evasive",
    meaning: "indirect or not straightforward; avoiding confrontation",
    partOfSpeech: "Adjective",
    examples: [
      "The politician gave evasive answers to avoid the controversial topic.",
      "His evasive maneuvers helped him escape the pursuing car.",
    ],
    synonyms: ["elusive", "ambiguous", "slippery"],
    antonyms: ["direct", "forthright", "straightforward"],
  },
  {
    word: "exculpate",
    meaning: "clear from blame or guilt",
    partOfSpeech: "Verb",
    examples: [
      "The new evidence exculpated the wrongly accused man.",
      "The lawyer worked tirelessly to exculpate her client.",
    ],
    synonyms: ["acquit", "absolve", "vindicate"],
    antonyms: ["incriminate", "accuse", "convict"],
  },
  {
    word: "expedite",
    meaning: "accelerate or speed up a process or action",
    partOfSpeech: "Verb",
    examples: [
      "The manager expedited the approval to meet the deadline.",
      "Paying extra can expedite the shipping of your order.",
    ],
    synonyms: ["accelerate", "hasten", "facilitate"],
    antonyms: ["delay", "hinder", "slow down"],
  },
  {
    word: "fastidious",
    meaning: "showing great care and attention to detail; hard to please",
    partOfSpeech: "Adjective",
    examples: [
      "She was fastidious about keeping her workspace impeccably clean.",
      "His fastidious taste in music made him dismissive of popular genres.",
    ],
    synonyms: ["meticulous", "punctilious", "particular"],
    antonyms: ["sloppy", "careless", "undemanding"],
  },
  {
    word: "feign",
    meaning: "pretend or simulate",
    partOfSpeech: "Verb",
    examples: [
      "He feigned illness to avoid going to school.",
      "She feigned interest in the conversation while checking her phone.",
    ],
    synonyms: ["pretend", "fake", "simulate"],
    antonyms: ["be genuine", "be sincere", "be authentic"],
  },
  {
    word: "furtive",
    meaning: "sneaky or secretive, especially due to guilt",
    partOfSpeech: "Adjective",
    examples: [
      "She cast a furtive glance at her watch during the meeting.",
      "His furtive behavior raised suspicions among his colleagues.",
    ],
    synonyms: ["sneaky", "secretive", "sly"],
    antonyms: ["open", "overt", "honest"],
  },
  {
    word: "hamper",
    meaning: "obstruct or hinder progress",
    partOfSpeech: "Verb",
    examples: [
      "Bad weather hampered the rescue efforts.",
      "Bureaucratic red tape can hamper innovation in large organizations.",
    ],
    synonyms: ["hinder", "impede", "obstruct"],
    antonyms: ["facilitate", "assist", "promote"],
  },
  {
    word: "indispensable",
    meaning: "completely necessary; essential",
    partOfSpeech: "Adjective",
    examples: [
      "Clean water is indispensable for human survival.",
      "Her role in the project was indispensable to its success.",
    ],
    synonyms: ["essential", "crucial", "vital"],
    antonyms: ["dispensable", "unnecessary", "nonessential"],
  },
  {
    word: "lament",
    meaning: "express deep sorrow or regret",
    partOfSpeech: "Verb",
    examples: [
      "The poet lamented the loss of innocence in modern society.",
      "Fans lamented the cancellation of their favorite TV show.",
    ],
    synonyms: ["mourn", "bemoan", "grieve"],
    antonyms: ["celebrate", "rejoice", "applaud"],
  },
  {
    word: "myopic",
    meaning: "short-sighted; lacking creativity",
    partOfSpeech: "Adjective",
    examples: [
      "The company's myopic focus on quarterly profits hurt its long-term growth.",
      "His myopic view prevented him from seeing the bigger picture.",
    ],
    synonyms: ["short-sighted", "narrow-minded", "unimaginative"],
    antonyms: ["farsighted", "visionary", "broad-minded"],
  },
  {
    word: "nonchalant",
    meaning: "appearing casually calm or indifferent",
    partOfSpeech: "Adjective",
    examples: [
      "He tried to appear nonchalant despite the embarrassing situation.",
      "She gave a nonchalant shrug when asked about her achievements.",
    ],
    synonyms: ["unconcerned", "indifferent", "blasé"],
    antonyms: ["anxious", "concerned", "enthusiastic"],
  },
  {
    word: "partial",
    meaning: "prejudiced or biased toward one side",
    partOfSpeech: "Adjective",
    examples: [
      "The judge was accused of being partial toward the wealthy defendant.",
      "Her partial account of events made the story unreliable.",
    ],
    synonyms: ["biased", "prejudiced", "one-sided"],
    antonyms: ["impartial", "unbiased", "fair"],
  },
  {
    word: "pensive",
    meaning: "deep in thought",
    partOfSpeech: "Adjective",
    examples: [
      "She had a pensive expression as she stared out the window.",
      "The book left him in a pensive mood for the rest of the evening.",
    ],
    synonyms: ["thoughtful", "contemplative", "reflective"],
    antonyms: ["thoughtless", "unreflective", "shallow"],
  },
  {
    word: "portend",
    meaning:
      "indicate or warn that something, often unpleasant, is going to happen",
    partOfSpeech: "Verb",
    examples: [
      "The dark clouds portended a severe storm.",
      "The economic downturn portended layoffs across the industry.",
    ],
    synonyms: ["foretell", "augur", "presage"],
    antonyms: ["disregard", "ignore", "overlook"],
  },
  {
    word: "provincial",
    meaning: "narrow in perspective; not sophisticated",
    partOfSpeech: "Adjective",
    examples: [
      "His provincial attitudes made him uncomfortable in diverse cities.",
      "The small-town newspaper had a provincial outlook on global issues.",
    ],
    synonyms: ["narrow-minded", "parochial", "unsophisticated"],
    antonyms: ["cosmopolitan", "sophisticated", "worldly"],
  },
  {
    word: "rudimentary",
    meaning: "basic or elementary; in the earliest stages of development",
    partOfSpeech: "Adjective",
    examples: [
      "The tribe had only rudimentary tools made of stone and wood.",
      "Her knowledge of French is rudimentary at best.",
    ],
    synonyms: ["basic", "elementary", "primitive"],
    antonyms: ["advanced", "sophisticated", "developed"],
  },
  {
    word: "salutary",
    meaning: "beneficial, especially in promoting health or well-being",
    partOfSpeech: "Adjective",
    examples: [
      "The strict regulations had a salutary effect on public safety.",
      "The doctor recommended salutary changes to her diet and exercise routine.",
    ],
    synonyms: ["beneficial", "healthful", "wholesome"],
    antonyms: ["harmful", "damaging", "deleterious"],
  },
  {
    word: "sever",
    meaning: "cut off or put an end to",
    partOfSpeech: "Verb",
    examples: [
      "The dictator severed all diplomatic ties with neighboring countries.",
      "The accident severed the cable, causing a blackout.",
    ],
    synonyms: ["cut", "disconnect", "terminate"],
    antonyms: ["join", "unite", "connect"],
  },
  {
    word: "slight",
    meaning: "treat with disrespect or neglect; insult",
    partOfSpeech: "Verb",
    examples: [
      "She felt slighted when her contributions were ignored.",
      "He took offense at what he perceived as a slight to his reputation.",
    ],
    synonyms: ["snub", "insult", "disrespect"],
    antonyms: ["respect", "honor", "praise"],
  },
  {
    word: "somnolent",
    meaning: "sleepy or drowsy",
    partOfSpeech: "Adjective",
    examples: [
      "The somnolent audience struggled to stay awake during the lengthy lecture.",
      "The warm, somnolent afternoon made productivity nearly impossible.",
    ],
    synonyms: ["sleepy", "drowsy", "lethargic"],
    antonyms: ["alert", "awake", "energetic"],
  },
  {
    word: "stoic",
    meaning: "unaffected by pain or distress; showing no emotion",
    partOfSpeech: "Adjective",
    examples: [
      "She remained stoic throughout the ordeal, never shedding a tear.",
      "His stoic demeanor under pressure earned him respect.",
    ],
    synonyms: ["unemotional", "impassive", "phlegmatic"],
    antonyms: ["emotional", "expressive", "sensitive"],
  },
  {
    word: "supersede",
    meaning: "replace or take the place of",
    partOfSpeech: "Verb",
    examples: [
      "The new policy will supersede the old one starting next month.",
      "Digital media has largely superseded physical formats like CDs and DVDs.",
    ],
    synonyms: ["replace", "supplant", "override"],
    antonyms: ["precede", "retain", "preserve"],
  },
  {
    word: "tout",
    meaning: "promote or praise energetically",
    partOfSpeech: "Verb",
    examples: [
      "The company touted its new product as a revolutionary breakthrough.",
      "Critics touted the film as the director's best work yet.",
    ],
    synonyms: ["promote", "praise", "hype"],
    antonyms: ["criticize", "denigrate", "disparage"],
  },
  {
    word: "wane",
    meaning: "decrease in power, size, or intensity",
    partOfSpeech: "Verb",
    examples: [
      "The moon wanes after the full moon phase.",
      "Her enthusiasm for the project began to wane as difficulties arose.",
    ],
    synonyms: ["decline", "diminish", "fade"],
    antonyms: ["wax", "grow", "increase"],
  },
];

export const GROUP_11 = [
  {
    word: "abhor",
    meaning: "detest or regard with deep hatred",
    partOfSpeech: "Verb",
    examples: [
      "She abhors violence in any form.",
      "He abhors people who are cruel to animals.",
    ],
    synonyms: ["detest", "loathe", "despise"],
    antonyms: ["adore", "love", "cherish"],
  },
  {
    word: "boisterous",
    meaning: "energetic, noisy, and lively",
    partOfSpeech: "Adjective",
    examples: [
      "The boisterous crowd cheered loudly at the concert.",
      "Boisterous children ran through the playground.",
    ],
    synonyms: ["rowdy", "rambunctious", "lively"],
    antonyms: ["quiet", "calm", "subdued"],
  },
  {
    word: "chivalrous",
    meaning: "honorable and polite",
    partOfSpeech: "Adjective",
    examples: [
      "His chivalrous behavior included holding doors open for others.",
      "The knight was known for his chivalrous deeds.",
    ],
    synonyms: ["gallant", "courteous", "noble"],
    antonyms: ["rude", "discourteous", "ungentlemanly"],
  },
  {
    word: "churlish",
    meaning: "rude or lacking good manners",
    partOfSpeech: "Adjective",
    examples: [
      "His churlish remarks offended everyone at the dinner party.",
      "It would be churlish to refuse such a generous offer.",
    ],
    synonyms: ["rude", "ill-mannered", "boorish"],
    antonyms: ["polite", "courteous", "gracious"],
  },
  {
    word: "clandestine",
    meaning: "secretive; avoiding attention",
    partOfSpeech: "Adjective",
    examples: [
      "They held clandestine meetings to plan the surprise party.",
      "The spy carried out clandestine operations.",
    ],
    synonyms: ["secret", "covert", "surreptitious"],
    antonyms: ["open", "overt", "public"],
  },
  {
    word: "complacent",
    meaning:
      "smugly self-satisfied, often without awareness of potential dangers",
    partOfSpeech: "Adjective",
    examples: [
      "The team became complacent after their early success.",
      "We cannot afford to be complacent about climate change.",
    ],
    synonyms: ["smug", "self-satisfied", "overconfident"],
    antonyms: ["dissatisfied", "ambitious", "striving"],
  },
  {
    word: "cumbersome",
    meaning: "large, heavy, slow or complicated",
    partOfSpeech: "Adjective",
    examples: [
      "The cumbersome package was difficult to carry.",
      "The legal process was slow and cumbersome.",
    ],
    synonyms: ["unwieldy", "bulky", "clumsy"],
    antonyms: ["compact", "manageable", "streamlined"],
  },
  {
    word: "debilitating",
    meaning: "weakening or impairing strength",
    partOfSpeech: "Adjective",
    examples: [
      "She suffered from a debilitating illness for years.",
      "The heat had a debilitating effect on the marathon runners.",
    ],
    synonyms: ["weakening", "enfeebling", "crippling"],
    antonyms: ["strengthening", "invigorating", "energizing"],
  },
  {
    word: "deliberate",
    meaning: "1. think over or discuss carefully 2. intentional",
    partOfSpeech: "Verb/Adjective",
    examples: [
      "The jury will deliberate the verdict carefully.",
      "That was a deliberate attempt to mislead us.",
    ],
    synonyms: ["ponder", "consider", "intentional"],
    antonyms: ["impulsive", "accidental", "unintentional"],
  },
  {
    word: "droll",
    meaning: "amusing in a quirky or unusual way",
    partOfSpeech: "Adjective",
    examples: [
      "His droll sense of humor wasn't appreciated by everyone.",
      "The play had a droll, offbeat charm.",
    ],
    synonyms: ["whimsical", "quirky", "wry"],
    antonyms: ["serious", "solemn", "humorless"],
  },
  {
    word: "eccentric",
    meaning: "deviating from conventional behavior; quirky",
    partOfSpeech: "Adjective",
    examples: [
      "The eccentric professor wore mismatched shoes to class.",
      "Her eccentric habits made her the talk of the neighborhood.",
    ],
    synonyms: ["unconventional", "peculiar", "odd"],
    antonyms: ["conventional", "normal", "ordinary"],
  },
  {
    word: "fractious",
    meaning: "quarrelsome and irritable; unruly",
    partOfSpeech: "Adjective",
    examples: [
      "The fractious child refused to cooperate.",
      "The meeting became fractious as tempers flared.",
    ],
    synonyms: ["irritable", "cranky", "unruly"],
    antonyms: ["cooperative", "agreeable", "docile"],
  },
  {
    word: "limpid",
    meaning: "clear, transparent, or easy to understand",
    partOfSpeech: "Adjective",
    examples: [
      "The limpid water revealed colorful fish below.",
      "Her limpid prose made complex ideas accessible.",
    ],
    synonyms: ["clear", "transparent", "lucid"],
    antonyms: ["murky", "opaque", "unclear"],
  },
  {
    word: "mawkish",
    meaning: "overly sentimental in a way that seems exaggerated or insincere",
    partOfSpeech: "Adjective",
    examples: [
      "The movie's mawkish ending felt forced.",
      "He avoided mawkish displays of emotion.",
    ],
    synonyms: ["saccharine", "cloying", "maudlin"],
    antonyms: ["unsentimental", "stoic", "restrained"],
  },
  {
    word: "obeisance",
    meaning: "reverence for an individual",
    partOfSpeech: "Noun",
    examples: [
      "They made obeisance to the king by bowing deeply.",
      "The traditional obeisance showed respect for elders.",
    ],
    synonyms: ["homage", "respect", "deference"],
    antonyms: ["disrespect", "insolence", "impertinence"],
  },
  {
    word: "ostentatious",
    meaning: "showy or designed to impress",
    partOfSpeech: "Adjective",
    examples: [
      "The mansion's ostentatious decor was overwhelming.",
      "She avoided ostentatious displays of wealth.",
    ],
    synonyms: ["flamboyant", "gaudy", "pretentious"],
    antonyms: ["modest", "understated", "restrained"],
  },
  {
    word: "panacea",
    meaning: "a perfect cure for something",
    partOfSpeech: "Noun",
    examples: [
      "There is no panacea for all of society's problems.",
      "He mistakenly believed the new policy would be a panacea.",
    ],
    synonyms: ["cure-all", "universal remedy", "magic bullet"],
    antonyms: ["problem", "disease", "affliction"],
  },
  {
    word: "perfunctory",
    meaning: "done without enthusiasm or effort",
    partOfSpeech: "Adjective",
    examples: [
      "He gave a perfunctory apology without meaning it.",
      "The inspection was perfunctory at best.",
    ],
    synonyms: ["cursory", "mechanical", "halfhearted"],
    antonyms: ["enthusiastic", "thorough", "meticulous"],
  },
  {
    word: "perilous",
    meaning: "risky and hazardous",
    partOfSpeech: "Adjective",
    examples: [
      "The climb up the mountain was perilous.",
      "They found themselves in a perilous situation.",
    ],
    synonyms: ["dangerous", "risky", "hazardous"],
    antonyms: ["safe", "secure", "protected"],
  },
  {
    word: "pervasive",
    meaning: "spreading quickly and in great numbers",
    partOfSpeech: "Adjective",
    examples: [
      "The pervasive smell of smoke filled the building.",
      "Corruption was pervasive in the government.",
    ],
    synonyms: ["widespread", "ubiquitous", "prevalent"],
    antonyms: ["rare", "scarce", "limited"],
  },
  {
    word: "preclude",
    meaning: "prevent something from occurring",
    partOfSpeech: "Verb",
    examples: [
      "His injury precluded him from playing in the game.",
      "The contract precludes any future lawsuits.",
    ],
    synonyms: ["prevent", "prohibit", "rule out"],
    antonyms: ["allow", "permit", "enable"],
  },
  {
    word: "predilection",
    meaning: "preference or bias towards something",
    partOfSpeech: "Noun",
    examples: [
      "She has a predilection for romantic comedies.",
      "His predilection for spicy food was well-known.",
    ],
    synonyms: ["preference", "partiality", "propensity"],
    antonyms: ["aversion", "dislike", "distaste"],
  },
  {
    word: "rapacious",
    meaning: "very greedy",
    partOfSpeech: "Adjective",
    examples: [
      "The rapacious corporation exploited its workers.",
      "Pirates were known for their rapacious nature.",
    ],
    synonyms: ["greedy", "avaricious", "predatory"],
    antonyms: ["generous", "altruistic", "benevolent"],
  },
  {
    word: "relish",
    meaning: "take great pleasure or delight in",
    partOfSpeech: "Verb",
    examples: [
      "He relished the opportunity to prove his critics wrong.",
      "She relished every bite of the delicious meal.",
    ],
    synonyms: ["enjoy", "savor", "delight in"],
    antonyms: ["dislike", "detest", "abhor"],
  },
  {
    word: "satirical",
    meaning: "using humor or irony to criticize or mock",
    partOfSpeech: "Adjective",
    examples: [
      "The show was a satirical take on modern politics.",
      "His satirical cartoons often angered politicians.",
    ],
    synonyms: ["ironic", "sarcastic", "mocking"],
    antonyms: ["serious", "earnest", "sincere"],
  },
  {
    word: "sham",
    meaning: "fake; not genuine",
    partOfSpeech: "Noun/Adjective",
    examples: [
      "The investigation exposed the company as a sham.",
      "His apology was nothing but a sham.",
    ],
    synonyms: ["fake", "fraud", "pretence"],
    antonyms: ["genuine", "real", "authentic"],
  },
  {
    word: "skirt",
    meaning: "go around or avoid something, especially an issue or obstacle",
    partOfSpeech: "Verb",
    examples: [
      "The politician skirted the question about his finances.",
      "We skirted the edge of the forest to avoid detection.",
    ],
    synonyms: ["avoid", "evade", "circumvent"],
    antonyms: ["confront", "face", "address"],
  },
  {
    word: "sluggish",
    meaning: "lacking energy; slow and listless",
    partOfSpeech: "Adjective",
    examples: [
      "The hot weather made everyone feel sluggish.",
      "The economy showed sluggish growth this quarter.",
    ],
    synonyms: ["lethargic", "inactive", "slow"],
    antonyms: ["energetic", "lively", "vigorous"],
  },
  {
    word: "spartan",
    meaning: "marked by simplicity, frugality, and avoidance of luxury",
    partOfSpeech: "Adjective",
    examples: [
      "His spartan apartment had only the bare essentials.",
      "The athletes maintained a spartan training regimen.",
    ],
    synonyms: ["austere", "ascetic", "frugal"],
    antonyms: ["luxurious", "opulent", "lavish"],
  },
  {
    word: "truculent",
    meaning: "aggressively combative or confrontational",
    partOfSpeech: "Adjective",
    examples: [
      "The truculent teenager argued with everyone.",
      "His truculent attitude made negotiations difficult.",
    ],
    synonyms: ["belligerent", "defiant", "hostile"],
    antonyms: ["cooperative", "friendly", "peaceable"],
  },
];

export const GROUP_12 = [
  {
    word: "acrimonious",
    meaning: "bitter and sharp in language or tone",
    partOfSpeech: "Adjective",
    examples: [
      "The divorce proceedings became increasingly acrimonious.",
      "Their acrimonious debate left the audience uncomfortable.",
    ],
    synonyms: ["bitter", "caustic", "hostile"],
    antonyms: ["harmonious", "amicable", "cordial"],
  },
  {
    word: "belligerent",
    meaning: "argumentative, threatening, or aggressive",
    partOfSpeech: "Adjective",
    examples: [
      "The belligerent drunk started fights with everyone at the bar.",
      "The nation took a belligerent stance toward its neighbors.",
    ],
    synonyms: ["aggressive", "hostile", "combative"],
    antonyms: ["peaceful", "friendly", "cooperative"],
  },
  {
    word: "beneficent",
    meaning: "kind and charitable, producing good results or benefits",
    partOfSpeech: "Adjective",
    examples: [
      "The beneficent millionaire donated his entire fortune to charity.",
      "The new policy had beneficent effects on public health.",
    ],
    synonyms: ["benevolent", "philanthropic", "altruistic"],
    antonyms: ["malevolent", "selfish", "harmful"],
  },
  {
    word: "canny",
    meaning: "shrewd and cautious",
    partOfSpeech: "Adjective",
    examples: [
      "The canny investor bought the stock just before its value soared.",
      "Her canny negotiation skills saved the company millions.",
    ],
    synonyms: ["astute", "shrewd", "prudent"],
    antonyms: ["naive", "foolish", "gullible"],
  },
  {
    word: "cavalier",
    meaning: "exhibiting an overly casual attitude; unconcerned",
    partOfSpeech: "Adjective",
    examples: [
      "His cavalier dismissal of the safety concerns angered the workers.",
      "She took a cavalier approach to her finances and soon went into debt.",
    ],
    synonyms: ["nonchalant", "indifferent", "flippant"],
    antonyms: ["attentive", "concerned", "diligent"],
  },
  {
    word: "distressed",
    meaning: "experiencing nervousness, irritation, or sadness",
    partOfSpeech: "Adjective",
    examples: [
      "The distressed mother searched frantically for her lost child.",
      "He sounded deeply distressed when he called about the accident.",
    ],
    synonyms: ["upset", "anguished", "troubled"],
    antonyms: ["calm", "composed", "untroubled"],
  },
  {
    word: "dwindling",
    meaning: "decreasing steadily in size or quantity",
    partOfSpeech: "Adjective",
    examples: [
      "The dwindling water supply caused concern among city officials.",
      "Her dwindling patience was evident in her sharp replies.",
    ],
    synonyms: ["declining", "diminishing", "shrinking"],
    antonyms: ["increasing", "growing", "expanding"],
  },
  {
    word: "eclipse",
    meaning: "overshadow or surpass",
    partOfSpeech: "Verb",
    examples: [
      "Her talent quickly eclipsed that of her teachers.",
      "The new skyscraper eclipsed all other buildings in the city.",
    ],
    synonyms: ["outshine", "surpass", "dwarf"],
    antonyms: ["trail", "lag", "fall behind"],
  },
  {
    word: "encyclopedic",
    meaning: "thorough; comprehensive in scope",
    partOfSpeech: "Adjective",
    examples: [
      "His encyclopedic knowledge of jazz impressed the audience.",
      "The professor gave an encyclopedic lecture covering centuries of history.",
    ],
    synonyms: ["comprehensive", "exhaustive", "all-encompassing"],
    antonyms: ["limited", "narrow", "superficial"],
  },
  {
    word: "exacerbate",
    meaning: "make a situation or condition worse",
    partOfSpeech: "Verb",
    examples: [
      "His angry comments only exacerbated the tension in the room.",
      "The drought exacerbated the already dire food shortage.",
    ],
    synonyms: ["worsen", "aggravate", "intensify"],
    antonyms: ["alleviate", "mitigate", "improve"],
  },
  {
    word: "exasperated",
    meaning: "severely irritated or angry",
    partOfSpeech: "Adjective",
    examples: [
      "The exasperated teacher finally sent the disruptive student to the principal.",
      "She gave an exasperated sigh when her computer crashed again.",
    ],
    synonyms: ["frustrated", "irritated", "aggravated"],
    antonyms: ["pleased", "delighted", "content"],
  },
  {
    word: "fungible",
    meaning: "interchangeable for another item of a similar type",
    partOfSpeech: "Adjective",
    examples: [
      "Bitcoin is fungible - one bitcoin can be exchanged for another.",
      "In this recipe, butter and margarine are fungible ingredients.",
    ],
    synonyms: ["interchangeable", "exchangeable", "replaceable"],
    antonyms: ["unique", "noninterchangeable", "distinct"],
  },
  {
    word: "hackneyed",
    meaning: "lacking creativity; derivative and overused",
    partOfSpeech: "Adjective",
    examples: [
      "The movie relied on hackneyed plot devices we've all seen before.",
      "His speech was full of hackneyed phrases that meant nothing.",
    ],
    synonyms: ["clichéd", "trite", "banal"],
    antonyms: ["original", "fresh", "innovative"],
  },
  {
    word: "incongruous",
    meaning: "out of place; lacking harmony",
    partOfSpeech: "Adjective",
    examples: [
      "The modern art piece looked incongruous in the classical gallery.",
      "His casual attire was incongruous with the formal occasion.",
    ],
    synonyms: ["inconsistent", "incompatible", "jarring"],
    antonyms: ["harmonious", "consistent", "compatible"],
  },
  {
    word: "interchangeable",
    meaning: "capable of being used in place of each other",
    partOfSpeech: "Adjective",
    examples: [
      "The parts are completely interchangeable between models.",
      "In this context, the two terms are virtually interchangeable.",
    ],
    synonyms: ["exchangeable", "fungible", "identical"],
    antonyms: ["unique", "distinct", "individual"],
  },
  {
    word: "laconic",
    meaning: "economical with words; quiet and reserved",
    partOfSpeech: "Adjective",
    examples: [
      "His laconic reply of 'Fine' ended the conversation.",
      "The sheriff was known for his laconic style of speaking.",
    ],
    synonyms: ["concise", "terse", "succinct"],
    antonyms: ["verbose", "loquacious", "wordy"],
  },
  {
    word: "lucrative",
    meaning: "producing a significant amount of money",
    partOfSpeech: "Adjective",
    examples: [
      "She left her job for a more lucrative position in finance.",
      "The app proved to be a lucrative business venture.",
    ],
    synonyms: ["profitable", "remunerative", "gainful"],
    antonyms: ["unprofitable", "fruitless", "unremunerative"],
  },
  {
    word: "magisterial",
    meaning:
      "1. possessing great authority 2. exercising authority in an excessive manner",
    partOfSpeech: "Adjective",
    examples: [
      "The professor delivered a magisterial lecture on ancient history.",
      "His magisterial tone annoyed those who preferred collaboration.",
    ],
    synonyms: ["authoritative", "commanding", "imperious"],
    antonyms: ["humble", "meek", "unassuming"],
  },
  {
    word: "onerous",
    meaning: "involving a heavy burden; challenging or difficult",
    partOfSpeech: "Adjective",
    examples: [
      "The new regulations placed an onerous burden on small businesses.",
      "She found the responsibilities of leadership to be onerous at times.",
    ],
    synonyms: ["burdensome", "oppressive", "arduous"],
    antonyms: ["easy", "light", "effortless"],
  },
  {
    word: "opprobrium",
    meaning: "severe criticism or public shame",
    partOfSpeech: "Noun",
    examples: [
      "The scandal brought opprobrium upon the entire political party.",
      "He suffered the opprobrium of his peers after the failed experiment.",
    ],
    synonyms: ["disgrace", "infamy", "scorn"],
    antonyms: ["praise", "honor", "esteem"],
  },
  {
    word: "parsimonious",
    meaning: "extremely frugal or stingy",
    partOfSpeech: "Adjective",
    examples: [
      "His parsimonious habits allowed him to retire early.",
      "The parsimonious landlord refused to fix the broken heater.",
    ],
    synonyms: ["stingy", "miserly", "penny-pinching"],
    antonyms: ["generous", "lavish", "prodigal"],
  },
  {
    word: "peripheral",
    meaning: "of marginal importance; minor",
    partOfSpeech: "Adjective",
    examples: [
      "The issue was peripheral to the main discussion.",
      "She only had a peripheral role in the project's success.",
    ],
    synonyms: ["secondary", "incidental", "tangential"],
    antonyms: ["central", "crucial", "essential"],
  },
  {
    word: "provocative",
    meaning: "causing outrage or irritation, often intentionally",
    partOfSpeech: "Adjective",
    examples: [
      "The artist's provocative work sparked heated debates.",
      "She made provocative comments just to get a reaction.",
    ],
    synonyms: ["inflammatory", "controversial", "stimulating"],
    antonyms: ["soothing", "conciliatory", "uncontroversial"],
  },
  {
    word: "renounce",
    meaning: "reject or give up",
    partOfSpeech: "Verb",
    examples: [
      "He renounced his claim to the throne.",
      "She renounced violence and became a peace activist.",
    ],
    synonyms: ["abandon", "relinquish", "repudiate"],
    antonyms: ["claim", "embrace", "accept"],
  },
  {
    word: "tempestuous",
    meaning:
      "1. (of emotions or actions) unrestrained and turbulent 2. (of weather) extremely stormy",
    partOfSpeech: "Adjective",
    examples: [
      "Their tempestuous relationship was full of dramatic fights and reconciliations.",
      "The small boat struggled in the tempestuous seas.",
    ],
    synonyms: ["turbulent", "stormy", "volatile"],
    antonyms: ["calm", "peaceful", "tranquil"],
  },
  {
    word: "tenable",
    meaning: "(of an idea or method) justifiable and rational",
    partOfSpeech: "Adjective",
    examples: [
      "His theory was no longer tenable after new evidence emerged.",
      "The argument that the earth is flat is not scientifically tenable.",
    ],
    synonyms: ["defensible", "reasonable", "plausible"],
    antonyms: ["untenable", "indefensible", "unreasonable"],
  },
  {
    word: "transgression",
    meaning: "a violation of a law, rule, or social norm",
    partOfSpeech: "Noun",
    examples: [
      "The priest forgave his transgression during confession.",
      "Minor transgressions were punished with detention.",
    ],
    synonyms: ["offense", "misdeed", "violation"],
    antonyms: ["observance", "obedience", "compliance"],
  },
  {
    word: "urbane",
    meaning: "refined, sophisticated, and courteous",
    partOfSpeech: "Adjective",
    examples: [
      "The urbane diplomat charmed everyone at the reception.",
      "His urbane manners made him popular in high society.",
    ],
    synonyms: ["suave", "polished", "cultivated"],
    antonyms: ["rustic", "uncouth", "unsophisticated"],
  },
  {
    word: "verisimilitude",
    meaning: "appearance or semblance of truth or reality; believability",
    partOfSpeech: "Noun",
    examples: [
      "The novel's attention to detail gave it great verisimilitude.",
      "The special effects added verisimilitude to the science fiction movie.",
    ],
    synonyms: ["realism", "authenticity", "plausibility"],
    antonyms: ["implausibility", "unreality", "falseness"],
  },
  {
    word: "vitiate",
    meaning: "impair or corrupt the quality of something",
    partOfSpeech: "Verb",
    examples: [
      "The fraudulent data vitiated the entire study.",
      "His prejudiced comments vitiated the otherwise productive discussion.",
    ],
    synonyms: ["contaminate", "spoil", "undermine"],
    antonyms: ["purify", "strengthen", "validate"],
  },
];

export const GROUP_13 = [
  {
    word: "affinity",
    meaning:
      "1. a natural liking or connection to a person, thing, or idea 2. a similarity between two things",
    partOfSpeech: "Noun",
    examples: [
      "She had an affinity for classical music from a young age.",
      "There's a strong affinity between these two chemical compounds.",
    ],
    synonyms: ["rapport", "connection", "similarity"],
    antonyms: ["aversion", "dislike", "dissimilarity"],
  },
  {
    word: "altruistic",
    meaning: "showing generosity and concern for the welfare of others",
    partOfSpeech: "Adjective",
    examples: [
      "Her altruistic actions during the crisis saved many lives.",
      "The billionaire made an altruistic donation to medical research.",
    ],
    synonyms: ["selfless", "philanthropic", "benevolent"],
    antonyms: ["selfish", "egoistic", "greedy"],
  },
  {
    word: "baroque",
    meaning: "overly detailed or ornamental",
    partOfSpeech: "Adjective",
    examples: [
      "The baroque architecture was covered in intricate carvings.",
      "His writing style was unnecessarily baroque and difficult to follow.",
    ],
    synonyms: ["ornate", "elaborate", "flamboyant"],
    antonyms: ["plain", "simple", "minimalist"],
  },
  {
    word: "byzantine",
    meaning: "overly complex or intricate",
    partOfSpeech: "Adjective",
    examples: [
      "The byzantine tax code confused even the accountants.",
      "They navigated the byzantine bureaucracy to get their permits.",
    ],
    synonyms: ["convoluted", "tangled", "labyrinthine"],
    antonyms: ["straightforward", "simple", "clear"],
  },
  {
    word: "compromise",
    meaning: "weaken or diminish in quality, character, or value",
    partOfSpeech: "Verb",
    examples: [
      "Using cheap materials will compromise the building's structural integrity.",
      "He refused to compromise his principles for political gain.",
    ],
    synonyms: ["weaken", "undermine", "dilute"],
    antonyms: ["strengthen", "enhance", "fortify"],
  },
  {
    word: "conciliatory",
    meaning: "intended to appease or calm someone down",
    partOfSpeech: "Adjective",
    examples: [
      "The CEO made conciliatory remarks to the striking workers.",
      "She took a conciliatory tone to defuse the argument.",
    ],
    synonyms: ["placating", "pacific", "appeasing"],
    antonyms: ["provocative", "aggressive", "combative"],
  },
  {
    word: "countenance",
    meaning: "1. accept or permit 2. the look of a person's face or expression",
    partOfSpeech: "Verb/Noun",
    examples: [
      "The school cannot countenance cheating of any kind.",
      "His stern countenance made the children behave immediately.",
    ],
    synonyms: ["tolerate", "approve", "expression"],
    antonyms: ["prohibit", "forbid", "disapprove"],
  },
  {
    word: "covert",
    meaning: "hidden or concealed; not openly practiced",
    partOfSpeech: "Adjective",
    examples: [
      "The spy carried out covert operations behind enemy lines.",
      "Their covert relationship was unknown to colleagues.",
    ],
    synonyms: ["secret", "clandestine", "surreptitious"],
    antonyms: ["overt", "open", "public"],
  },
  {
    word: "credible",
    meaning: "able to be trusted; persuasive",
    partOfSpeech: "Adjective",
    examples: [
      "The witness gave credible testimony in court.",
      "We need credible evidence before making accusations.",
    ],
    synonyms: ["believable", "trustworthy", "plausible"],
    antonyms: ["incredible", "unbelievable", "implausible"],
  },
  {
    word: "diffuse",
    meaning: "1. spread or disperse widely 2. scattered over a large area",
    partOfSpeech: "Verb/Adjective",
    examples: [
      "The organization worked to diffuse knowledge about climate change.",
      "The diffuse lighting created a soft glow throughout the room.",
    ],
    synonyms: ["disperse", "disseminate", "scattered"],
    antonyms: ["concentrate", "gather", "focused"],
  },
  {
    word: "documentary",
    meaning: "based on or reproducing real events or facts",
    partOfSpeech: "Adjective",
    examples: [
      "The film provided documentary evidence of the historical event.",
      "She preferred documentary photography over staged portraits.",
    ],
    synonyms: ["factual", "nonfictional", "authentic"],
    antonyms: ["fictional", "fabricated", "staged"],
  },
  {
    word: "exhaustive",
    meaning: "thorough and comprehensive",
    partOfSpeech: "Adjective",
    examples: [
      "The police conducted an exhaustive search of the premises.",
      "Her thesis included an exhaustive review of the literature.",
    ],
    synonyms: ["comprehensive", "thorough", "complete"],
    antonyms: ["superficial", "incomplete", "partial"],
  },
  {
    word: "exhilarating",
    meaning: "intensely exciting",
    partOfSpeech: "Adjective",
    examples: [
      "The roller coaster ride was absolutely exhilarating.",
      "They described the safari as an exhilarating experience.",
    ],
    synonyms: ["thrilling", "electrifying", "stimulating"],
    antonyms: ["boring", "tedious", "dull"],
  },
  {
    word: "extraneous",
    meaning: "lacking relevance; unnecessary",
    partOfSpeech: "Adjective",
    examples: [
      "The editor removed all extraneous details from the article.",
      "Please stick to the topic and avoid extraneous comments.",
    ],
    synonyms: ["irrelevant", "superfluous", "unnecessary"],
    antonyms: ["relevant", "essential", "pertinent"],
  },
  {
    word: "fervor",
    meaning: "intense enthusiasm toward something",
    partOfSpeech: "Noun",
    examples: [
      "The crowd cheered with fervor as their team scored.",
      "She spoke about human rights with great fervor.",
    ],
    synonyms: ["passion", "ardor", "zeal"],
    antonyms: ["apathy", "indifference", "coolness"],
  },
  {
    word: "futile",
    meaning: "incapable of resulting in anything worthwhile",
    partOfSpeech: "Adjective",
    examples: [
      "It would be futile to resist the hurricane's force.",
      "Their attempts to revive the business proved futile.",
    ],
    synonyms: ["pointless", "hopeless", "vain"],
    antonyms: ["productive", "effective", "useful"],
  },
  {
    word: "illusory",
    meaning: "unreal or based on illusion",
    partOfSpeech: "Adjective",
    examples: [
      "The oasis turned out to be an illusory mirage.",
      "His sense of control was completely illusory.",
    ],
    synonyms: ["imaginary", "delusory", "unreal"],
    antonyms: ["real", "genuine", "substantial"],
  },
  {
    word: "invidious",
    meaning:
      "1. likely to cause resentment or envy 2. unfair; prejudiced against someone or something",
    partOfSpeech: "Adjective",
    examples: [
      "The manager faced criticism for making invidious comparisons between employees.",
      "The policy created an invidious distinction between different groups.",
    ],
    synonyms: ["discriminatory", "offensive", "unjust"],
    antonyms: ["fair", "just", "impartial"],
  },
  {
    word: "lethargic",
    meaning: "sluggish; tired",
    partOfSpeech: "Adjective",
    examples: [
      "The hot weather made everyone feel lethargic.",
      "After the large meal, he became lethargic and sleepy.",
    ],
    synonyms: ["sluggish", "listless", "torpid"],
    antonyms: ["energetic", "vigorous", "animated"],
  },
  {
    word: "metaphorical",
    meaning: "symbolic rather than literal",
    partOfSpeech: "Adjective",
    examples: [
      "When she spoke of 'lighting the fire of revolution,' she was being metaphorical.",
      "The metaphorical language in the poem created vivid imagery.",
    ],
    synonyms: ["figurative", "symbolic", "nonliteral"],
    antonyms: ["literal", "actual", "real"],
  },
  {
    word: "mimic",
    meaning: "to imitate or copy someone or something",
    partOfSpeech: "Verb",
    examples: [
      "The comedian could perfectly mimic the president's speech patterns.",
      "Some animals mimic their surroundings as camouflage.",
    ],
    synonyms: ["imitate", "copy", "emulate"],
    antonyms: ["originate", "create", "innovate"],
  },
  {
    word: "numinous",
    meaning: "having a spiritual or divine quality",
    partOfSpeech: "Adjective",
    examples: [
      "The ancient temple had a numinous atmosphere that inspired awe.",
      "She described her mystical experience as numinous and ineffable.",
    ],
    synonyms: ["spiritual", "divine", "transcendent"],
    antonyms: ["mundane", "earthly", "profane"],
  },
  {
    word: "obscure",
    meaning:
      "1. not clearly expressed or understood 2. making something unclear and vague",
    partOfSpeech: "Adjective/Verb",
    examples: [
      "The poem's meaning remained obscure despite careful study.",
      "The politician tried to obscure his involvement in the scandal.",
    ],
    synonyms: ["unclear", "ambiguous", "conceal"],
    antonyms: ["clear", "lucid", "reveal"],
  },
  {
    word: "overt",
    meaning: "open and observable; not hidden or secret",
    partOfSpeech: "Adjective",
    examples: [
      "Their overt hostility made everyone uncomfortable.",
      "The company made an overt attempt to improve its public image.",
    ],
    synonyms: ["open", "obvious", "undisguised"],
    antonyms: ["covert", "hidden", "secret"],
  },
  {
    word: "pellucid",
    meaning:
      "1. transparently clear in meaning or style 2. (of a body of water or substance) transparent and clear",
    partOfSpeech: "Adjective",
    examples: [
      "Her pellucid explanation made the complex topic easy to understand.",
      "We could see fish swimming in the pellucid waters of the Caribbean.",
    ],
    synonyms: ["clear", "lucid", "transparent"],
    antonyms: ["opaque", "unclear", "cloudy"],
  },
  {
    word: "perpetuate",
    meaning: "make (something) continue for an indefinite time",
    partOfSpeech: "Verb",
    examples: [
      "The myth was perpetuated through generations of storytelling.",
      "We must not perpetuate harmful stereotypes in our media.",
    ],
    synonyms: ["continue", "sustain", "preserve"],
    antonyms: ["discontinue", "abolish", "terminate"],
  },
  {
    word: "rational",
    meaning: "based on clear thought or reason",
    partOfSpeech: "Adjective",
    examples: [
      "There was no rational explanation for the strange phenomenon.",
      "She remained calm and rational during the crisis.",
    ],
    synonyms: ["logical", "reasonable", "sensible"],
    antonyms: ["irrational", "illogical", "unreasonable"],
  },
  {
    word: "scathing",
    meaning: "harshly or bitterly critical; vitriolic",
    partOfSpeech: "Adjective",
    examples: [
      "The critic wrote a scathing review of the play.",
      "Her scathing remarks about his work left him devastated.",
    ],
    synonyms: ["cutting", "mordant", "caustic"],
    antonyms: ["complimentary", "praising", "flattering"],
  },
  {
    word: "subtle",
    meaning: "delicate or slight; not obvious",
    partOfSpeech: "Adjective",
    examples: [
      "There were subtle differences between the two versions of the painting.",
      "She gave him a subtle hint that she wanted to leave the party.",
    ],
    synonyms: ["nuanced", "understated", "delicate"],
    antonyms: ["obvious", "blatant", "conspicuous"],
  },
  {
    word: "superficial",
    meaning:
      "1. existing, occurring, or located on the surface 2. not profound or thorough",
    partOfSpeech: "Adjective",
    examples: [
      "The patient only suffered superficial wounds in the accident.",
      "His knowledge of the subject was quite superficial.",
    ],
    synonyms: ["surface", "shallow", "cursory"],
    antonyms: ["deep", "profound", "thorough"],
  },
];

export const GROUP_14 = [
  {
    word: "acquiesce",
    meaning: "agree or consent quietly without objection",
    partOfSpeech: "Verb",
    examples: [
      "After much debate, she finally acquiesced to their demands.",
      "The manager acquiesced to the team's proposed changes.",
    ],
    synonyms: ["consent", "comply", "concede"],
    antonyms: ["resist", "oppose", "refuse"],
  },
  {
    word: "adroit",
    meaning: "deft, dexterous, or skillful",
    partOfSpeech: "Adjective",
    examples: [
      "The adroit surgeon completed the complex procedure flawlessly.",
      "Her adroit handling of the negotiation impressed everyone.",
    ],
    synonyms: ["skillful", "nimble", "adept"],
    antonyms: ["clumsy", "awkward", "inept"],
  },
  {
    word: "amend",
    meaning:
      "make changes to a document, law, or statement, usually to improve it",
    partOfSpeech: "Verb",
    examples: [
      "The constitution was amended to guarantee equal rights.",
      "Please amend your report to include the latest data.",
    ],
    synonyms: ["revise", "modify", "correct"],
    antonyms: ["maintain", "preserve", "keep"],
  },
  {
    word: "animus",
    meaning: "strong dislike or enmity; hostile attitude",
    partOfSpeech: "Noun",
    examples: [
      "There was clear animus between the rival political candidates.",
      "His animus toward modern art was well-known.",
    ],
    synonyms: ["hostility", "antagonism", "malice"],
    antonyms: ["friendship", "amity", "goodwill"],
  },
  {
    word: "apologist",
    meaning:
      "a person who defends or supports something that is being criticized",
    partOfSpeech: "Noun",
    examples: [
      "He became an apologist for the controversial government policy.",
      "The company's apologists argued the layoffs were necessary.",
    ],
    synonyms: ["defender", "advocate", "justifier"],
    antonyms: ["critic", "detractor", "opponent"],
  },
  {
    word: "astringent",
    meaning: "sharp or harsh in demeanor or style",
    partOfSpeech: "Adjective",
    examples: [
      "Her astringent wit often made people uncomfortable.",
      "The critic's astringent review devastated the young author.",
    ],
    synonyms: ["severe", "caustic", "acerbic"],
    antonyms: ["mild", "gentle", "kind"],
  },
  {
    word: "collaborate",
    meaning:
      "1. work together on a project 2. cooperate deviously with an enemy",
    partOfSpeech: "Verb",
    examples: [
      "The scientists collaborated on groundbreaking research.",
      "During the war, some citizens collaborated with the occupying forces.",
    ],
    synonyms: ["cooperate", "team up", "conspire"],
    antonyms: ["work alone", "resist", "oppose"],
  },
  {
    word: "competent",
    meaning: "having sufficient skill, knowledge, ability, or qualifications",
    partOfSpeech: "Adjective",
    examples: [
      "The competent mechanic fixed the problem quickly.",
      "She proved herself competent to handle the demanding position.",
    ],
    synonyms: ["capable", "proficient", "qualified"],
    antonyms: ["incompetent", "unskilled", "incapable"],
  },
  {
    word: "correlate",
    meaning:
      "show a mutual relationship or connection between two or more things",
    partOfSpeech: "Verb",
    examples: [
      "The study correlated smoking with lung disease.",
      "Higher test scores correlated strongly with regular study habits.",
    ],
    synonyms: ["associate", "connect", "relate"],
    antonyms: ["disconnect", "separate", "dissociate"],
  },
  {
    word: "deride",
    meaning: "ridicule or mock",
    partOfSpeech: "Verb",
    examples: [
      "The bullies derided him for his unusual clothing.",
      "Early space exploration was derided as science fiction.",
    ],
    synonyms: ["mock", "ridicule", "taunt"],
    antonyms: ["praise", "compliment", "admire"],
  },
  {
    word: "dictate",
    meaning: "give orders or commands",
    partOfSpeech: "Verb",
    examples: [
      "The general dictated terms to the defeated army.",
      "Company policy dictates proper safety procedures.",
    ],
    synonyms: ["command", "order", "decree"],
    antonyms: ["request", "ask", "suggest"],
  },
  {
    word: "discreet",
    meaning: "respectful of privacy or secrecy; inconspicuous",
    partOfSpeech: "Adjective",
    examples: [
      "The discreet waiter never mentioned their famous guest's presence.",
      "She made discreet inquiries about the job opening.",
    ],
    synonyms: ["tactful", "unobtrusive", "prudent"],
    antonyms: ["indiscreet", "obvious", "blatant"],
  },
  {
    word: "divorced",
    meaning: "separate from something that was previously connected",
    partOfSpeech: "Adjective",
    examples: [
      "His theory was completely divorced from reality.",
      "The artist's later work became divorced from her early style.",
    ],
    synonyms: ["separated", "disconnected", "detached"],
    antonyms: ["connected", "united", "attached"],
  },
  {
    word: "elitist",
    meaning:
      "1. a person who believes in the rule of an elite or privileged group 2. believing oneself or a group to be superior to others",
    partOfSpeech: "Noun/Adjective",
    examples: [
      "The club's elitist policies excluded many qualified applicants.",
      "His elitist attitude made him unpopular with coworkers.",
    ],
    synonyms: ["snobbish", "exclusive", "aristocratic"],
    antonyms: ["egalitarian", "inclusive", "democratic"],
  },
  {
    word: "exacting",
    meaning: "requiring a lot of care, effort, or attention",
    partOfSpeech: "Adjective",
    examples: [
      "The exacting standards of the chef were legendary.",
      "Proofreading legal documents is exacting work.",
    ],
    synonyms: ["demanding", "strict", "stringent"],
    antonyms: ["lenient", "easy", "undemanding"],
  },
  {
    word: "flummoxed",
    meaning: "completely unable to understand; bewildered",
    partOfSpeech: "Adjective",
    examples: [
      "The complex instructions left me completely flummoxed.",
      "She looked flummoxed by the unexpected question.",
    ],
    synonyms: ["baffled", "perplexed", "confounded"],
    antonyms: ["understanding", "clear", "enlightened"],
  },
  {
    word: "fruitful",
    meaning: "producing good or abundant results",
    partOfSpeech: "Adjective",
    examples: [
      "Their collaboration proved to be very fruitful.",
      "The investigation yielded fruitful leads.",
    ],
    synonyms: ["productive", "successful", "profitable"],
    antonyms: ["fruitless", "unproductive", "barren"],
  },
  {
    word: "inborn",
    meaning: "present from birth; innate",
    partOfSpeech: "Adjective",
    examples: [
      "Her inborn musical talent was evident by age three.",
      "Some animals show inborn fear of predators.",
    ],
    synonyms: ["innate", "inherent", "natural"],
    antonyms: ["learned", "acquired", "developed"],
  },
  {
    word: "polymath",
    meaning: "a person with extraordinarily broad and comprehensive knowledge",
    partOfSpeech: "Noun",
    examples: [
      "Leonardo da Vinci was the quintessential Renaissance polymath.",
      "The professor was a true polymath, lecturing on everything from physics to poetry.",
    ],
    synonyms: ["genius", "renaissance person", "erudite"],
    antonyms: ["specialist", "novice", "ignoramus"],
  },
  {
    word: "reticent",
    meaning:
      "keeping one's thoughts and opinions to oneself; reserved or restrained",
    partOfSpeech: "Adjective",
    examples: [
      "He was reticent about his wartime experiences.",
      "The normally reticent scientist surprised everyone by speaking passionately.",
    ],
    synonyms: ["reserved", "taciturn", "uncommunicative"],
    antonyms: ["talkative", "outgoing", "expressive"],
  },
  {
    word: "stringent",
    meaning: "strict or severe; tight",
    partOfSpeech: "Adjective",
    examples: [
      "The university has stringent admission requirements.",
      "Stringent safety measures were implemented after the accident.",
    ],
    synonyms: ["rigorous", "strict", "exacting"],
    antonyms: ["lenient", "flexible", "relaxed"],
  },
  {
    word: "subservient",
    meaning:
      "1. obedient to an authority or other people 2. less important; inferior to",
    partOfSpeech: "Adjective",
    examples: [
      "The assistant was expected to be completely subservient to the CEO.",
      "Economic concerns often become subservient to political ones.",
    ],
    synonyms: ["submissive", "obsequious", "secondary"],
    antonyms: ["dominant", "independent", "superior"],
  },
  {
    word: "surreptitious",
    meaning: "done secretly or stealthily",
    partOfSpeech: "Adjective",
    examples: [
      "They exchanged surreptitious glances during the meeting.",
      "The journalist took surreptitious photos of the facility.",
    ],
    synonyms: ["clandestine", "covert", "sneaky"],
    antonyms: ["open", "overt", "aboveboard"],
  },
  {
    word: "tantalizing",
    meaning: "arousing desire or interest without fulfillment",
    partOfSpeech: "Adjective",
    examples: [
      "The detective had only tantalizing clues about the case.",
      "The aroma from the bakery was tantalizing.",
    ],
    synonyms: ["teasing", "tempting", "alluring"],
    antonyms: ["satisfying", "fulfilling", "gratifying"],
  },
  {
    word: "tantamount",
    meaning: "equivalent in effect or value",
    partOfSpeech: "Adjective",
    examples: [
      "His silence was tantamount to admitting guilt.",
      "The CEO's approval is tantamount to a guarantee of funding.",
    ],
    synonyms: ["equivalent", "equal", "comparable"],
    antonyms: ["unequal", "inferior", "dissimilar"],
  },
  {
    word: "torpor",
    meaning: "a state of apathy or lethargy",
    partOfSpeech: "Noun",
    examples: [
      "The heat induced a pleasant torpor in the afternoon.",
      "After the crisis passed, she fell into a deep torpor.",
    ],
    synonyms: ["lethargy", "listlessness", "sluggishness"],
    antonyms: ["energy", "vigor", "animation"],
  },
  {
    word: "trenchant",
    meaning: "keen and biting; vigorously articulate",
    partOfSpeech: "Adjective",
    examples: [
      "The journalist's trenchant analysis exposed the corruption.",
      "His trenchant wit made him a feared debater.",
    ],
    synonyms: ["incisive", "penetrating", "sharp"],
    antonyms: ["vague", "dull", "weak"],
  },
  {
    word: "umbrage",
    meaning: "a feeling of anger or annoyance caused by something offensive",
    partOfSpeech: "Noun",
    examples: [
      "She took umbrage at his rude comments.",
      "Many citizens took umbrage at the new tax law.",
    ],
    synonyms: ["offense", "resentment", "pique"],
    antonyms: ["pleasure", "delight", "satisfaction"],
  },
  {
    word: "versatile",
    meaning:
      "having varied uses or many functions; capable of doing many things",
    partOfSpeech: "Adjective",
    examples: [
      "The versatile actor excelled in both comedy and drama.",
      "This versatile kitchen tool replaces a dozen others.",
    ],
    synonyms: ["adaptable", "multifaceted", "all-around"],
    antonyms: ["limited", "specialized", "inflexible"],
  },
  {
    word: "wayward",
    meaning: "obstinate, contrary and unpredictable",
    partOfSpeech: "Adjective",
    examples: [
      "The wayward child constantly tested his parents' patience.",
      "The wayward golf ball landed in the lake.",
    ],
    synonyms: ["willful", "headstrong", "erratic"],
    antonyms: ["compliant", "obedient", "predictable"],
  },
];

export const GROUP_15 = [
  {
    word: "alienate",
    meaning: "make someone feel isolated and estranged",
    partOfSpeech: "Verb",
    examples: [
      "His rude comments alienated many of his colleagues.",
      "The policy risks alienating the company's core customer base.",
    ],
    synonyms: ["estrange", "distance", "isolate"],
    antonyms: ["unite", "reconcile", "endear"],
  },
  {
    word: "apathy",
    meaning: "lack of interest or enthusiasm towards something",
    partOfSpeech: "Noun",
    examples: [
      "Voter apathy resulted in low turnout for the election.",
      "She viewed his apathy toward environmental issues as troubling.",
    ],
    synonyms: ["indifference", "unconcern", "dispassion"],
    antonyms: ["enthusiasm", "passion", "interest"],
  },
  {
    word: "apropos",
    meaning: "appropriate or relevant to the situation",
    partOfSpeech: "Adjective/Preposition",
    examples: [
      "His remarks about teamwork were apropos given the project's challenges.",
      "Apropos of your earlier question, let me clarify the policy.",
    ],
    synonyms: ["pertinent", "relevant", "applicable"],
    antonyms: ["irrelevant", "inappropriate", "unrelated"],
  },
  {
    word: "apt",
    meaning:
      "1. suitable or appropriate 2. having a tendency toward something 3. quick to learn",
    partOfSpeech: "Adjective",
    examples: [
      "The metaphor was particularly apt for describing their relationship.",
      "He's apt to forget appointments if he doesn't write them down.",
      "She's an apt student who grasps concepts quickly.",
    ],
    synonyms: ["appropriate", "likely", "clever"],
    antonyms: ["inappropriate", "unlikely", "slow"],
  },
  {
    word: "cloak",
    meaning: "conceal or disguise",
    partOfSpeech: "Verb",
    examples: [
      "The spy cloaked his activities behind a legitimate business.",
      "Their proposal cloaked significant risks in optimistic language.",
    ],
    synonyms: ["hide", "mask", "veil"],
    antonyms: ["reveal", "expose", "uncover"],
  },
  {
    word: "consensus",
    meaning: "general agreement among the members of a group",
    partOfSpeech: "Noun",
    examples: [
      "After much debate, the committee reached a consensus.",
      "There's growing consensus about the need for climate action.",
    ],
    synonyms: ["agreement", "accord", "unanimity"],
    antonyms: ["disagreement", "discord", "dissension"],
  },
  {
    word: "distort",
    meaning: "give a false or misleading account of",
    partOfSpeech: "Verb",
    examples: [
      "The media often distorts complex scientific findings.",
      "Anger distorted his perception of the situation.",
    ],
    synonyms: ["misrepresent", "twist", "falsify"],
    antonyms: ["represent accurately", "clarify", "illuminate"],
  },
  {
    word: "divergent",
    meaning: "growing further apart; disagreeing",
    partOfSpeech: "Adjective",
    examples: [
      "The siblings took divergent paths in their careers.",
      "The panel had divergent opinions on the issue.",
    ],
    synonyms: ["differing", "conflicting", "separating"],
    antonyms: ["convergent", "similar", "united"],
  },
  {
    word: "elated",
    meaning: "very happy or proud",
    partOfSpeech: "Adjective",
    examples: [
      "She was elated when she received the acceptance letter.",
      "The team felt elated after their championship victory.",
    ],
    synonyms: ["ecstatic", "overjoyed", "thrilled"],
    antonyms: ["depressed", "disappointed", "dejected"],
  },
  {
    word: "enchant",
    meaning: "capture the attention or to charm",
    partOfSpeech: "Verb",
    examples: [
      "The storyteller enchanted the children with magical tales.",
      "Visitors are enchanted by the city's old-world charm.",
    ],
    synonyms: ["captivate", "beguile", "enthrall"],
    antonyms: ["repel", "disenchant", "bore"],
  },
  {
    word: "entrenched",
    meaning: "firmly ingrained and unlikely to change",
    partOfSpeech: "Adjective",
    examples: [
      "The country faced entrenched corruption at all government levels.",
      "His views on the subject were deeply entrenched.",
    ],
    synonyms: ["established", "rooted", "fixed"],
    antonyms: ["superficial", "temporary", "changeable"],
  },
  {
    word: "exotic",
    meaning: "foreign, especially in an exciting way",
    partOfSpeech: "Adjective",
    examples: [
      "She collected exotic spices from her travels.",
      "The zoo housed many exotic animals.",
    ],
    synonyms: ["foreign", "unusual", "unfamiliar"],
    antonyms: ["native", "ordinary", "commonplace"],
  },
  {
    word: "exploitative",
    meaning: "taking unfair advantage of a person, group, or situation",
    partOfSpeech: "Adjective",
    examples: [
      "The company was criticized for its exploitative labor practices.",
      "Some reality shows are exploitative of their participants.",
    ],
    synonyms: ["manipulative", "predatory", "oppressive"],
    antonyms: ["fair", "equitable", "just"],
  },
  {
    word: "foreseeable",
    meaning: "able to be predicted or expected",
    partOfSpeech: "Adjective",
    examples: [
      "The accident was foreseeable given the dangerous conditions.",
      "In the foreseeable future, we expect continued growth.",
    ],
    synonyms: ["predictable", "anticipated", "expected"],
    antonyms: ["unforeseeable", "unexpected", "sudden"],
  },
  {
    word: "forsake",
    meaning: "abandon or give up",
    partOfSpeech: "Verb",
    examples: [
      "He forsook his artistic ambitions for a steady job.",
      "The village was forsaken after the mine closed.",
    ],
    synonyms: ["abandon", "desert", "renounce"],
    antonyms: ["keep", "hold", "maintain"],
  },
  {
    word: "gratify",
    meaning: "please or satisfy; make content",
    partOfSpeech: "Verb",
    examples: [
      "It gratified her to see her students succeed.",
      "He was gratified by the positive response to his work.",
    ],
    synonyms: ["please", "satisfy", "delight"],
    antonyms: ["displease", "dissatisfy", "frustrate"],
  },
  {
    word: "heed",
    meaning: "mind or regard with care; pay attention",
    partOfSpeech: "Verb",
    examples: [
      "They failed to heed the warning signs.",
      "I advise you to heed your doctor's recommendations.",
    ],
    synonyms: ["notice", "obey", "consider"],
    antonyms: ["ignore", "disregard", "neglect"],
  },
  {
    word: "judicious",
    meaning:
      "having, characterized by, or done with good judgment or sound thinking",
    partOfSpeech: "Adjective",
    examples: [
      "She made judicious use of her limited resources.",
      "The judge was known for his judicious rulings.",
    ],
    synonyms: ["wise", "prudent", "sensible"],
    antonyms: ["foolish", "imprudent", "unwise"],
  },
  {
    word: "lucid",
    meaning: "clear and easy to understand",
    partOfSpeech: "Adjective",
    examples: [
      "The professor gave a lucid explanation of the complex theory.",
      "Despite his fever, he had moments of lucid thought.",
    ],
    synonyms: ["clear", "understandable", "coherent"],
    antonyms: ["confusing", "unclear", "obscure"],
  },
  {
    word: "pertinent",
    meaning:
      "important with regard to (a subject or matter); pertaining; relevant",
    partOfSpeech: "Adjective",
    examples: [
      "Please keep your comments pertinent to the topic.",
      "The lawyer objected to testimony not pertinent to the case.",
    ],
    synonyms: ["relevant", "applicable", "germane"],
    antonyms: ["irrelevant", "inapplicable", "extraneous"],
  },
  {
    word: "propriety",
    meaning: "conformity to established standards of good or proper behavior",
    partOfSpeech: "Noun",
    examples: [
      "She always acted with perfect propriety at formal events.",
      "Some questioned the propriety of the mayor's business dealings.",
    ],
    synonyms: ["decorum", "correctness", "seemliness"],
    antonyms: ["impropriety", "indecorum", "rudeness"],
  },
  {
    word: "scintillating",
    meaning: "brilliantly or impressively clever, exciting, amusing or witty",
    partOfSpeech: "Adjective",
    examples: [
      "The audience enjoyed the speaker's scintillating wit.",
      "Their conversation was scintillating and wide-ranging.",
    ],
    synonyms: ["sparkling", "brilliant", "stimulating"],
    antonyms: ["dull", "boring", "uninteresting"],
  },
  {
    word: "sensational",
    meaning: "exaggerated for effect, often sacrificing accuracy",
    partOfSpeech: "Adjective",
    examples: [
      "The tabloids published sensational headlines about the celebrity.",
      "The documentary avoided sensational treatment of the tragedy.",
    ],
    synonyms: ["exaggerated", "melodramatic", "lurid"],
    antonyms: ["restrained", "accurate", "understated"],
  },
  {
    word: "sophisticated",
    meaning:
      "1. experienced, worldly, or cultured 2. advanced or intricate, often in tech",
    partOfSpeech: "Adjective",
    examples: [
      "She had sophisticated tastes in art and music.",
      "The lab uses sophisticated equipment for DNA analysis.",
    ],
    synonyms: ["cultured", "refined", "complex"],
    antonyms: ["naive", "simple", "unsophisticated"],
  },
  {
    word: "strife",
    meaning: "conflict or struggle",
    partOfSpeech: "Noun",
    examples: [
      "The period was marked by political strife and unrest.",
      "Family strife can affect children's development.",
    ],
    synonyms: ["conflict", "discord", "friction"],
    antonyms: ["peace", "harmony", "accord"],
  },
  {
    word: "understated",
    meaning: "expressed in a subtle, restrained, or low-key manner",
    partOfSpeech: "Adjective",
    examples: [
      "Her understated elegance made her stand out at the gala.",
      "The decor was understated but tasteful.",
    ],
    synonyms: ["subtle", "restrained", "modest"],
    antonyms: ["flamboyant", "ostentatious", "exaggerated"],
  },
  {
    word: "unscrupulous",
    meaning: "without moral principles",
    partOfSpeech: "Adjective",
    examples: [
      "Unscrupulous dealers often take advantage of first-time buyers.",
      "The scheme was the work of unscrupulous con artists.",
    ],
    synonyms: ["unprincipled", "dishonest", "corrupt"],
    antonyms: ["ethical", "principled", "honorable"],
  },
  {
    word: "veracity",
    meaning: "conformity to truth or accuracy",
    partOfSpeech: "Noun",
    examples: [
      "Journalists must verify the veracity of their sources.",
      "I doubt the veracity of his outlandish claims.",
    ],
    synonyms: ["truthfulness", "accuracy", "reliability"],
    antonyms: ["falsity", "deception", "dishonesty"],
  },
  {
    word: "virulent",
    meaning: "extremely hostile or malicious; intensely acrimonious",
    partOfSpeech: "Adjective",
    examples: [
      "The debate turned virulent as personal attacks began.",
      "A virulent strain of the virus spread rapidly.",
    ],
    synonyms: ["venomous", "malicious", "bitter"],
    antonyms: ["kind", "benevolent", "mild"],
  },
  {
    word: "volatile",
    meaning:
      "likely to change unpredictably and quickly (usually for the worse)",
    partOfSpeech: "Adjective",
    examples: [
      "The volatile stock market made investors nervous.",
      "Their volatile relationship alternated between passion and anger.",
    ],
    synonyms: ["unstable", "unpredictable", "explosive"],
    antonyms: ["stable", "steady", "consistent"],
  },
];

export const GROUP_16 = [
  {
    word: "antedate",
    meaning: "occur before (something) in time",
    partOfSpeech: "Verb",
    examples: [
      "The artifact was found to antedate the Egyptian pyramids by centuries.",
      "Her letter antedated his arrival, proving she knew he was coming.",
    ],
    synonyms: ["precede", "predate", "forego"],
    antonyms: ["postdate", "follow", "succeed"],
  },
  {
    word: "banish",
    meaning: "expel from a place, especially as punishment; get rid of",
    partOfSpeech: "Verb",
    examples: [
      "The king decided to banish the traitor from the kingdom.",
      "She tried to banish all negative thoughts from her mind.",
    ],
    synonyms: ["exile", "deport", "oust"],
    antonyms: ["welcome", "accept", "embrace"],
  },
  {
    word: "bridle",
    meaning: "control or restrain",
    partOfSpeech: "Verb",
    examples: [
      "He struggled to bridle his anger during the argument.",
      "The rider used the reins to bridle the spirited horse.",
    ],
    synonyms: ["curb", "check", "constrain"],
    antonyms: ["release", "unleash", "free"],
  },
  {
    word: "comply",
    meaning: "obey someone's rules or wishes; meet certain standards",
    partOfSpeech: "Verb",
    examples: [
      "All visitors must comply with the museum's photography policy.",
      "The company failed to comply with environmental regulations.",
    ],
    synonyms: ["conform", "acquiesce", "submit"],
    antonyms: ["defy", "resist", "disobey"],
  },
  {
    word: "crestfallen",
    meaning: "disappointed, dispirited, or depressed",
    partOfSpeech: "Adjective",
    examples: [
      "The team was crestfallen after losing the championship game.",
      "She looked crestfallen when her application was rejected.",
    ],
    synonyms: ["dejected", "despondent", "downcast"],
    antonyms: ["elated", "cheerful", "jubilant"],
  },
  {
    word: "curtail",
    meaning: "restrict or limit; decrease in quantity",
    partOfSpeech: "Verb",
    examples: [
      "The school had to curtail outdoor activities due to bad weather.",
      "We need to curtail our spending to save money.",
    ],
    synonyms: ["reduce", "cut back", "diminish"],
    antonyms: ["increase", "extend", "expand"],
  },
  {
    word: "elucidate",
    meaning: "make clear; clarify or shed light on",
    partOfSpeech: "Verb",
    examples: [
      "The professor used diagrams to elucidate the complex theory.",
      "Could you elucidate your point further?",
    ],
    synonyms: ["explain", "illuminate", "interpret"],
    antonyms: ["obscure", "confuse", "complicate"],
  },
  {
    word: "evade",
    meaning:
      "1. avoid or escape from, typically in a cunning way 2. deflect from giving a clear answer",
    partOfSpeech: "Verb",
    examples: [
      "The suspect managed to evade capture for weeks.",
      "The politician evaded the reporter's question skillfully.",
    ],
    synonyms: ["dodge", "elude", "sidestep"],
    antonyms: ["confront", "face", "address"],
  },
  {
    word: "feckless",
    meaning: "without skill, ineffective, incompetent",
    partOfSpeech: "Adjective",
    examples: [
      "His feckless attempts at repair only made the problem worse.",
      "The feckless manager was soon replaced.",
    ],
    synonyms: ["inept", "useless", "irresponsible"],
    antonyms: ["competent", "capable", "effective"],
  },
  {
    word: "fester",
    meaning:
      "(of a negative feeling or a problem) worsen, especially due to lack of attention",
    partOfSpeech: "Verb",
    examples: [
      "Resentment began to fester between the two former friends.",
      "The untreated wound started to fester.",
    ],
    synonyms: ["rankle", "worsen", "deteriorate"],
    antonyms: ["heal", "improve", "resolve"],
  },
  {
    word: "iconoclastic",
    meaning: "challenging or overthrowing tradition or established beliefs",
    partOfSpeech: "Adjective",
    examples: [
      "The artist's iconoclastic works shocked the conservative art world.",
      "His iconoclastic views made him unpopular with traditionalists.",
    ],
    synonyms: ["unconventional", "radical", "heretical"],
    antonyms: ["conventional", "orthodox", "traditional"],
  },
  {
    word: "immure",
    meaning: "imprison or isolate",
    partOfSpeech: "Verb",
    examples: [
      "The rebels were immured in the castle dungeon.",
      "She felt immured in her small hometown.",
    ],
    synonyms: ["confine", "incarcerate", "seclude"],
    antonyms: ["free", "liberate", "release"],
  },
  {
    word: "improvise",
    meaning: "create something spontaneously without preparation",
    partOfSpeech: "Verb",
    examples: [
      "When the speaker didn't arrive, she had to improvise a speech.",
      "The jazz band loved to improvise during performances.",
    ],
    synonyms: ["ad-lib", "extemporize", "wing it"],
    antonyms: ["prepare", "plan", "rehearse"],
  },
  {
    word: "inhibit",
    meaning: "make difficult to accomplish; act as an obstacle",
    partOfSpeech: "Verb",
    examples: [
      "Fear can inhibit people from taking risks.",
      "The tight dress inhibited her movement.",
    ],
    synonyms: ["hinder", "restrain", "hamper"],
    antonyms: ["encourage", "facilitate", "promote"],
  },
  {
    word: "inscrutable",
    meaning: "not easily understood; mysterious",
    partOfSpeech: "Adjective",
    examples: [
      "The sphinx's inscrutable expression has puzzled people for centuries.",
      "He maintained an inscrutable smile throughout the interview.",
    ],
    synonyms: ["enigmatic", "impenetrable", "unfathomable"],
    antonyms: ["transparent", "clear", "obvious"],
  },
  {
    word: "lionize",
    meaning: "treat someone as a celebrity or hero",
    partOfSpeech: "Verb",
    examples: [
      "The town began to lionize the young athlete after his Olympic victory.",
      "Modern society tends to lionize wealthy entrepreneurs.",
    ],
    synonyms: ["celebrate", "glorify", "idolize"],
    antonyms: ["vilify", "criticize", "disparage"],
  },
  {
    word: "monotonous",
    meaning: "lacking in variety, repetitively dull",
    partOfSpeech: "Adjective",
    examples: [
      "The monotonous drone of the lecture put everyone to sleep.",
      "Factory work can be monotonous and tiring.",
    ],
    synonyms: ["tedious", "repetitive", "humdrum"],
    antonyms: ["varied", "interesting", "exciting"],
  },
  {
    word: "peculiar",
    meaning: "strange or unusual; specific to a particular place or group",
    partOfSpeech: "Adjective",
    examples: [
      "There was a peculiar smell coming from the basement.",
      "This custom is peculiar to the mountain villages.",
    ],
    synonyms: ["odd", "quirky", "distinctive"],
    antonyms: ["normal", "common", "universal"],
  },
  {
    word: "premeditate",
    meaning: "consider or plan beforehand",
    partOfSpeech: "Verb",
    examples: [
      "The prosecution argued that the murder was premeditated.",
      "Her generous act appeared spontaneous, not premeditated.",
    ],
    synonyms: ["plan", "plot", "calculate"],
    antonyms: ["improvise", "spontaneize", "ad-lib"],
  },
  {
    word: "profligate",
    meaning: "inclined to waste resources or behave extravagantly",
    partOfSpeech: "Adjective",
    examples: [
      "His profligate spending left him bankrupt.",
      "The profligate use of natural resources cannot continue.",
    ],
    synonyms: ["wasteful", "extravagant", "prodigal"],
    antonyms: ["frugal", "thrifty", "economical"],
  },
  {
    word: "reconcile",
    meaning: "bring back or return to harmony; restore a relationship",
    partOfSpeech: "Verb",
    examples: [
      "After years of feuding, the brothers finally reconciled.",
      "It's difficult to reconcile his actions with his words.",
    ],
    synonyms: ["harmonize", "settle", "mend"],
    antonyms: ["alienate", "estrange", "separate"],
  },
  {
    word: "refine",
    meaning:
      "1. purify; bring to a pure state 2. improve in accuracy or excellence",
    partOfSpeech: "Verb",
    examples: [
      "Crude oil is refined to produce gasoline.",
      "She refined her argument after considering the feedback.",
    ],
    synonyms: ["purify", "improve", "perfect"],
    antonyms: ["coarsen", "degrade", "adulterate"],
  },
  {
    word: "relinquish",
    meaning: "give up or release a claim, right, or possession",
    partOfSpeech: "Verb",
    examples: [
      "He reluctantly relinquished control of the company.",
      "The soldier refused to relinquish his post.",
    ],
    synonyms: ["surrender", "yield", "abandon"],
    antonyms: ["retain", "keep", "maintain"],
  },
  {
    word: "ruminate",
    meaning: "meditate or reflect; ponder over",
    partOfSpeech: "Verb",
    examples: [
      "She needed time to ruminate on the job offer.",
      "Cows ruminate their food as well as their thoughts.",
    ],
    synonyms: ["contemplate", "muse", "cogitate"],
    antonyms: ["ignore", "disregard", "dismiss"],
  },
  {
    word: "skittish",
    meaning: "easily scared or startled; timid; difficult to control",
    partOfSpeech: "Adjective",
    examples: [
      "The skittish horse bolted at the sound of fireworks.",
      "Investors have been skittish about the volatile market.",
    ],
    synonyms: ["nervous", "jumpy", "fidgety"],
    antonyms: ["calm", "composed", "steady"],
  },
  {
    word: "superfluous",
    meaning: "more than is necessary; excessive",
    partOfSpeech: "Adjective",
    examples: [
      "The report was full of superfluous details that obscured the main points.",
      "After packing, she removed several superfluous items from her suitcase.",
    ],
    synonyms: ["redundant", "unnecessary", "excess"],
    antonyms: ["essential", "necessary", "vital"],
  },
  {
    word: "synoptic",
    meaning: "providing a general overview or summary",
    partOfSpeech: "Adjective",
    examples: [
      "The professor began with a synoptic view of the entire field.",
      "The report gives a synoptic account of the year's events.",
    ],
    synonyms: ["summary", "abridged", "concise"],
    antonyms: ["detailed", "comprehensive", "exhaustive"],
  },
  {
    word: "thorough",
    meaning: "careful and complete concerning every detail",
    partOfSpeech: "Adjective",
    examples: [
      "She gave the document a thorough review before approving it.",
      "The police conducted a thorough investigation.",
    ],
    synonyms: ["exhaustive", "meticulous", "comprehensive"],
    antonyms: ["superficial", "cursory", "partial"],
  },
  {
    word: "visionary",
    meaning: "showing foresight or imagination",
    partOfSpeech: "Adjective",
    examples: [
      "The CEO's visionary leadership transformed the company.",
      "Many of da Vinci's visionary inventions were ahead of their time.",
    ],
    synonyms: ["innovative", "forward-thinking", "imaginative"],
    antonyms: ["unimaginative", "short-sighted", "practical"],
  },
  {
    word: "vociferous",
    meaning: "expressing opinions or feelings loudly and forcefully",
    partOfSpeech: "Adjective",
    examples: [
      "The protestors were vociferous in their demands.",
      "A vociferous minority opposed the new policy.",
    ],
    synonyms: ["vehement", "outspoken", "clamorous"],
    antonyms: ["quiet", "reticent", "subdued"],
  },
];

export const GROUP_17 = [
  {
    word: "acclaim",
    meaning: "1. praise with great approval 2. enthusiastic approval",
    partOfSpeech: "Noun/Verb",
    examples: [
      "The novel received critical acclaim from reviewers.",
      "The audience acclaimed the performance with a standing ovation.",
    ],
    synonyms: ["praise", "applause", "commendation"],
    antonyms: ["criticism", "condemnation", "disapproval"],
  },
  {
    word: "ascertain",
    meaning: "find out or learn with certainty",
    partOfSpeech: "Verb",
    examples: [
      "Detectives worked to ascertain the cause of death.",
      "We need to ascertain whether these reports are accurate.",
    ],
    synonyms: ["determine", "establish", "verify"],
    antonyms: ["guess", "assume", "speculate"],
  },
  {
    word: "assertive",
    meaning:
      "confident and direct in claiming one's rights or expressing ideas",
    partOfSpeech: "Adjective",
    examples: [
      "She became more assertive after the leadership training.",
      "His assertive manner sometimes came across as aggressive.",
    ],
    synonyms: ["confident", "forceful", "self-assured"],
    antonyms: ["timid", "submissive", "retiring"],
  },
  {
    word: "bogus",
    meaning: "fake or fraudulent",
    partOfSpeech: "Adjective",
    examples: [
      "The police discovered a stack of bogus hundred-dollar bills.",
      "His excuse sounded completely bogus.",
    ],
    synonyms: ["counterfeit", "spurious", "sham"],
    antonyms: ["genuine", "authentic", "real"],
  },
  {
    word: "cataclysmic",
    meaning: "causing great disruption or upheaval",
    partOfSpeech: "Adjective",
    examples: [
      "The asteroid impact had cataclysmic effects on the dinosaurs.",
      "The stock market crash was a cataclysmic event for investors.",
    ],
    synonyms: ["disastrous", "devastating", "apocalyptic"],
    antonyms: ["benign", "harmless", "inconsequential"],
  },
  {
    word: "circumscribe",
    meaning: "limit or restrict something narrowly",
    partOfSpeech: "Verb",
    examples: [
      "The new laws circumscribe the powers of the president.",
      "Their activities were circumscribed by the small budget.",
    ],
    synonyms: ["restrict", "limit", "confine"],
    antonyms: ["expand", "extend", "liberate"],
  },
  {
    word: "complementary",
    meaning: "enhancing or perfecting when combined with something else",
    partOfSpeech: "Adjective",
    examples: [
      "The two theories are complementary rather than contradictory.",
      "The wine was chosen as a complementary pairing for the meal.",
    ],
    synonyms: ["harmonious", "matching", "supportive"],
    antonyms: ["contrary", "opposing", "clashing"],
  },
  {
    word: "contentious",
    meaning: "causing or likely to provoke disagreement",
    partOfSpeech: "Adjective",
    examples: [
      "The contentious debate lasted late into the night.",
      "Property boundaries are often contentious issues between neighbors.",
    ],
    synonyms: ["controversial", "disputable", "argumentative"],
    antonyms: ["agreeable", "harmonious", "uncontroversial"],
  },
  {
    word: "disingenuous",
    meaning: "pretending to be sincere or straightforward",
    partOfSpeech: "Adjective",
    examples: [
      "Her apology seemed disingenuous and insincere.",
      "It was disingenuous of him to claim he didn't know about the policy.",
    ],
    synonyms: ["insincere", "deceptive", "duplicitous"],
    antonyms: ["genuine", "sincere", "honest"],
  },
  {
    word: "divulge",
    meaning: "disclose or reveal information",
    partOfSpeech: "Verb",
    examples: [
      "She refused to divulge the source of her information.",
      "The documents divulge shocking details about the incident.",
    ],
    synonyms: ["disclose", "reveal", "impart"],
    antonyms: ["conceal", "hide", "suppress"],
  },
  {
    word: "dogmatic",
    meaning: "insisting on one's ideas without being open to others'",
    partOfSpeech: "Adjective",
    examples: [
      "His dogmatic approach left no room for discussion.",
      "The professor was known for his dogmatic teaching style.",
    ],
    synonyms: ["doctrinaire", "opinionated", "inflexible"],
    antonyms: ["open-minded", "flexible", "receptive"],
  },
  {
    word: "fallacious",
    meaning: "false or mistaken",
    partOfSpeech: "Adjective",
    examples: [
      "His argument was based on fallacious reasoning.",
      "The advertisement made several fallacious claims.",
    ],
    synonyms: ["erroneous", "incorrect", "misleading"],
    antonyms: ["valid", "sound", "accurate"],
  },
  {
    word: "foolhardy",
    meaning: "recklessly daring; careless",
    partOfSpeech: "Adjective",
    examples: [
      "It would be foolhardy to attempt the climb without proper equipment.",
      "His foolhardy decision cost the company millions.",
    ],
    synonyms: ["rash", "imprudent", "reckless"],
    antonyms: ["cautious", "prudent", "sensible"],
  },
  {
    word: "hinder",
    meaning: "obstruct or delay progress",
    partOfSpeech: "Verb",
    examples: [
      "Bad weather hindered the rescue efforts.",
      "Lack of funding is hindering the research project.",
    ],
    synonyms: ["impede", "hamper", "obstruct"],
    antonyms: ["help", "assist", "facilitate"],
  },
  {
    word: "impair",
    meaning: "weaken or affect negatively",
    partOfSpeech: "Verb",
    examples: [
      "Alcohol can severely impair your judgment.",
      "The injury impaired his ability to walk.",
    ],
    synonyms: ["weaken", "diminish", "damage"],
    antonyms: ["improve", "enhance", "strengthen"],
  },
  {
    word: "impugn",
    meaning: "challenge the truth or honesty of something",
    partOfSpeech: "Verb",
    examples: [
      "The lawyer tried to impugn the witness's credibility.",
      "I don't mean to impugn your motives, but your actions seem suspicious.",
    ],
    synonyms: ["challenge", "question", "dispute"],
    antonyms: ["support", "confirm", "validate"],
  },
  {
    word: "incessant",
    meaning: "constant or unending (of something bad or unpleasant)",
    partOfSpeech: "Adjective",
    examples: [
      "The incessant noise from the construction site drove us crazy.",
      "She grew tired of his incessant complaining.",
    ],
    synonyms: ["ceaseless", "unrelenting", "persistent"],
    antonyms: ["intermittent", "occasional", "sporadic"],
  },
  {
    word: "inclined",
    meaning: "feeling a tendency or preference towards",
    partOfSpeech: "Adjective",
    examples: [
      "I'm inclined to believe his version of events.",
      "She's always been mathematically inclined.",
    ],
    synonyms: ["disposed", "prone", "leaning"],
    antonyms: ["disinclined", "unwilling", "averse"],
  },
  {
    word: "inveterate",
    meaning: "deep-rooted or long-established",
    partOfSpeech: "Adjective",
    examples: [
      "He's an inveterate liar who can't be trusted.",
      "Her inveterate optimism helped her through difficult times.",
    ],
    synonyms: ["habitual", "chronic", "confirmed"],
    antonyms: ["occasional", "new", "unaccustomed"],
  },
  {
    word: "miserly",
    meaning: "reluctant to spend or share; greedy",
    partOfSpeech: "Adjective",
    examples: [
      "The miserly old man refused to turn on the heat.",
      "The company's miserly benefits package disappointed employees.",
    ],
    synonyms: ["stingy", "penny-pinching", "tightfisted"],
    antonyms: ["generous", "liberal", "munificent"],
  },
  {
    word: "patent",
    meaning: "clear or obvious",
    partOfSpeech: "Adjective",
    examples: [
      "It was patent to everyone that she was lying.",
      "The solution to the problem was now patent.",
    ],
    synonyms: ["evident", "manifest", "apparent"],
    antonyms: ["obscure", "hidden", "unclear"],
  },
  {
    word: "petulant",
    meaning: "irritable or impatient, especially over minor issues",
    partOfSpeech: "Adjective",
    examples: [
      "The child's petulant behavior annoyed the other passengers.",
      "He gave a petulant response to the reasonable request.",
    ],
    synonyms: ["peevish", "fretful", "irritable"],
    antonyms: ["good-natured", "cheerful", "patient"],
  },
  {
    word: "pithy",
    meaning: "expressing ideas in a concise, meaningful manner",
    partOfSpeech: "Adjective",
    examples: [
      "The professor was known for his pithy remarks.",
      "She summarized the complex issue in a few pithy sentences.",
    ],
    synonyms: ["succinct", "terse", "concise"],
    antonyms: ["verbose", "wordy", "rambling"],
  },
  {
    word: "pliant",
    meaning: "flexible or easily influenced",
    partOfSpeech: "Adjective",
    examples: [
      "The young branches were pliant and easy to bend.",
      "He was too pliant and easily swayed by others' opinions.",
    ],
    synonyms: ["malleable", "adaptable", "yielding"],
    antonyms: ["rigid", "inflexible", "unyielding"],
  },
  {
    word: "sanctimonious",
    meaning: "giving the impression that one is morally superior",
    partOfSpeech: "Adjective",
    examples: [
      "His sanctimonious lectures about charity annoyed everyone.",
      "She dismissed their concerns with a sanctimonious smile.",
    ],
    synonyms: ["self-righteous", "holier-than-thou", "pious"],
    antonyms: ["humble", "modest", "unpretentious"],
  },
  {
    word: "sound",
    meaning: "based on reason, sense, or judgment",
    partOfSpeech: "Adjective",
    examples: [
      "The committee made a sound decision based on the evidence.",
      "His advice was always sound and practical.",
    ],
    synonyms: ["valid", "reasonable", "reliable"],
    antonyms: ["unsound", "flawed", "questionable"],
  },
  {
    word: "tarnish",
    meaning: "compromise, damage, or sully",
    partOfSpeech: "Verb",
    examples: [
      "The scandal tarnished the politician's reputation.",
      "Silver tarnishes when exposed to air.",
    ],
    synonyms: ["sully", "stain", "blemish"],
    antonyms: ["enhance", "polish", "honor"],
  },
  {
    word: "tepid",
    meaning: "exhibiting little passion or eagerness; lukewarm",
    partOfSpeech: "Adjective",
    examples: [
      "The audience gave the performance only tepid applause.",
      "Her tepid response suggested she wasn't very interested.",
    ],
    synonyms: ["lukewarm", "halfhearted", "unenthusiastic"],
    antonyms: ["enthusiastic", "passionate", "ardent"],
  },
  {
    word: "upbraid",
    meaning: "criticize or scold severely",
    partOfSpeech: "Verb",
    examples: [
      "The coach upbraided the team for their poor performance.",
      "She upbraided him for his irresponsible behavior.",
    ],
    synonyms: ["reprimand", "berate", "rebuke"],
    antonyms: ["praise", "commend", "compliment"],
  },
  {
    word: "vexation",
    meaning: "the state of being irritated or incensed",
    partOfSpeech: "Noun",
    examples: [
      "To his great vexation, the package still hadn't arrived.",
      "The constant delays were a source of vexation for commuters.",
    ],
    synonyms: ["annoyance", "irritation", "frustration"],
    antonyms: ["pleasure", "delight", "satisfaction"],
  },
];

export const GROUP_18 = [
  {
    word: "abet",
    meaning: "assist or encourage, usually in wrongdoing",
    partOfSpeech: "Verb",
    examples: [
      "He was accused of abetting the thief by providing an alibi.",
      "By remaining silent, she abetted the bullying behavior.",
    ],
    synonyms: ["aid", "assist", "encourage"],
    antonyms: ["hinder", "discourage", "deter"],
  },
  {
    word: "accessible",
    meaning: "clear; capable of being understood",
    partOfSpeech: "Adjective",
    examples: [
      "The professor made complex concepts accessible to all students.",
      "The museum is fully accessible to wheelchair users.",
    ],
    synonyms: ["understandable", "approachable", "available"],
    antonyms: ["inaccessible", "unclear", "obscure"],
  },
  {
    word: "acquisitive",
    meaning: "eager to acquire and possess things",
    partOfSpeech: "Adjective",
    examples: [
      "His acquisitive nature led to a house full of collectibles.",
      "The company became increasingly acquisitive, buying smaller competitors.",
    ],
    synonyms: ["greedy", "grasping", "covetous"],
    antonyms: ["generous", "altruistic", "unmaterialistic"],
  },
  {
    word: "amalgamate",
    meaning: "merge or combine two or more things",
    partOfSpeech: "Verb",
    examples: [
      "The two companies decided to amalgamate their operations.",
      "Different cultural traditions amalgamated to form a unique festival.",
    ],
    synonyms: ["combine", "unite", "blend"],
    antonyms: ["separate", "divide", "split"],
  },
  {
    word: "attenuate",
    meaning: "weaken or lessen in force or thickness",
    partOfSpeech: "Verb",
    examples: [
      "The medication helps attenuate the symptoms.",
      "The signal attenuates as it travels through the cable.",
    ],
    synonyms: ["weaken", "reduce", "diminish"],
    antonyms: ["strengthen", "intensify", "amplify"],
  },
  {
    word: "augment",
    meaning: "increase or make larger",
    partOfSpeech: "Verb",
    examples: [
      "She took a second job to augment her income.",
      "The chef augmented the sauce with fresh herbs.",
    ],
    synonyms: ["increase", "expand", "enhance"],
    antonyms: ["decrease", "reduce", "diminish"],
  },
  {
    word: "aversion",
    meaning: "an intense or definite dislike",
    partOfSpeech: "Noun",
    examples: [
      "He has a strong aversion to public speaking.",
      "Her aversion to risk prevented her from investing.",
    ],
    synonyms: ["dislike", "distaste", "repugnance"],
    antonyms: ["affinity", "liking", "fondness"],
  },
  {
    word: "blithe",
    meaning: "showing indifference",
    partOfSpeech: "Adjective",
    examples: [
      "His blithe disregard for the rules got him into trouble.",
      "She continued with blithe unconcern despite the warnings.",
    ],
    synonyms: ["heedless", "carefree", "nonchalant"],
    antonyms: ["concerned", "anxious", "careful"],
  },
  {
    word: "contempt",
    meaning:
      "the feeling or attitude of regarding someone or something as inferior",
    partOfSpeech: "Noun",
    examples: [
      "She looked at the thief with undisguised contempt.",
      "His contempt for authority figures was evident.",
    ],
    synonyms: ["scorn", "disdain", "disrespect"],
    antonyms: ["respect", "admiration", "esteem"],
  },
  {
    word: "dawdle",
    meaning: "move slowly and without purpose",
    partOfSpeech: "Verb",
    examples: [
      "Stop dawdling or we'll be late for school!",
      "He dawdled along the path, enjoying the sunshine.",
    ],
    synonyms: ["linger", "loiter", "dally"],
    antonyms: ["hurry", "rush", "speed"],
  },
  {
    word: "deflect",
    meaning:
      "change the direction or course of something; make someone deviate",
    partOfSpeech: "Verb",
    examples: [
      "The shield deflected the arrow from its target.",
      "She skillfully deflected questions about her personal life.",
    ],
    synonyms: ["divert", "redirect", "avert"],
    antonyms: ["accept", "confront", "receive"],
  },
  {
    word: "discount",
    meaning: "regard something as being unimportant or incorrect",
    partOfSpeech: "Verb",
    examples: [
      "We shouldn't discount the possibility of rain.",
      "He discounted all criticism of his methods.",
    ],
    synonyms: ["disregard", "dismiss", "ignore"],
    antonyms: ["consider", "accept", "believe"],
  },
  {
    word: "dissident",
    meaning: "one who disagrees or opposes authority",
    partOfSpeech: "Noun",
    examples: [
      "Political dissidents were often imprisoned.",
      "The dissident scientist challenged the established theory.",
    ],
    synonyms: ["protester", "rebel", "nonconformist"],
    antonyms: ["conformist", "supporter", "loyalist"],
  },
  {
    word: "efficacious",
    meaning: "producing the desired effect; successful",
    partOfSpeech: "Adjective",
    examples: [
      "The treatment proved efficacious in most cases.",
      "Her efficacious leadership turned the company around.",
    ],
    synonyms: ["effective", "productive", "potent"],
    antonyms: ["ineffective", "useless", "futile"],
  },
  {
    word: "equitable",
    meaning: "characterized by fairness and justice",
    partOfSpeech: "Adjective",
    examples: [
      "The judge made an equitable division of the estate.",
      "We need an equitable solution that benefits everyone.",
    ],
    synonyms: ["fair", "just", "impartial"],
    antonyms: ["unfair", "biased", "unjust"],
  },
  {
    word: "erratic",
    meaning: "unpredictable and inconsistent",
    partOfSpeech: "Adjective",
    examples: [
      "His erratic behavior worried his friends.",
      "The team's performance has been erratic this season.",
    ],
    synonyms: ["unpredictable", "inconsistent", "capricious"],
    antonyms: ["consistent", "steady", "predictable"],
  },
  {
    word: "industrious",
    meaning: "hard-working and persistent",
    partOfSpeech: "Adjective",
    examples: [
      "The industrious student always completed assignments early.",
      "Ants are remarkably industrious creatures.",
    ],
    synonyms: ["diligent", "assiduous", "hardworking"],
    antonyms: ["lazy", "indolent", "slothful"],
  },
  {
    word: "inform",
    meaning: "give form or character to something or someone",
    partOfSpeech: "Verb",
    examples: [
      "His experiences in the war informed his later writing.",
      "These principles inform all our business decisions.",
    ],
    synonyms: ["shape", "influence", "inspire"],
    antonyms: ["mislead", "deceive", "distort"],
  },
  {
    word: "irksome",
    meaning: "causing annoyance or irritation",
    partOfSpeech: "Adjective",
    examples: [
      "The constant buzzing of the fly was irksome.",
      "He found the paperwork requirements irksome and unnecessary.",
    ],
    synonyms: ["annoying", "vexing", "bothersome"],
    antonyms: ["pleasing", "delightful", "agreeable"],
  },
  {
    word: "manacle",
    meaning: "(used figuratively) restrain, limit, or secure something",
    partOfSpeech: "Verb",
    examples: [
      "She felt manacled by her family's expectations.",
      "The strict regulations manacled the company's creativity.",
    ],
    synonyms: ["shackle", "restrain", "confine"],
    antonyms: ["liberate", "free", "release"],
  },
  {
    word: "modest",
    meaning:
      "1. unassuming about one's abilities or not boastful 2. limited in quantity or amount",
    partOfSpeech: "Adjective",
    examples: [
      "Despite her fame, she remained remarkably modest.",
      "They live in a modest house in the suburbs.",
    ],
    synonyms: ["humble", "unpretentious", "moderate"],
    antonyms: ["boastful", "arrogant", "extravagant"],
  },
  {
    word: "noxious",
    meaning: "harmful; injurious",
    partOfSpeech: "Adjective",
    examples: [
      "The factory released noxious fumes into the air.",
      "She was exposed to noxious chemicals at work.",
    ],
    synonyms: ["toxic", "poisonous", "harmful"],
    antonyms: ["harmless", "beneficial", "nontoxic"],
  },
  {
    word: "pernicious",
    meaning: "causing harm in a subtle way; deadly",
    partOfSpeech: "Adjective",
    examples: [
      "Racism is a pernicious social problem.",
      "The drug has pernicious side effects.",
    ],
    synonyms: ["destructive", "damaging", "malignant"],
    antonyms: ["beneficial", "harmless", "innocuous"],
  },
  {
    word: "predicament",
    meaning: "a challenging or awkward situation",
    partOfSpeech: "Noun",
    examples: [
      "The lost hikers found themselves in a serious predicament.",
      "How will you get out of this predicament?",
    ],
    synonyms: ["dilemma", "quandary", "plight"],
    antonyms: ["solution", "advantage", "boon"],
  },
  {
    word: "proficient",
    meaning: "good at something; skilled",
    partOfSpeech: "Adjective",
    examples: [
      "She became proficient in three languages.",
      "The training program made him proficient in the software.",
    ],
    synonyms: ["skilled", "competent", "adept"],
    antonyms: ["incompetent", "unskilled", "inept"],
  },
  {
    word: "prolix",
    meaning: "using an excessive amount of words; long-winded",
    partOfSpeech: "Adjective",
    examples: [
      "The prolix speaker bored the audience.",
      "His prolix writing style needed serious editing.",
    ],
    synonyms: ["verbose", "wordy", "rambling"],
    antonyms: ["concise", "succinct", "terse"],
  },
  {
    word: "scorn",
    meaning:
      "1. express strong disapproval or contempt for 2. a feeling or display of contempt or disdain",
    partOfSpeech: "Verb/Noun",
    examples: [
      "She scorned his attempts at reconciliation.",
      "His work was met with scorn by critics.",
    ],
    synonyms: ["contempt", "disdain", "derision"],
    antonyms: ["admiration", "respect", "praise"],
  },
  {
    word: "subordinate",
    meaning:
      "1. having a lower rank; less important 2. treat as of less value or importance 3. a person who is under or less important than another",
    partOfSpeech: "Adjective/Verb/Noun",
    examples: [
      "She held a subordinate position in the company.",
      "He subordinated his personal needs to the team's goals.",
      "The manager spoke with her subordinate about the project.",
    ],
    synonyms: ["inferior", "secondary", "underling"],
    antonyms: ["superior", "primary", "boss"],
  },
  {
    word: "unseemly",
    meaning: "not proper or in good taste",
    partOfSpeech: "Adjective",
    examples: [
      "Their unseemly behavior embarrassed the hosts.",
      "It would be unseemly to discuss money at a funeral.",
    ],
    synonyms: ["improper", "indecorous", "inappropriate"],
    antonyms: ["proper", "decorous", "appropriate"],
  },
  {
    word: "veritable",
    meaning: "(used to emphasize something) true; genuine",
    partOfSpeech: "Adjective",
    examples: [
      "The garden was a veritable paradise of flowers.",
      "He's a veritable encyclopedia of baseball statistics.",
    ],
    synonyms: ["genuine", "real", "authentic"],
    antonyms: ["false", "fake", "bogus"],
  },
];

export const GROUP_19 = [
  {
    word: "acolyte",
    meaning: "a person who assists or follows another person or cause",
    partOfSpeech: "Noun",
    examples: [
      "The famous philosopher had many acolytes who spread his teachings.",
      "As the CEO's acolyte, she learned the business from the ground up.",
    ],
    synonyms: ["follower", "disciple", "apprentice"],
    antonyms: ["leader", "opponent", "critic"],
  },
  {
    word: "anoint",
    meaning: "appoint or designate for a particular role or purpose",
    partOfSpeech: "Verb",
    examples: [
      "The board anointed her as the company's next CEO.",
      "The media anointed the young athlete as the next superstar.",
    ],
    synonyms: ["appoint", "designate", "choose"],
    antonyms: ["dethrone", "oust", "remove"],
  },
  {
    word: "base",
    meaning: "lacking principles; morally wrong",
    partOfSpeech: "Adjective",
    examples: [
      "His base motives were revealed when he betrayed his friends.",
      "The criminal's base actions shocked the community.",
    ],
    synonyms: ["immoral", "dishonorable", "vile"],
    antonyms: ["noble", "virtuous", "honorable"],
  },
  {
    word: "coercion",
    meaning:
      "the act of persuading or forcing someone to do something using threats or violence",
    partOfSpeech: "Noun",
    examples: [
      "The confession was obtained through coercion and was therefore inadmissible.",
      "No agreement is valid if signed under coercion.",
    ],
    synonyms: ["force", "compulsion", "duress"],
    antonyms: ["persuasion", "volition", "choice"],
  },
  {
    word: "coin",
    meaning: "invent or devise a new word or phrase",
    partOfSpeech: "Verb",
    examples: [
      "Shakespeare coined many words that we still use today.",
      "The marketing team coined a catchy slogan for the new product.",
    ],
    synonyms: ["create", "invent", "originate"],
    antonyms: ["copy", "repeat", "reuse"],
  },
  {
    word: "cunning",
    meaning: "showing skill in achieving one's ends through deceit or evasion",
    partOfSpeech: "Adjective",
    examples: [
      "The cunning fox outsmarted the hunters.",
      "She used cunning tactics to gain an advantage in the negotiations.",
    ],
    synonyms: ["crafty", "sly", "wily"],
    antonyms: ["naive", "guileless", "ingenuous"],
  },
  {
    word: "discomfit",
    meaning: "make someone feel uneasy or embarrassed",
    partOfSpeech: "Verb",
    examples: [
      "The unexpected question discomfited the speaker.",
      "His direct gaze discomfited those who weren't being truthful.",
    ],
    synonyms: ["embarrass", "fluster", "disconcert"],
    antonyms: ["comfort", "put at ease", "reassure"],
  },
  {
    word: "dissent",
    meaning:
      "1. hold or express an opinion that varies from a prevailing or official position 2. difference of opinion or disagreement",
    partOfSpeech: "Verb/Noun",
    examples: [
      "Several justices dissented from the court's majority opinion.",
      "There was considerable dissent among committee members about the proposal.",
    ],
    synonyms: ["disagree", "object", "protest"],
    antonyms: ["agree", "concur", "consent"],
  },
  {
    word: "distill",
    meaning: "extract the most important information from something",
    partOfSpeech: "Verb",
    examples: [
      "The report distills years of research into key findings.",
      "Her wisdom was distilled from decades of experience.",
    ],
    synonyms: ["condense", "concentrate", "extract"],
    antonyms: ["expand", "elaborate", "dilute"],
  },
  {
    word: "dubious",
    meaning:
      "1. (of a person's feeling) suspicious of 2. (of a thing or concept) not to be relied upon",
    partOfSpeech: "Adjective",
    examples: [
      "I'm dubious about his claims of innocence.",
      "The company used dubious accounting practices to inflate profits.",
    ],
    synonyms: ["doubtful", "questionable", "skeptical"],
    antonyms: ["certain", "trustworthy", "reliable"],
  },
  {
    word: "ebullient",
    meaning: "cheerful and joyful",
    partOfSpeech: "Adjective",
    examples: [
      "Her ebullient personality made her popular at parties.",
      "The team was ebullient after their championship win.",
    ],
    synonyms: ["exuberant", "enthusiastic", "vivacious"],
    antonyms: ["gloomy", "depressed", "apathetic"],
  },
  {
    word: "facetious",
    meaning: "treating serious issues with inappropriate humor",
    partOfSpeech: "Adjective",
    examples: [
      "His facetious remarks during the meeting were poorly timed.",
      "Don't be facetious - this is a serious matter.",
    ],
    synonyms: ["flippant", "frivolous", "joking"],
    antonyms: ["serious", "earnest", "solemn"],
  },
  {
    word: "fallible",
    meaning: "capable of making an error",
    partOfSpeech: "Adjective",
    examples: [
      "Even experts are fallible and can make mistakes.",
      "The system is fallible because it depends on human input.",
    ],
    synonyms: ["imperfect", "error-prone", "faulty"],
    antonyms: ["infallible", "perfect", "flawless"],
  },
  {
    word: "florid",
    meaning: "excessively complicated; including too many details",
    partOfSpeech: "Adjective",
    examples: [
      "His florid writing style made the text difficult to follow.",
      "The florid decorations overwhelmed the small room.",
    ],
    synonyms: ["ornate", "elaborate", "baroque"],
    antonyms: ["plain", "simple", "austere"],
  },
  {
    word: "gawky",
    meaning: "awkward or clumsy",
    partOfSpeech: "Adjective",
    examples: [
      "The gawky teenager was still growing into his body.",
      "Her gawky movements revealed her nervousness.",
    ],
    synonyms: ["ungainly", "clumsy", "uncoordinated"],
    antonyms: ["graceful", "elegant", "coordinated"],
  },
  {
    word: "inveigle",
    meaning: "persuade someone to do something using flattery or deception",
    partOfSpeech: "Verb",
    examples: [
      "He managed to inveigle his way into the exclusive party.",
      "She inveigled her brother into doing her chores.",
    ],
    synonyms: ["coax", "wheedle", "manipulate"],
    antonyms: ["discourage", "dissuade", "deter"],
  },
  {
    word: "jettison",
    meaning: "abandon or discard (something that is no longer needed)",
    partOfSpeech: "Verb",
    examples: [
      "The company jettisoned its unpopular product line.",
      "To save fuel, the pilot jettisoned excess cargo.",
    ],
    synonyms: ["discard", "dump", "shed"],
    antonyms: ["keep", "retain", "hold"],
  },
  {
    word: "mendacity",
    meaning: "dishonesty",
    partOfSpeech: "Noun",
    examples: [
      "The politician's career was ruined by his mendacity.",
      "A culture of mendacity had infected the organization.",
    ],
    synonyms: ["deceit", "falsehood", "lying"],
    antonyms: ["honesty", "truthfulness", "integrity"],
  },
  {
    word: "munificent",
    meaning: "more generous than is usual or necessary",
    partOfSpeech: "Adjective",
    examples: [
      "The munificent donor funded the entire building project.",
      "She was known for her munificent contributions to charity.",
    ],
    synonyms: ["generous", "lavish", "bountiful"],
    antonyms: ["stingy", "miserly", "tightfisted"],
  },
  {
    word: "naive",
    meaning: "showing a lack of experience or judgment",
    partOfSpeech: "Adjective",
    examples: [
      "His naive belief in people's goodness often led to disappointment.",
      "It was naive of her to trust strangers with her money.",
    ],
    synonyms: ["inexperienced", "gullible", "innocent"],
    antonyms: ["sophisticated", "worldly", "shrewd"],
  },
  {
    word: "noble",
    meaning: "having high moral qualities or principles",
    partOfSpeech: "Adjective",
    examples: [
      "Her noble actions inspired others to help the community.",
      "He had noble intentions, even if his methods were flawed.",
    ],
    synonyms: ["virtuous", "honorable", "ethical"],
    antonyms: ["ignoble", "base", "dishonorable"],
  },
  {
    word: "parochial",
    meaning: "having a limited perspective; narrow-minded",
    partOfSpeech: "Adjective",
    examples: [
      "His parochial views prevented him from understanding global issues.",
      "The small town's parochial attitudes discouraged outsiders.",
    ],
    synonyms: ["provincial", "insular", "narrow"],
    antonyms: ["cosmopolitan", "broad-minded", "sophisticated"],
  },
  {
    word: "pedestrian",
    meaning: "lacking imagination or excitement; ordinary",
    partOfSpeech: "Adjective",
    examples: [
      "The speech was pedestrian and failed to inspire the audience.",
      "Their pedestrian approach to design produced uninspiring results.",
    ],
    synonyms: ["dull", "banal", "unimaginative"],
    antonyms: ["imaginative", "original", "inspired"],
  },
  {
    word: "prevaricate",
    meaning: "evade or dodge the truth by being evasive",
    partOfSpeech: "Verb",
    examples: [
      "When asked directly, he prevaricated instead of giving a straight answer.",
      "Politicians often prevaricate when faced with difficult questions.",
    ],
    synonyms: ["equivocate", "hedge", "beat around the bush"],
    antonyms: ["confess", "declare", "assert"],
  },
  {
    word: "prime",
    meaning:
      "1. prepare something for use or action 2. of primary importance; major",
    partOfSpeech: "Verb/Adjective",
    examples: [
      "She primed the wall before painting it.",
      "The prime objective is to reduce costs without sacrificing quality.",
    ],
    synonyms: ["prepare", "ready", "main"],
    antonyms: ["secondary", "unprepared", "minor"],
  },
  {
    word: "radical",
    meaning:
      "1. far-reaching; major 2. a person advocating thorough or complete social or political reform",
    partOfSpeech: "Adjective/Noun",
    examples: [
      "The company underwent radical changes under new management.",
      "The radicals demanded complete overhaul of the political system.",
    ],
    synonyms: ["extreme", "revolutionary", "drastic"],
    antonyms: ["moderate", "conservative", "incremental"],
  },
  {
    word: "recrudescent",
    meaning: "breaking out again; reviving",
    partOfSpeech: "Adjective",
    examples: [
      "Doctors were concerned about recrudescent infections in some patients.",
      "The recrudescent violence threatened the peace process.",
    ],
    synonyms: ["recurring", "resurgent", "renewed"],
    antonyms: ["waning", "subsiding", "abating"],
  },
  {
    word: "temporal",
    meaning:
      "1. relating to time 2. relating to worldly rather than spiritual matters",
    partOfSpeech: "Adjective",
    examples: [
      "The temporal sequence of events was crucial to understanding the crime.",
      "Monks try to rise above temporal concerns and focus on spiritual matters.",
    ],
    synonyms: ["secular", "worldly", "chronological"],
    antonyms: ["spiritual", "eternal", "religious"],
  },
  {
    word: "transitory",
    meaning: "not permanent; temporary",
    partOfSpeech: "Adjective",
    examples: [
      "The pain was transitory and soon passed.",
      "Their stay in the city was meant to be transitory.",
    ],
    synonyms: ["temporary", "fleeting", "ephemeral"],
    antonyms: ["permanent", "lasting", "enduring"],
  },
  {
    word: "viable",
    meaning: "capable of working or succeeding; feasible",
    partOfSpeech: "Adjective",
    examples: [
      "After the changes, the business became viable again.",
      "Is this plan viable given our current resources?",
    ],
    synonyms: ["workable", "practicable", "achievable"],
    antonyms: ["unworkable", "impracticable", "unfeasible"],
  },
];

export const GROUP_20 = [
  {
    word: "abreast",
    meaning: "informed or familiar about a particular topic",
    partOfSpeech: "Adjective",
    examples: [
      "She stayed abreast of the latest developments in her field.",
      "It's important to keep abreast with current events.",
    ],
    synonyms: ["informed", "updated", "knowledgeable"],
    antonyms: ["uninformed", "ignorant", "unaware"],
  },
  {
    word: "confound",
    meaning: "perplex or puzzle someone",
    partOfSpeech: "Verb",
    examples: [
      "The magician's tricks confounded the audience.",
      "The complex instructions confounded the new employees.",
    ],
    synonyms: ["baffle", "bewilder", "flummox"],
    antonyms: ["enlighten", "clarify", "explain"],
  },
  {
    word: "digression",
    meaning: "an aside that strays from the main subject",
    partOfSpeech: "Noun",
    examples: [
      "The professor's digression about his vacation lasted 20 minutes.",
      "The book contained many interesting digressions from the main narrative.",
    ],
    synonyms: ["diversion", "detour", "tangent"],
    antonyms: ["focus", "concentration", "adherence"],
  },
  {
    word: "discrepancy",
    meaning: "a difference between two or more things that should be the same",
    partOfSpeech: "Noun",
    examples: [
      "There was a discrepancy between the two witness accounts.",
      "The audit revealed discrepancies in the financial records.",
    ],
    synonyms: ["inconsistency", "disparity", "variance"],
    antonyms: ["agreement", "consistency", "correspondence"],
  },
  {
    word: "duplicitous",
    meaning: "deceptive in speech or action",
    partOfSpeech: "Adjective",
    examples: [
      "The duplicitous salesman promised features the product didn't have.",
      "Her duplicitous behavior made it hard to trust her.",
    ],
    synonyms: ["deceitful", "two-faced", "dishonest"],
    antonyms: ["honest", "sincere", "truthful"],
  },
  {
    word: "expedient",
    meaning:
      "affording short-term benefits, often at the expense of the long-term",
    partOfSpeech: "Adjective",
    examples: [
      "The temporary fix was expedient but ultimately caused more problems.",
      "He chose the most expedient solution rather than the best one.",
    ],
    synonyms: ["convenient", "practical", "advantageous"],
    antonyms: ["impractical", "inexpedient", "disadvantageous"],
  },
  {
    word: "fabricate",
    meaning: "invent in order to deceive",
    partOfSpeech: "Verb",
    examples: [
      "The witness was caught fabricating his testimony.",
      "She fabricated an excuse for being late.",
    ],
    synonyms: ["invent", "make up", "falsify"],
    antonyms: ["reveal", "expose", "truthfully report"],
  },
  {
    word: "glum",
    meaning: "appearing or feeling sad or despondent",
    partOfSpeech: "Adjective",
    examples: [
      "He looked glum after hearing the bad news.",
      "The team was glum after their loss.",
    ],
    synonyms: ["morose", "sullen", "dejected"],
    antonyms: ["cheerful", "happy", "upbeat"],
  },
  {
    word: "harbinger",
    meaning:
      "a person or thing that foreshadows or foretells the coming of someone or something",
    partOfSpeech: "Noun",
    examples: [
      "The first robin is often seen as a harbinger of spring.",
      "The economic downturn was a harbinger of worse things to come.",
    ],
    synonyms: ["forerunner", "precursor", "herald"],
    antonyms: ["aftermath", "result", "consequence"],
  },
  {
    word: "intrinsic",
    meaning: "belonging to the essential nature of a thing",
    partOfSpeech: "Adjective",
    examples: [
      "The intrinsic value of gold makes it desirable.",
      "Helping others has intrinsic rewards.",
    ],
    synonyms: ["inherent", "innate", "essential"],
    antonyms: ["extrinsic", "external", "superficial"],
  },
  {
    word: "largesse",
    meaning:
      "the trait of being willing to donate money or resources; generosity",
    partOfSpeech: "Noun",
    examples: [
      "The millionaire was known for his largesse to charitable causes.",
      "The festival was made possible by the largesse of local businesses.",
    ],
    synonyms: ["generosity", "munificence", "bounty"],
    antonyms: ["stinginess", "miserliness", "parsimony"],
  },
  {
    word: "libertine",
    meaning: "someone loose in morals; a pleasure-seeker",
    partOfSpeech: "Noun",
    examples: [
      "The young libertine spent his inheritance on wine and parties.",
      "In his youth, he lived the life of a libertine.",
    ],
    synonyms: ["hedonist", "debauchee", "profligate"],
    antonyms: ["ascetic", "puritan", "moralist"],
  },
  {
    word: "malfeasance",
    meaning: "wrongdoing or misconduct, especially by a public official",
    partOfSpeech: "Noun",
    examples: [
      "The mayor was accused of malfeasance in office.",
      "Corporate malfeasance led to the company's downfall.",
    ],
    synonyms: ["misconduct", "wrongdoing", "misbehavior"],
    antonyms: ["integrity", "probity", "honesty"],
  },
  {
    word: "manifest",
    meaning: "1. be revealed; show plainly 2. obvious; not obscure or hidden",
    partOfSpeech: ["Verb", "Adjective"],
    examples: [
      "His anger manifested in his red face and clenched fists.",
      "The benefits of exercise are manifest in her good health.",
    ],
    synonyms: ["evident", "apparent", "demonstrate"],
    antonyms: ["hidden", "concealed", "obscure"],
  },
  {
    word: "minute",
    meaning: "very small",
    partOfSpeech: "Adjective",
    examples: [
      "The scientist studied minute particles under the microscope.",
      "There were minute differences between the two samples.",
    ],
    synonyms: ["tiny", "microscopic", "infinitesimal"],
    antonyms: ["large", "huge", "gigantic"],
  },
  {
    word: "modish",
    meaning: "in the current fashion or style",
    partOfSpeech: "Adjective",
    examples: [
      "Her modish outfit turned heads at the party.",
      "The restaurant's decor was modish but not very comfortable.",
    ],
    synonyms: ["fashionable", "trendy", "stylish"],
    antonyms: ["outdated", "unfashionable", "old-fashioned"],
  },
  {
    word: "nascent",
    meaning: "in the process of emerging, being born, or starting to develop",
    partOfSpeech: "Adjective",
    examples: [
      "The nascent technology showed great promise.",
      "Her nascent singing career was just beginning.",
    ],
    synonyms: ["emerging", "developing", "incipient"],
    antonyms: ["mature", "developed", "established"],
  },
  {
    word: "perennial",
    meaning: "lasting or remaining active throughout the year, or all the time",
    partOfSpeech: "Adjective",
    examples: [
      "The perennial river flows even during dry seasons.",
      "Her perennial optimism inspired those around her.",
    ],
    synonyms: ["constant", "enduring", "continual"],
    antonyms: ["temporary", "ephemeral", "transient"],
  },
  {
    word: "pious",
    meaning:
      "1. deeply devout; exhibiting religious devotion 2. making a show of one's own virtue",
    partOfSpeech: "Adjective",
    examples: [
      "The pious woman attended church daily.",
      "His pious remarks didn't match his actions.",
    ],
    synonyms: ["devout", "religious", "sanctimonious"],
    antonyms: ["impious", "irreligious", "sacrilegious"],
  },
  {
    word: "providential",
    meaning: "happening at a fortunate or opportune time",
    partOfSpeech: "Adjective",
    examples: [
      "The providential arrival of the ambulance saved his life.",
      "The rain came at a providential moment for the farmers.",
    ],
    synonyms: ["fortunate", "timely", "auspicious"],
    antonyms: ["unfortunate", "ill-timed", "inauspicious"],
  },
  {
    word: "prowess",
    meaning: "skillfulness and dexterity",
    partOfSpeech: "Noun",
    examples: [
      "Her athletic prowess won her many medals.",
      "The chef's culinary prowess was evident in every dish.",
    ],
    synonyms: ["skill", "expertise", "mastery"],
    antonyms: ["incompetence", "ineptitude", "clumsiness"],
  },
  {
    word: "schism",
    meaning: "a split or separation within a group or organization",
    partOfSpeech: "Noun",
    examples: [
      "The schism in the church led to the formation of a new denomination.",
      "Political schisms divided the once-unified party.",
    ],
    synonyms: ["division", "rift", "split"],
    antonyms: ["unity", "harmony", "accord"],
  },
  {
    word: "slander",
    meaning: "baselessly speak ill of someone or something",
    partOfSpeech: "Noun/Verb",
    examples: [
      "The celebrity sued the magazine for slander.",
      "He slandered his former business partner.",
    ],
    synonyms: ["defamation", "libel", "malign"],
    antonyms: ["praise", "commendation", "acclaim"],
  },
  {
    word: "stalwart",
    meaning: "determined; staunch",
    partOfSpeech: "Adjective",
    examples: [
      "She was a stalwart supporter of human rights.",
      "The stalwart defenders held their ground.",
    ],
    synonyms: ["loyal", "steadfast", "resolute"],
    antonyms: ["disloyal", "unreliable", "wavering"],
  },
  {
    word: "supplicate",
    meaning: "make a humble request; beg or beseech",
    partOfSpeech: "Verb",
    examples: [
      "The villagers supplicated the king for mercy.",
      "He supplicated before the altar.",
    ],
    synonyms: ["plead", "entreat", "implore"],
    antonyms: ["demand", "command", "order"],
  },
  {
    word: "terse",
    meaning: "brief and to the point",
    partOfSpeech: "Adjective",
    examples: [
      "His terse reply suggested he was annoyed.",
      "The instructions were terse but clear.",
    ],
    synonyms: ["concise", "succinct", "laconic"],
    antonyms: ["verbose", "wordy", "lengthy"],
  },
  {
    word: "tirade",
    meaning: "a prolonged, vehement speech or criticism",
    partOfSpeech: "Noun",
    examples: [
      "The coach delivered a tirade about poor teamwork.",
      "His political tirade lasted over an hour.",
    ],
    synonyms: ["diatribe", "harangue", "rant"],
    antonyms: ["praise", "compliment", "accolade"],
  },
  {
    word: "universal",
    meaning: "applicable or common to all purposes, conditions, or situations",
    partOfSpeech: "Adjective",
    examples: [
      "The desire for happiness is universal.",
      "The film had universal appeal across all age groups.",
    ],
    synonyms: ["global", "general", "ubiquitous"],
    antonyms: ["particular", "specific", "limited"],
  },
  {
    word: "vanquish",
    meaning: "defeat completely",
    partOfSpeech: "Verb",
    examples: [
      "The hero vanquished the villain in the final battle.",
      "She vanquished her fear of public speaking.",
    ],
    synonyms: ["conquer", "overcome", "subdue"],
    antonyms: ["surrender", "lose", "succumb"],
  },
  {
    word: "woeful",
    meaning: "1. very sad 2. very bad; awful",
    partOfSpeech: "Adjective",
    examples: [
      "The woeful widow mourned her husband.",
      "The team's woeful performance disappointed their fans.",
    ],
    synonyms: ["sorrowful", "miserable", "deplorable"],
    antonyms: ["joyful", "excellent", "admirable"],
  },
];

export const GROUP_99 = [
  {
    word: "accretion",
    meaning: "a gradual accumulation of something",
    partOfSpeech: "Noun",
    examples: [
      "The accretion of ice on the wings made the plane too heavy to fly.",
      "The firm's success was due to the slow accretion of small profits over many years.",
    ],
    antonyms: ["erosion", "decrease"],
    synonyms: ["accumulation", "buildup"],
  },
  {
    word: "acerbic",
    meaning: "sharp or biting in tone, taste, or manner",
    partOfSpeech: "Adjective",
    examples: [
      "The acerbic comments from the critic ruined the young artist's mood.",
      "His acerbic wit made him a difficult dinner companion.",
    ],
    antonyms: ["gentle", "kind"],
    synonyms: ["caustic", "trenchant"],
  },
  {
    word: "acme",
    meaning:
      "the point at which something is at its best or most developed; the pinnacle",
    partOfSpeech: "Noun",
    examples: [
      "The acme of his career was winning the Nobel Prize.",
      "Leonardo da Vinci's art represented the acme of the High Renaissance.",
    ],
    antonyms: ["nadir", "bottom"],
    synonyms: ["pinnacle", "zenith"],
  },
  {
    word: "affable",
    meaning: "friendly, good-natured, or easy to talk to",
    partOfSpeech: "Adjective",
    examples: [
      "Her affable nature made her a popular host.",
      "The new boss was surprisingly affable, greeting everyone with a smile.",
    ],
    antonyms: ["aloof", "reserved"],
    synonyms: ["amiable", "genial"],
  },
  {
    word: "ambiguous",
    meaning: "open to more than one interpretation; unclear",
    partOfSpeech: "Adjective",
    examples: [
      "The ending of the film was intentionally ambiguous, leaving the audience to debate its meaning.",
      "His ambiguous answer left me unsure of his true intentions.",
    ],
    antonyms: ["clear", "explicit"],
    synonyms: ["equivocal", "vague"],
  },
  {
    word: "amortize",
    meaning: "to reduce a debt by making regular payments",
    partOfSpeech: "Verb",
    examples: [
      "The company decided to amortize its loan over a 15-year period.",
      "She set up a payment plan to amortize her car debt.",
    ],
    antonyms: ["sustain", "increase"],
    synonyms: ["pay off", "liquidate"],
  },
  {
    word: "anodyne",
    meaning: "a drug that allays pain; something that soothes or calms",
    partOfSpeech: "Noun",
    examples: [
      "The doctor prescribed anodyne medication to help with his pain.",
      "She offered anodyne comments to placate the upset child.",
    ],
    antonyms: ["irritant", "agitator"],
    synonyms: ["soother", "pacifier"],
  },
  {
    word: "anomaly",
    meaning:
      "something that deviates from what is standard, normal, or expected",
    partOfSpeech: "Noun",
    examples: [
      "The low test score was an anomaly for the usually stellar student.",
      "The team's loss was a minor anomaly in an otherwise perfect season.",
    ],
    antonyms: ["regularity", "normality"],
    synonyms: ["aberration", "deviation"],
  },
  {
    word: "apocryphal",
    meaning: "of doubtful authenticity, although widely circulated as true",
    partOfSpeech: "Adjective",
    examples: [
      "The story of George Washington and the cherry tree is likely apocryphal.",
      "His account of his past adventures was highly apocryphal.",
    ],
    antonyms: ["authentic", "genuine"],
    synonyms: ["fictitious", "spurious"],
  },
  {
    word: "apostate",
    meaning: "a person who renounces a religious or political belief",
    partOfSpeech: "Noun",
    examples: [
      "The apostate was excommunicated for leaving the church.",
      "He was branded an apostate for his radical political views.",
    ],
    antonyms: ["devotee", "believer"],
    synonyms: ["renegade", "traitor"],
  },
  {
    word: "arrogate",
    meaning: "to take or claim something without justification",
    partOfSpeech: "Verb",
    examples: [
      "The politician arrogated the power of the legislature to himself.",
      "He arrogated the title of 'king' without any right to do so.",
    ],
    antonyms: ["renounce", "abdicate"],
    synonyms: ["appropriate", "usurp"],
  },
  {
    word: "asperity",
    meaning: "harshness of tone or manner",
    partOfSpeech: "Noun",
    examples: [
      "The asperity of his voice startled the children.",
      "She was known for the asperity of her critiques, but they were always accurate.",
    ],
    antonyms: ["mildness", "gentleness"],
    synonyms: ["acrimony", "bitterness"],
  },
  {
    word: "assiduous",
    meaning: "showing great care and perseverance",
    partOfSpeech: "Adjective",
    examples: [
      "The student was assiduous in his studies, always turning in work on time.",
      "The assiduous researcher spent countless hours in the lab.",
    ],
    antonyms: ["lazy", "careless"],
    synonyms: ["diligent", "industrious"],
  },
  {
    word: "avarice",
    meaning: "extreme greed for wealth or material gain",
    partOfSpeech: "Noun",
    examples: [
      "The king's avarice led him to tax his people into poverty.",
      "Avarice is one of the seven deadly sins.",
    ],
    antonyms: ["generosity", "benevolence"],
    synonyms: ["cupidity", "greed"],
  },
  {
    word: "canard",
    meaning: "a false or unfounded rumor or story",
    partOfSpeech: "Noun",
    examples: [
      "The rumor that the actor was a spy was a complete canard.",
      "He was angered by the spreading of the malicious canard about him.",
    ],
    antonyms: ["fact", "truth"],
    synonyms: ["hoax", "fabrication"],
  },
  {
    word: "cardinal",
    meaning: "of the greatest importance; fundamental",
    partOfSpeech: "Adjective",
    examples: [
      "Patience is a cardinal virtue for a teacher.",
      "The cardinal rule of the game is to be honest.",
    ],
    antonyms: ["minor", "insignificant"],
    synonyms: ["primary", "essential"],
  },
  {
    word: "catalyst",
    meaning: "a person or thing that precipitates an event or change",
    partOfSpeech: "Noun",
    examples: [
      "The new CEO was a catalyst for change in the struggling company.",
      "His emotional speech was the catalyst for the political movement.",
    ],
    antonyms: ["impediment", "stifler"],
    synonyms: ["precipitator", "impetus"],
  },
  {
    word: "collusion",
    meaning: "secret cooperation or conspiracy, especially to deceive others",
    partOfSpeech: "Noun",
    examples: [
      "The two companies were accused of collusion to fix prices.",
      "Their collusion was exposed by a whistle-blower.",
    ],
    antonyms: ["honesty", "openness"],
    synonyms: ["conspiracy", "cahoots"],
  },
  {
    word: "contumacious",
    meaning: "stubbornly disobedient to authority",
    partOfSpeech: "Adjective",
    examples: [
      "The contumacious student refused to follow the teacher's instructions.",
      "His contumacious attitude landed him in detention.",
    ],
    antonyms: ["obedient", "compliant"],
    synonyms: ["rebellious", "insubordinate"],
  },
  {
    word: "copious",
    meaning: "abundant in supply or quantity",
    partOfSpeech: "Adjective",
    examples: [
      "The chef prepared copious amounts of food for the banquet.",
      "The new library had a copious supply of books.",
    ],
    antonyms: ["sparse", "lacking"],
    synonyms: ["abundant", "plentiful"],
  },
  {
    word: "credulous",
    meaning: "having or showing too great a readiness to believe things",
    partOfSpeech: "Adjective",
    examples: [
      "The credulous tourist was easily scammed by the street vendor.",
      "He was a credulous fool, believing every lie he was told.",
    ],
    antonyms: ["skeptical", "cynical"],
    synonyms: ["gullible", "naive"],
  },
  {
    word: "cupidity",
    meaning: "greed for money or possessions",
    partOfSpeech: "Noun",
    examples: [
      "The criminal's cupidity drove him to steal from his own family.",
      "He was a man consumed by cupidity, desiring more and more wealth.",
    ],
    antonyms: ["generosity", "benevolence"],
    synonyms: ["avarice", "greed"],
  },
  {
    word: "decorous",
    meaning: "polite and restrained; proper",
    partOfSpeech: "Adjective",
    examples: [
      "The guests at the wedding were decorous, behaving with respect and grace.",
      "She found it difficult to maintain a decorous attitude during the argument.",
    ],
    antonyms: ["improper", "unseemly"],
    synonyms: ["polite", "dignified"],
  },
  {
    word: "diaphanous",
    meaning: "light, delicate, and translucent",
    partOfSpeech: "Adjective",
    examples: [
      "She wore a diaphanous silk gown to the party.",
      "The morning mist was a diaphanous veil over the valley.",
    ],
    antonyms: ["opaque", "thick"],
    synonyms: ["sheer", "translucent"],
  },
  {
    word: "disabuse",
    meaning: "to persuade someone that an idea or belief is mistaken",
    partOfSpeech: "Verb",
    examples: [
      "I had to disabuse him of the notion that I was wealthy.",
      "The harsh reality of the job was meant to disabuse new employees of their high expectations.",
    ],
    antonyms: ["delude", "deceive"],
    synonyms: ["enlighten", "correct"],
  },
  {
    word: "discrete",
    meaning: "individually separate and distinct",
    partOfSpeech: "Adjective",
    examples: [
      "The system is made up of several discrete components that work together.",
      "The artist used a series of discrete dots to create the image.",
    ],
    antonyms: ["continuous", "connected"],
    synonyms: ["separate", "distinct"],
  },
  {
    word: "disparage",
    meaning: "to regard or represent as being of little worth",
    partOfSpeech: "Verb",
    examples: [
      "It is never wise to disparage your rivals in a public forum.",
      "He made it his mission to disparage his opponent's work.",
    ],
    antonyms: ["praise", "compliment"],
    synonyms: ["belittle", "denigrate"],
  },
  {
    word: "dispassionate",
    meaning: "not influenced by strong emotion; impartial",
    partOfSpeech: "Adjective",
    examples: [
      "The judge was expected to make a dispassionate decision.",
      "He offered a dispassionate analysis of the situation, free of personal bias.",
    ],
    antonyms: ["biased", "emotional"],
    synonyms: ["impartial", "unemotional"],
  },
  {
    word: "dissemble",
    meaning: "to conceal one's true motives, feelings, or beliefs",
    partOfSpeech: "Verb",
    examples: [
      "He dissembled his true feelings with a false smile.",
      "The politician attempted to dissemble his involvement in the scandal.",
    ],
    antonyms: ["reveal", "disclose"],
    synonyms: ["feign", "pretend"],
  },
  {
    word: "elegy",
    meaning: "a poem of serious reflection, typically a lament for the dead",
    partOfSpeech: "Noun",
    examples: [
      "He wrote a moving elegy to his late friend.",
      "The author's new book is a beautiful elegy to a lost era.",
    ],
    antonyms: ["eulogy", "panegyric"],
    synonyms: ["lament", "dirge"],
  },
  {
    word: "eminent",
    meaning: "famous and respected within a particular sphere or profession",
    partOfSpeech: "Adjective",
    examples: [
      "He was an eminent scholar of ancient history.",
      "The conference was attended by several eminent scientists.",
    ],
    antonyms: ["unknown", "obscure"],
    synonyms: ["prominent", "distinguished"],
  },
  {
    word: "encomium",
    meaning:
      "a speech or piece of writing that praises someone or something highly",
    partOfSpeech: "Noun",
    examples: [
      "The new president's speech was an encomium for her predecessor.",
      "He delivered a heartfelt encomium at his father's retirement party.",
    ],
    antonyms: ["censure", "criticism"],
    synonyms: ["eulogy", "tribute"],
  },
  {
    word: "enumerate",
    meaning: "to mention a number of things one by one; to list",
    partOfSpeech: "Verb",
    examples: [
      "He began to enumerate all the reasons he was leaving the company.",
      "The report enumerated the challenges facing the new project.",
    ],
    antonyms: ["generalize", "summarize"],
    synonyms: ["list", "itemize"],
  },
  {
    word: "equivocal",
    meaning: "open to more than one interpretation; ambiguous",
    partOfSpeech: "Adjective",
    examples: [
      "His equivocal statement left the audience guessing his true position.",
      "The research findings were equivocal, making it hard to draw a conclusion.",
    ],
    antonyms: ["unequivocal", "clear"],
    synonyms: ["ambiguous", "vague"],
  },
  {
    word: "ersatz",
    meaning: "made or used as a substitute, typically an inferior one",
    partOfSpeech: "Adjective",
    examples: [
      "The coffee substitute was a poor ersatz for the real thing.",
      "They had to settle for ersatz wood paneling instead of real oak.",
    ],
    antonyms: ["authentic", "genuine"],
    synonyms: ["imitation", "substitute"],
  },
  {
    word: "espouse",
    meaning: "to adopt or support a cause, belief, or way of life",
    partOfSpeech: "Verb",
    examples: [
      "She espoused the cause of civil rights throughout her career.",
      "The group espoused a vegetarian diet for both health and ethical reasons.",
    ],
    antonyms: ["reject", "oppose"],
    synonyms: ["advocate", "champion"],
  },
  {
    word: "eulogy",
    meaning:
      "a speech or piece of writing that praises someone highly, typically for someone who has recently died",
    partOfSpeech: "Noun",
    examples: [
      "He delivered a heartfelt eulogy at his friend's funeral.",
      "The article was a moving eulogy to the late scientist.",
    ],
    antonyms: ["censure", "condemnation"],
    synonyms: ["tribute", "panegyric"],
  },
  {
    word: "expurgate",
    meaning:
      "to remove matter thought to be objectionable or unsuitable from a text",
    partOfSpeech: "Verb",
    examples: [
      "The publisher had to expurgate several passages from the controversial book.",
      "The editors expurgated the film to make it suitable for a younger audience.",
    ],
    antonyms: ["adulterate", "add"],
    synonyms: ["censor", "cleanse"],
  },
  {
    word: "extemporaneous",
    meaning: "done or said without preparation; impromptu",
    partOfSpeech: "Adjective",
    examples: [
      "His extemporaneous speech was surprisingly eloquent.",
      "She gave an extemporaneous performance that earned a standing ovation.",
    ],
    antonyms: ["planned", "rehearsed"],
    synonyms: ["impromptu", "unrehearsed"],
  },
  {
    word: "fatuous",
    meaning: "silly and pointless",
    partOfSpeech: "Adjective",
    examples: [
      "It was a fatuous comment that contributed nothing to the debate.",
      "The fatuous plan was destined to fail from the start.",
    ],
    antonyms: ["sensible", "intelligent"],
    synonyms: ["foolish", "inane"],
  },
  {
    word: "fetid",
    meaning: "smelling extremely unpleasant",
    partOfSpeech: "Adjective",
    examples: [
      "The neglected garbage can had a fetid odor.",
      "The fetid air in the old cellar was unbearable.",
    ],
    antonyms: ["fragrant", "aromatic"],
    synonyms: ["stinking", "malodorous"],
  },
  {
    word: "fortuitous",
    meaning: "happening by chance, especially a lucky chance",
    partOfSpeech: "Adjective",
    examples: [
      "It was a fortuitous coincidence that they were both in Paris at the same time.",
      "The discovery of the ancient artifact was a fortuitous finding for the archeologists.",
    ],
    antonyms: ["unlucky", "unfortunate"],
    synonyms: ["accidental", "serendipitous"],
  },
  {
    word: "frugal",
    meaning: "economical with money or food; thrifty",
    partOfSpeech: "Adjective",
    examples: [
      "She was known for her frugal habits, always looking for a good deal.",
      "The family lived a frugal but comfortable life.",
    ],
    antonyms: ["extravagant", "wasteful"],
    synonyms: ["thrifty", "prudent"],
  },
  {
    word: "genial",
    meaning: "friendly and cheerful",
    partOfSpeech: "Adjective",
    examples: [
      "The genial host made everyone feel welcome.",
      "His genial smile made the children feel at ease.",
    ],
    antonyms: ["unfriendly", "aloof"],
    synonyms: ["amiable", "affable"],
  },
  {
    word: "guile",
    meaning: "sly or cunning intelligence; deceit",
    partOfSpeech: "Noun",
    examples: [
      "The trickster used his guile to outwit the authorities.",
      "Her innocent appearance hid a mind full of guile.",
    ],
    antonyms: ["honesty", "sincerity"],
    synonyms: ["cunning", "deceit"],
  },
  {
    word: "halcyon",
    meaning:
      "denoting a period of time in the past that was idyllically happy and peaceful",
    partOfSpeech: "Adjective",
    examples: [
      "She looked back on her college years as her halcyon days.",
      "The writer romanticized the halcyon days of his youth.",
    ],
    antonyms: ["turbulent", "unsettled"],
    synonyms: ["serene", "tranquil"],
  },
  {
    word: "hegemony",
    meaning:
      "leadership or dominance, especially by one country or social group over others",
    partOfSpeech: "Noun",
    examples: [
      "The nation's economic hegemony was a source of its power.",
      "The local team maintained its hegemony over the league for years.",
    ],
    antonyms: ["subservience", "subordination"],
    synonyms: ["dominance", "supremacy"],
  },
  {
    word: "hermetic",
    meaning: "insulated or protected from outside influence",
    partOfSpeech: "Adjective",
    examples: [
      "The laboratory was kept under hermetic conditions to prevent contamination.",
      "He lived a hermetic existence, with little contact with the outside world.",
    ],
    antonyms: ["permeable", "accessible"],
    synonyms: ["sealed", "airtight"],
  },
  {
    word: "hubris",
    meaning: "excessive pride or self-confidence",
    partOfSpeech: "Noun",
    examples: [
      "The hero's tragic flaw was his hubris, which led to his downfall.",
      "His hubris prevented him from listening to the warnings of his advisors.",
    ],
    antonyms: ["humility", "modesty"],
    synonyms: ["arrogance", "pride"],
  },
  {
    word: "iconoclast",
    meaning: "a person who attacks cherished beliefs or institutions",
    partOfSpeech: "Noun",
    examples: [
      "The artist was known as an iconoclast for his rejection of traditional art forms.",
      "He was an iconoclast who challenged the established norms of society.",
    ],
    antonyms: ["conformist", "traditionalist"],
    synonyms: ["maverick", "rebel"],
  },
  {
    word: "idiosyncrasy",
    meaning: "a peculiar habit or characteristic",
    partOfSpeech: "Noun",
    examples: [
      "One of his idiosyncrasies was his habit of wearing two different colored socks.",
      "The car's peculiar idiosyncrasy was its tendency to stall at every stop sign.",
    ],
    antonyms: ["normality", "regularity"],
    synonyms: ["peculiarity", "quirk"],
  },
  {
    word: "ignoble",
    meaning: "not honorable in character or purpose",
    partOfSpeech: "Adjective",
    examples: [
      "The dictator was known for his ignoble acts of cruelty.",
      "It was an ignoble goal to seek revenge on an old friend.",
    ],
    antonyms: ["honorable", "noble"],
    synonyms: ["dishonorable", "base"],
  },
  {
    word: "impecunious",
    meaning: "having little or no money",
    partOfSpeech: "Adjective",
    examples: [
      "He started his career as an impecunious artist, living on bread and water.",
      "The recession left many people in an impecunious state.",
    ],
    antonyms: ["wealthy", "rich"],
    synonyms: ["penniless", "poor"],
  },
  {
    word: "impute",
    meaning: "to attribute a fault or responsibility to",
    partOfSpeech: "Verb",
    examples: [
      "The police imputed the crime to the suspect without any real evidence.",
      "He was quick to impute blame to others for his own failures.",
    ],
    antonyms: ["exonerate", "absolve"],
    synonyms: ["ascribe", "attribute"],
  },
  {
    word: "incipient",
    meaning: "in an initial stage; beginning to happen or develop",
    partOfSpeech: "Adjective",
    examples: [
      "The doctor's timely diagnosis caught the disease in its incipient stage.",
      "We need to address this incipient crisis before it escalates.",
    ],
    antonyms: ["developed", "mature"],
    synonyms: ["developing", "nascent"],
  },
  {
    word: "incorrigible",
    meaning: "not able to be corrected, improved, or reformed",
    partOfSpeech: "Adjective",
    examples: [
      "The incorrigible student was constantly getting into trouble.",
      "She was an incorrigible liar, incapable of telling the truth.",
    ],
    antonyms: ["reformable", "manageable"],
    synonyms: ["unruly", "unmanageable"],
  },
  {
    word: "ineffable",
    meaning: "too great or extreme to be expressed or described in words",
    partOfSpeech: "Adjective",
    examples: [
      "The beauty of the sunset was ineffable.",
      "He experienced an ineffable feeling of peace and joy.",
    ],
    antonyms: ["describable", "definable"],
    synonyms: ["indescribable", "transcendent"],
  },
  {
    word: "inexorable",
    meaning: "impossible to stop or prevent",
    partOfSpeech: "Adjective",
    examples: [
      "The inexorable march of time cannot be stopped.",
      "The company faced an inexorable decline in sales.",
    ],
    antonyms: ["avoidable", "preventable"],
    synonyms: ["unrelenting", "unstoppable"],
  },
  {
    word: "ingratiate",
    meaning:
      "to bring oneself into favor with someone by flattering or trying to please them",
    partOfSpeech: "Verb",
    examples: [
      "He tried to ingratiate himself with his new boss by praising his every idea.",
      "She sought to ingratiate herself with her in-laws.",
    ],
    antonyms: ["alienate", "offend"],
    synonyms: ["curry favor", "flatter"],
  },
  {
    word: "invective",
    meaning: "insulting, abusive, or highly critical language",
    partOfSpeech: "Noun",
    examples: [
      "The politician's speech was filled with invective against his opponents.",
      "He was known for his invective, never holding back in his critiques.",
    ],
    antonyms: ["praise", "compliment"],
    synonyms: ["abuse", "vituperation"],
  },
  {
    word: "jingoism",
    meaning:
      "extreme patriotism, especially in the form of aggressive or warlike foreign policy",
    partOfSpeech: "Noun",
    examples: [
      "The leader's speech was full of jingoism, inciting his people to war.",
      "The newspaper was accused of spreading jingoism with its articles.",
    ],
    antonyms: ["pacifism", "internationalism"],
    synonyms: ["chauvinism", "nationalism"],
  },
  {
    word: "juxtapose",
    meaning: "to place two things side by side for a contrasting effect",
    partOfSpeech: "Verb",
    examples: [
      "The artist juxtaposed bright colors with dark ones in his painting.",
      "The film juxtaposed scenes of luxury with scenes of poverty.",
    ],
    antonyms: ["separate", "distribute"],
    synonyms: ["contrast", "compare"],
  },
  {
    word: "lachrymose",
    meaning: "tearful or given to weeping",
    partOfSpeech: "Adjective",
    examples: [
      "The movie was a lachrymose tale of a lost love.",
      "He was in a lachrymose state after hearing the bad news.",
    ],
    antonyms: ["joyful", "happy"],
    synonyms: ["weepy", "sentimental"],
  },
  {
    word: "largess",
    meaning: "generosity in bestowing money or gifts upon others",
    partOfSpeech: "Noun",
    examples: [
      "The millionaire was known for his largess, donating to many charities.",
      "The company's largess was appreciated by its employees.",
    ],
    antonyms: ["stinginess", "avarice"],
    synonyms: ["generosity", "benevolence"],
  },
  {
    word: "latent",
    meaning: "existing but not yet developed or manifest; hidden or concealed",
    partOfSpeech: "Adjective",
    examples: [
      "He had a latent talent for music that he discovered in college.",
      "The virus remained latent in the body for years before it became active.",
    ],
    antonyms: ["active", "obvious"],
    synonyms: ["dormant", "hidden"],
  },
  {
    word: "luminous",
    meaning: "full of or shedding light; bright or shining",
    partOfSpeech: "Adjective",
    examples: [
      "The full moon was a luminous orb in the night sky.",
      "Her face was luminous with happiness.",
    ],
    antonyms: ["dark", "dim"],
    synonyms: ["radiant", "glowing"],
  },
  {
    word: "magnate",
    meaning: "a wealthy and influential person, especially in business",
    partOfSpeech: "Noun",
    examples: [
      "He was a shipping magnate with a fortune in the billions.",
      "The media magnate owned newspapers and television stations across the globe.",
    ],
    antonyms: ["pauper", "commoner"],
    synonyms: ["tycoon", "mogul"],
  },
  {
    word: "malinger",
    meaning: "to feign illness in order to escape duty or work",
    partOfSpeech: "Verb",
    examples: [
      "He was accused of malingering to avoid his chores.",
      "The employee was fired after he was caught malingering.",
    ],
    antonyms: ["work", "strive"],
    synonyms: ["shirk", "feign"],
  },
  {
    word: "maudlin",
    meaning: "self-pityingly or tearfully sentimental",
    partOfSpeech: "Adjective",
    examples: [
      "He became maudlin and started crying about his past mistakes.",
      "The maudlin movie was filled with sappy romantic scenes.",
    ],
    antonyms: ["stoic", "unemotional"],
    synonyms: ["sentimental", "weepy"],
  },
  {
    word: "maverick",
    meaning: "an unorthodox or independent-minded person",
    partOfSpeech: "Noun",
    examples: [
      "He was a maverick who never followed the crowd.",
      "The artist was considered a maverick for her experimental style.",
    ],
    antonyms: ["conformist", "traditionalist"],
    synonyms: ["individualist", "nonconformist"],
  },
  {
    word: "mendicant",
    meaning: "a beggar",
    partOfSpeech: "Noun",
    examples: [
      "The mendicant stood on the street corner, asking for spare change.",
      "He lived a life as a mendicant, relying on the kindness of strangers.",
    ],
    antonyms: ["millionaire", "magnate"],
    synonyms: ["beggar", "pauper"],
  },
  {
    word: "meretricious",
    meaning: "apparently attractive but having no real value or integrity",
    partOfSpeech: "Adjective",
    examples: [
      "The meretricious display of wealth was a facade for their poverty.",
      "Her meretricious promises were nothing but a lie.",
    ],
    antonyms: ["genuine", "sincere"],
    synonyms: ["flashy", "gaudy"],
  },
  {
    word: "noisome",
    meaning: "having an extremely offensive smell",
    partOfSpeech: "Adjective",
    examples: [
      "The noisome fumes from the factory were a public health concern.",
      "The discarded food had a noisome odor that attracted pests.",
    ],
    antonyms: ["fragrant", "pleasant"],
    synonyms: ["fetid", "malodorous"],
  },
  {
    word: "nuance",
    meaning: "a subtle difference in or shade of meaning, expression, or sound",
    partOfSpeech: "Noun",
    examples: [
      "The artist was able to capture every nuance of light in her painting.",
      "The speech was delivered with such nuance that every word seemed to have a deeper meaning.",
    ],
    antonyms: ["obviousness", "blatancy"],
    synonyms: ["subtlety", "shade"],
  },
  {
    word: "oblique",
    meaning: "not expressed in a direct way; indirect",
    partOfSpeech: "Adjective",
    examples: [
      "He gave an oblique answer, avoiding the direct question.",
      "The article made an oblique reference to a recent scandal.",
    ],
    antonyms: ["direct", "explicit"],
    synonyms: ["indirect", "evasive"],
  },
  {
    word: "palliate",
    meaning:
      "to make a disease or its symptoms less severe or unpleasant without removing the cause",
    partOfSpeech: "Verb",
    examples: [
      "The medication will palliate the pain, but it won't cure the disease.",
      "He tried to palliate his guilt by making a small donation to charity.",
    ],
    antonyms: ["aggravate", "worsen"],
    synonyms: ["alleviate", "mitigate"],
  },
  {
    word: "panegyric",
    meaning:
      "a public speech or published text in praise of someone or something",
    partOfSpeech: "Noun",
    examples: [
      "The mayor's speech was a panegyric to the city's founders.",
      "He delivered a panegyric to his mentor at the awards ceremony.",
    ],
    antonyms: ["censure", "criticism"],
    synonyms: ["eulogy", "encomium"],
  },
  {
    word: "pariah",
    meaning: "an outcast",
    partOfSpeech: "Noun",
    examples: [
      "He was treated like a pariah after his scandalous behavior was revealed.",
      "The village made him a pariah for his strange beliefs.",
    ],
    antonyms: ["celebrity", "insider"],
    synonyms: ["outcast", "exile"],
  },
  {
    word: "partisan",
    meaning: "a strong supporter of a party, cause, or person",
    partOfSpeech: "Noun",
    examples: [
      "The partisan newspaper only published articles that favored its political party.",
      "He was a strong partisan of the new bill.",
    ],
    antonyms: ["unbiased", "impartial"],
    synonyms: ["supporter", "advocate"],
  },
  {
    word: "paucity",
    meaning:
      "the presence of something only in small or insufficient quantities",
    partOfSpeech: "Noun",
    examples: [
      "There was a paucity of evidence to support the claim.",
      "The team was hampered by a paucity of talented players.",
    ],
    antonyms: ["plethora", "abundance"],
    synonyms: ["scarcity", "dearth"],
  },
  {
    word: "peripatetic",
    meaning: "traveling from place to place, especially to work or live",
    partOfSpeech: "Adjective",
    examples: [
      "He led a peripatetic life as a traveling musician.",
      "The peripatetic philosopher lectured at universities across the globe.",
    ],
    antonyms: ["stationary", "settled"],
    synonyms: ["itinerant", "nomadic"],
  },
  {
    word: "peruse",
    meaning: "to read something in a careful and thorough way",
    partOfSpeech: "Verb",
    examples: [
      "She perused the documents before signing the contract.",
      "He spent the afternoon perusing the books in the library.",
    ],
    antonyms: ["skim", "glance"],
    synonyms: ["examine", "read"],
  },
  {
    word: "pervasive",
    meaning: "spreading widely throughout an area or a group of people",
    partOfSpeech: "Adjective",
    examples: [
      "The pervasive smell of smoke lingered in the air.",
      "The media's influence has become a pervasive force in our society.",
    ],
    antonyms: ["contained", "limited"],
    synonyms: ["widespread", "ubiquitous"],
  },
  {
    word: "pious",
    meaning: "devoutly religious",
    partOfSpeech: "Adjective",
    examples: [
      "The pious man prayed every day.",
      "She led a pious life, dedicated to her faith.",
    ],
    antonyms: ["irreligious", "atheistic"],
    synonyms: ["devout", "religious"],
  },
  {
    word: "pith",
    meaning: "the essential or central part of something",
    partOfSpeech: "Noun",
    examples: [
      "He was able to get to the pith of the matter with a single question.",
      "The professor's lecture presented the pith of the philosophical text.",
    ],
    antonyms: ["exterior", "periphery"],
    synonyms: ["essence", "core"],
  },
  {
    word: "plethora",
    meaning: "a large or excessive amount of something",
    partOfSpeech: "Noun",
    examples: [
      "There was a plethora of food at the buffet.",
      "The city has a plethora of new restaurants to choose from.",
    ],
    antonyms: ["dearth", "scarcity"],
    synonyms: ["abundance", "surplus"],
  },
  {
    word: "posit",
    meaning: "to assume as a fact or put forward as a basis for argument",
    partOfSpeech: "Verb",
    examples: [
      "He posited that the Earth was round long before it was widely accepted.",
      "The new theory posits the existence of a new type of particle.",
    ],
    antonyms: ["disprove", "refute"],
    synonyms: ["assume", "presuppose"],
  },
  {
    word: "pragmatic",
    meaning:
      "dealing with things sensibly and realistically in a way that is based on practical rather than theoretical considerations",
    partOfSpeech: "Adjective",
    examples: [
      "He took a pragmatic approach to the problem, focusing on what would actually work.",
      "The leader was known for her pragmatic and effective policies.",
    ],
    antonyms: ["idealistic", "impractical"],
    synonyms: ["practical", "realistic"],
  },
  {
    word: "prattle",
    meaning: "to talk at length in a foolish or inconsequential way",
    partOfSpeech: "Verb",
    examples: [
      "She prattled on for hours about her vacation.",
      "He listened to the children's incessant prattle.",
    ],
    antonyms: ["be silent", "be quiet"],
    synonyms: ["chatter", "babble"],
  },
  {
    word: "precursor",
    meaning:
      "a person or thing that comes before another of the same kind; a forerunner",
    partOfSpeech: "Noun",
    examples: [
      "The discovery of the fossil was a precursor to the discovery of a new species.",
      "The new smartphone was a precursor to a complete overhaul of the product line.",
    ],
    antonyms: ["successor", "descendant"],
    synonyms: ["forerunner", "harbinger"],
  },
  {
    word: "predilection",
    meaning: "a preference or special liking for something",
    partOfSpeech: "Noun",
    examples: [
      "He had a predilection for spicy food.",
      "Her predilection for romance novels was well-known.",
    ],
    antonyms: ["dislike", "aversion"],
    synonyms: ["preference", "inclination"],
  },
  {
    word: "pulchritude",
    meaning: "physical beauty",
    partOfSpeech: "Noun",
    examples: [
      "The painting was a testament to the pulchritude of the human form.",
      "Her pulchritude was admired by everyone who met her.",
    ],
    antonyms: ["ugliness", "homeliness"],
    synonyms: ["beauty", "attractiveness"],
  },
  {
    word: "pusillanimous",
    meaning: "showing a lack of courage or determination; cowardly",
    partOfSpeech: "Adjective",
    examples: [
      "The pusillanimous soldier fled the battlefield.",
      "He was too pusillanimous to stand up for himself.",
    ],
    antonyms: ["brave", "courageous"],
    synonyms: ["cowardly", "timid"],
  },
  {
    word: "quotidian",
    meaning: "of or occurring every day; commonplace",
    partOfSpeech: "Adjective",
    examples: [
      "The novel captured the quotidian lives of ordinary people.",
      "Washing dishes and doing laundry were part of her quotidian routine.",
    ],
    antonyms: ["unusual", "extraordinary"],
    synonyms: ["everyday", "ordinary"],
  },
  {
    word: "raconteur",
    meaning: "a person who tells anecdotes in a skillful and amusing way",
    partOfSpeech: "Noun",
    examples: [
      "The old man was a natural raconteur, with a story for every occasion.",
      "She was a popular dinner guest because of her talent as a raconteur.",
    ],
    antonyms: ["bore", "dullard"],
    synonyms: ["storyteller", "narrator"],
  },
  {
    word: "recalcitrant",
    meaning: "having an uncooperative attitude toward authority or discipline",
    partOfSpeech: "Adjective",
    examples: [
      "The recalcitrant horse refused to move forward.",
      "The student was a recalcitrant troublemaker, unwilling to follow any rules.",
    ],
    antonyms: ["compliant", "obedient"],
    synonyms: ["unruly", "insubordinate"],
  },
  {
    word: "redress",
    meaning: "to remedy or set right an undesirable situation",
    partOfSpeech: "Verb",
    examples: [
      "He sought to redress the wrongs done to his family.",
      "The company offered a refund as a way to redress the customer's complaint.",
    ],
    antonyms: ["aggravate", "worsen"],
    synonyms: ["remedy", "rectify"],
  },
  {
    word: "refractory",
    meaning: "stubborn or unmanageable",
    partOfSpeech: "Adjective",
    examples: [
      "The refractory child refused to go to bed.",
      "The new medicine was ineffective on the refractory illness.",
    ],
    antonyms: ["obedient", "compliant"],
    synonyms: ["stubborn", "unruly"],
  },
  {
    word: "replete",
    meaning: "filled or well-supplied with something",
    partOfSpeech: "Adjective",
    examples: [
      "The library was replete with books on every subject.",
      "He left the dinner table feeling replete and satisfied.",
    ],
    antonyms: ["empty", "lacking"],
    synonyms: ["full", "abundant"],
  },
  {
    word: "reverent",
    meaning: "feeling or showing deep respect",
    partOfSpeech: "Adjective",
    examples: [
      "He spoke of his teacher in a reverent tone.",
      "The crowd stood in reverent silence during the ceremony.",
    ],
    antonyms: ["irreverent", "disrespectful"],
    synonyms: ["respectful", "devout"],
  },
  {
    word: "ribald",
    meaning:
      "referring to sexual matters in an amusingly rude or irreverent way",
    partOfSpeech: "Adjective",
    examples: [
      "The comedian's act was full of ribald jokes.",
      "He was known for his ribald sense of humor.",
    ],
    antonyms: ["proper", "chaste"],
    synonyms: ["vulgar", "lewd"],
  },
  {
    word: "salient",
    meaning: "most noticeable or important",
    partOfSpeech: "Adjective",
    examples: [
      "The salient point of the argument was his lack of evidence.",
      "He highlighted the most salient features of the proposal in his presentation.",
    ],
    antonyms: ["minor", "insignificant"],
    synonyms: ["prominent", "important"],
  },
  {
    word: "sardonic",
    meaning: "grimly mocking or cynical",
    partOfSpeech: "Adjective",
    examples: [
      "He offered a sardonic smile as he listened to the excuses.",
      "The documentary was a sardonic look at modern politics.",
    ],
    antonyms: ["sincere", "earnest"],
    synonyms: ["cynical", "mocking"],
  },
  {
    word: "slake",
    meaning: "to quench or satisfy one's thirst",
    partOfSpeech: "Verb",
    examples: [
      "He took a long drink to slake his thirst after the marathon.",
      "The cool water was a welcome relief to slake his parched throat.",
    ],
    antonyms: ["dry", "dehydrate"],
    synonyms: ["quench", "satisfy"],
  },
  {
    word: "solecism",
    meaning: "a grammatical mistake; a breach of good manners",
    partOfSpeech: "Noun",
    examples: [
      "The student's essay was filled with grammatical solecisms.",
      "He committed a social solecism by not shaking the host's hand.",
    ],
    antonyms: ["correctness", "propriety"],
    synonyms: ["blunder", "mistake"],
  },
  {
    word: "solicitous",
    meaning: "characterized by or showing interest or concern",
    partOfSpeech: "Adjective",
    examples: [
      "He was solicitous of his mother's health, calling her every day.",
      "The new employee was solicitous, always asking if anyone needed help.",
    ],
    antonyms: ["unconcerned", "indifferent"],
    synonyms: ["concerned", "attentive"],
  },
  {
    word: "squalid",
    meaning:
      "extremely dirty and unpleasant, especially as a result of poverty or neglect",
    partOfSpeech: "Adjective",
    examples: [
      "The family was forced to live in squalid conditions after losing their home.",
      "The building was a squalid slum, unfit for human habitation.",
    ],
    antonyms: ["clean", "pristine"],
    synonyms: ["filthy", "dingy"],
  },
  {
    word: "subversive",
    meaning:
      "seeking or intended to undermine an established system or institution",
    partOfSpeech: "Adjective",
    examples: [
      "The group was accused of engaging in subversive activities against the government.",
      "The play was banned for its subversive message.",
    ],
    antonyms: ["loyal", "patriotic"],
    synonyms: ["rebellious", "seditious"],
  },
  {
    word: "surly",
    meaning: "bad-tempered and unfriendly",
    partOfSpeech: "Adjective",
    examples: [
      "The surly waiter refused to take our order with a smile.",
      "He was in a surly mood after a long day at work.",
    ],
    antonyms: ["friendly", "amiable"],
    synonyms: ["grumpy", "rude"],
  },
  {
    word: "sybarite",
    meaning: "a person who is self-indulgent in their love of luxury",
    partOfSpeech: "Noun",
    examples: [
      "The sybarite insisted on only the finest silk sheets and gourmet food.",
      "He lived a life of luxury, a true sybarite.",
    ],
    antonyms: ["ascetic", "hermit"],
    synonyms: ["hedonist", "voluptuary"],
  },
  {
    word: "sycophant",
    meaning:
      "a person who acts obsequiously toward someone important in order to gain advantage",
    partOfSpeech: "Noun",
    examples: [
      "The sycophant flattered his boss with hopes of a promotion.",
      "He was surrounded by sycophants who praised his every move.",
    ],
    antonyms: ["critic", "detractor"],
    synonyms: ["flatterer", "toady"],
  },
  {
    word: "tawdry",
    meaning: "showy but cheap and of poor quality",
    partOfSpeech: "Adjective",
    examples: [
      "The store sold tawdry souvenirs to tourists.",
      "The tawdry jewelry was made of plastic and fake gems.",
    ],
    antonyms: ["elegant", "tasteful"],
    synonyms: ["gaudy", "gimcrack"],
  },
  {
    word: "temperance",
    meaning: "moderation or self-control",
    partOfSpeech: "Noun",
    examples: [
      "The temperance movement advocated for the prohibition of alcohol.",
      "He practiced temperance in all aspects of his life, from eating to spending.",
    ],
    antonyms: ["intemperance", "excess"],
    synonyms: ["moderation", "restraint"],
  },
  {
    word: "tyro",
    meaning: "a beginner or novice",
    partOfSpeech: "Noun",
    examples: [
      "He was a tyro in the kitchen, still learning how to boil water.",
      "The organization was a good place for tyros to gain experience.",
    ],
    antonyms: ["expert", "veteran"],
    synonyms: ["novice", "beginner"],
  },
  {
    word: "untenable",
    meaning: "not able to be defended against attack or objection",
    partOfSpeech: "Adjective",
    examples: [
      "The professor's theory was untenable in the face of new evidence.",
      "The argument was untenable and quickly fell apart.",
    ],
    antonyms: ["defensible", "tenable"],
    synonyms: ["indefensible", "unsustainable"],
  },
  {
    word: "vestige",
    meaning: "a trace of something that is disappearing or no longer exists",
    partOfSpeech: "Noun",
    examples: [
      "The old building was the last vestige of the city's industrial past.",
      "The ancient ritual was a vestige of a forgotten religion.",
    ],
    antonyms: ["presence", "existence"],
    synonyms: ["relic", "remnant"],
  },
  {
    word: "vitriolic",
    meaning: "filled with bitter criticism or malice",
    partOfSpeech: "Adjective",
    examples: [
      "The vitriolic attack from the critic ruined the young artist's reputation.",
      "His vitriolic remarks were meant to hurt his opponent.",
    ],
    antonyms: ["pleasant", "kind"],
    synonyms: ["caustic", "acerbic"],
  },
  {
    word: "wizened",
    meaning: "shriveled or wrinkled with age",
    partOfSpeech: "Adjective",
    examples: [
      "The old man's wizened face was a map of his long life.",
      "The wizened apple was too old to eat.",
    ],
    antonyms: ["smooth", "unwrinkled"],
    synonyms: ["wrinkled", "shrunken"],
  },
];

// Export all groups in an object for easy access
export const ALL_GROUPS = {
  "Group 1": GROUP_1,
  "Group 2": GROUP_2,
  "Group 3": GROUP_3,
  "Group 4": GROUP_4,
  "Group 5": GROUP_5,
  "Group 6": GROUP_6,
  "Group 7": GROUP_7,
  "Group 8": GROUP_8,
  "Group 9": GROUP_9,
  "Group 10": GROUP_10,
  "Group 11": GROUP_11,
  "Group 12": GROUP_12,
  "Group 13": GROUP_13,
  "Group 14": GROUP_14,
  "Group 15": GROUP_15,
  "Group 16": GROUP_16,
  "Group 17": GROUP_17,
  "Group 18": GROUP_18,
  "Group 19": GROUP_19,
  "Group 20": GROUP_20,
  "Group 99": GROUP_99,
};
